...
Seed 是训练、评估、部署并托管 **Taiji** 的项目、产品与运行时。Taiji 是一个**原生认知架构**——从在线预测编码机制构建，而不是 Transformer 的包装。内核从**局部预测误差**中学习（无反向传播、无注意力矩阵、无上下文窗口、运行时无教师模型）；在内核之上，Taiji 拥有自己的表征、持续状态、记忆、目标、规划与行动选择，同时在合适处刻意复用成熟算法（embedding、SSM、MoE 式路由、优化器、检索）。而 Taiji 的核心出发点，是**自我进化**——在学习中修订、生长并重组自己的架构（见[Taiji 是什么](#taiji-是什么)）。
...
项目的出发点不是某个具体机制，而是一个问题：**架构能不能自己发现自己的正确设计？** Taiji 被设计为**自我进化**的：它的感知、记忆、世界模型、技能与结构都是*候选*，系统在学习过程中自己决定修订、生长、分裂、合并或剪枝；它的目标、置信度与感知到的能力缺口驱动何时探索、训练、睡眠与重构。当前的内核、器官合同乃至 Taiji v1 设计都只是提案，不是最终答案。这一方向以核心需求形式明文固定（[Taiji 核心目标](plans/active/TAIJI_CORE_REQUIREMENTS.md) 的 CR-4 结构可塑性、CR-7 睡眠/做梦/玩耍、CR-9 递归改进），并以 A8 作为可证伪的能力反证门槛；"哪些已经可执行、哪些仍是脚手架"在[能力](#能力)与[现状](#现状)中如实标注，不做粉饰。

Taiji 的目标是一座**完整的原生认知架构**（合同见 [Taiji 原生架构 v1](plans/active/TAIJI_NATIVE_ARCHITECTURE_V1.md)）：
...
自我进化（CR-4）的可执行种子，与其余结论一样过 Gate：
