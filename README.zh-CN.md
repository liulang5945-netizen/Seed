...
| H3.6-B dev | 已获正式训练授权；control/treatment 三 seed dev 按冻结合同执行，final 延迟读取，停止门保持有效 |
...
- 当前主线：执行 H3.6-B 三 seed matched dev，完成后冻结 dev 与 bridge ablation，再依据停止门决定是否允许一次性读取 final；Mini 继续后置。
