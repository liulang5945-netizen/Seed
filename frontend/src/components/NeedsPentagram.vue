<template>
  <div class="needs-pentagram" role="img" :aria-label="ariaLabel">
    <svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" class="pentagram-svg">
      ...
      <text v-for="(pt, i) in labelPositions" :key="'label-' + i" :x="pt.x" :y="pt.y" :text-anchor="pt.anchor" :dominant-baseline="pt.baseline" class="pentagram-label">{{ needLabels[needKeys[i]] }} <tspan class="pentagram-value" :class="needLevels[i]">{{ Math.round(needs[needKeys[i]] || 0) }}</tspan></text>
    </svg>
  </div>
</template>
...
const needLevels = computed(() =>
  needKeys.map(k => {
    ...
    return val > 70 ? 'alert' : val >= 40 ? 'watch' : 'calm'
  })
)
...
.pentagram-guide { fill: none; stroke: color-mix(in srgb, var(--muted-foreground) 12%, transparent); stroke-width: 1; }
.pentagram-guide:last-of-type { stroke: color-mix(in srgb, var(--muted-foreground) 26%, transparent); stroke-width: 1.1; }
.pentagram-axis {
  stroke: color-mix(in srgb, var(--muted-foreground) 10%, transparent);
  stroke-width: 0.75;
  transition: stroke 0.4s var(--ease), stroke-width 0.4s var(--ease);
}
.pentagram-axis.critical { stroke: color-mix(in srgb, var(--danger) 46%, transparent); stroke-width: 1.6; }
...
.pentagram-fill { fill: color-mix(in srgb, var(--muted-foreground) 8%, transparent); transition: d 0.6s var(--ease); }
.pentagram-stroke {
  stroke: color-mix(in srgb, var(--muted-foreground) 52%, transparent);
  stroke-width: 1.8;
...
.pentagram-value { transition: fill 0.4s var(--ease); }
.pentagram-value.calm { fill: var(--muted-foreground, var(--text-muted)); font-weight: 500; }
.pentagram-value.watch { fill: var(--foreground); font-weight: 600; }
.pentagram-value.alert { fill: var(--danger); font-weight: 600; }
