...
- ⚠️ **本仓库可能同时存在并行会话（2026-09-19 实测）**：另一个会话曾在同一时段提交 4 个提交，
  其中一条还补上了我漏掉的泄漏面（发布通路 `scripts/release.py`）。
  ⇒ ① 审计/体检结论要标注**取证时刻**，提交前**重跑一次**；
  ② 提交前先看 `git log` 是否出现**非本人**的提交；
  ③ 不要对"我以为是垃圾"的未跟踪/未提交内容做清理 —— **先确认是否正被另一方使用**；
  ④ 同理，`M <file>` 突然消失不一定是自己弄丢的，追查 `git log -- <file>` 与作者/时间可澄清。
