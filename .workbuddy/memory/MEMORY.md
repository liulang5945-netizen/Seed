...
## ⭐ R2 突破：数据多样性 × 对比目标存在交互效应（2026-09-17，**首次正结果**）

- **设计**：scaled 语料（实体池 4×：train 92 条 3.7×、dev 48 条 3×），**同预算**
  （8 epochs × 92 = 736 步 ≈ 标准语料 30×25 = 750 步），两臂只差 λ。
- **结果**：treatment（λ=1）**0.0417 > control（λ=0）0.0208** ⇒ **首次 treatment 优于 control**。
- **对照臂自己从 0.1042 降到 0.0208**（4× 实体让任务变难）⇒ treatment 在更难条件下仍占优。
- **解释**：对比目标需要**足够多样的 (prefix, answer) 配对**才能区分"正确材料"与"打乱材料"
  —— 单一形状/少数实体时两者统计上不可分。
- **界限**：3 seeds、差距约 2 条 episode、未做显著性检验；只试了 4× 一档（无规模扫描）；
  绝对 exact 仍极低（约 2/48 条），**不构成能力收益主张**。
- **判别环境变了**：scaled 语料首次提供了**能区分好坏目标**的实验场 ——
  架构改动 (c) 现在终于**可判别**。
- 产物：`build_taiji_r2_h3_8_corpus_scaled.py`、`r2_h3_8_joint_sequence_scaled_v1.jsonl`、
  `eval_taiji_r2_hobj_matched_dev.py --fixture/--epochs/--max-episodes`（可复用）。

## ⚠️ 上轮"突破"未复现 —— 已降级（2026-09-17 晚，诚实修正）

规模扫描（修正设计：每条恒 4 遍、per-epoch shuffle、三档 4×/8×/16×）**未能复现**
"数据多样性 × 对比目标"的交互：

| scale | train | steps | control (λ=0) | treatment (λ=1) |
|---|---|---|---|---|
| 4× | 176 | 704 | **0.1818** | 0.0000 |
| 8× | 672 | 2688 | **0.2540** | 0.1270 |
| 16× | 2592 | 10368 | 0.1270 | 0.0000 |

=> 上轮 0.0417>0.0208 是**不同构语料（92 条 vs 176-2592 条）+ 差距仅 2 条 + 无显著性检验**
   的单次观察，**已降级为未证实**，不得再引用为"对比目标有效"。

**真正稳健的新读数是对照臂**：**纯 CE 的数据多样性单调改善**
—— 57 条 0.1042 → 176 条 0.1818 → 672 条 **0.2540**（16× 回落 0.1270，读数波动大）。
=> "扩大数据多样性对纯 CE 有效"证据强；对比目标三连败维持。

## ⏸️ 巩固(i-a)完成，**已暂停**（2026-09-17 晚，用户指示）

672 档 lambda=0 六 seeds（4 epochs x 672 = 2688 步/seed，per-epoch shuffle）：
dev exact = [0.381, 0.381, 0.0, 0.369, 0.190, 0.024]，**mean 0.2242 / pstdev 0.1642（双峰）**。

三条同时声明：① 非零确认（6/6 训练正常、4/6 >=0.19）；② "0.2540"不能作点估计，
口径必须是"0.22 +/- 0.16（双峰，6 seeds）"；③ **评测集已变**（dev 672 条 vs 标准 16 条），
"0.2242 vs 0.1042"**不是同一标尺上的提升**。

挂起项（恢复入口）：(c) 架构读出（先设计探针参数与 checkpoint 兼容）；
双峰解释（先做只读 per-seed 形状分解，复用 diag_a 思路）。
工具资产已入库：--fixture/--epochs/--max-episodes、enable_epoch_shuffle、
enable_first_byte_weight、三个诊断/巩固脚本。
结项：plans/reference/M5_R2_CONSOLIDATION_672_20260917.md

**方法教训**：正结果必须先问"能否在相邻设置复现"；交互效应声明前必须做规模扫描
（本轮扫描就是为此设计的，只是第一版设计自身有混杂缺陷，修正版才有效）。
trainer 已支持 per-epoch shuffle（`enable_epoch_shuffle`，默认关 => 行为不变）。

## R2-D8 尺度包进展（2026-09-18，**实现门通过、probe 8/9、按合同路由回评审**）

D3–D7 已把机制轴走完（几何→目标→因子→先验→粒度），D7 char-v1 产生系列最强 dev 读数；
D8 是**机制地图最后一根预注册轴 = 尺度/暴露**（唯一变量：train 组合放大）。

- **实现门通过**：在冻结 builder 里加 `--variant v3`（未另建文件，v1/v2 行为**逐字节不变**，
  已重放验证）；v3 train = **1278 行**（32 对象 × 12 色：6 单字 + 6 双字），
  dev/final **与 v1 逐字节相同**；11/11 门通过。
- **⚠️ 合同笔误**：合同 §2 写"1246 行"，但其自列构成 1152+64+62 = **1278**（差 32）。
  按"形状/模板/配对/顺序规则不变"生成 1278，**未为凑数删行**（删行＝引入规则外自由度）。
- **机制零改动证明**：v1/v2 重放逐字节一致；D7 T1 在 v2 上重放 `bias_end=0.17751`
  与 D7 记录 +0.178 一致、全门 true。
- **probe（v3+T1）8/9 门通过**：`copy_M1=1.0`、**multibyte 576/576**、copy 概率 1.0、
  misbind 0.0426；**唯一失败 `loss_increases_bounded_15pct`**（epoch5→10 +35.7%，
  但随后回落且**末值最优** ⇒ **抖动而非发散**；D7 同类上升仅 +1.8% ⇒ **该门与规模交互**）。
- **机制层比 D7 更强**：`bias_end 0.6549`（D7 0.178）、`combo_same 31/31`（D7 5/7）。
- **纪律**：合同 §4 规定「probe 败 → 回评审、**不进 matched**」⇒ **matched 未启动**，
  **未擅自放宽冻结门**。建议先补另两 seed 重跑 probe（钉死"抖动 vs 不稳定"）再裁决。
- **补证 (c) 已完成**（2026-09-18，给 probe 加 `--seed` 后补跑 918/919）：
  **失败门只在 1/3 seed 出现**（`{loss_increases_bounded_15pct: 1}`），另两 seed 最大跳幅仅
  **+0.39% / +0.71%**（远低于 15%）⇒ 按**补证前预定判据**判为**偶然抖动**。
  **诚实修正**：上一轮"门与规模交互"的推测**不被支持**，已据实修正。
  机制层 **3/3 全过**（copy_M1/multibyte 576/576/intact 均 1.0、misbind ≤0.110、bias_end 全正）。
  附带发现：combo_same/diff 在 917、918 为 `1.0/0.0`、在 919 为 `0.0/1.0` ⇒
  **学到恒定 combo 回答、该形状未条件化**（与"边际退化"诊断模式一致），matched 结项须单列。

## R2-D8 matched dev 结项（2026-09-18，**K1 主门首次通过**）

按 (b) 裁决放行（v3 train 1278 行 × 3 seeds；dev 仍读 v1，两者逐字节相同）：

| 门 | 阈值 | 实测 | 判 |
|---|---|---|---|
| **K1 多字值 `full_rate`（主门）** | ≥0.30 | **0.3621** | **✅ 首次通过** |
| K2 `M4_flip` | ≥0.50 | **0.0000** | ❌ |
| G1 `M3` | ≥0.26 | **0.3101** | ✅ |
| G2 `M4_flip`+`fact_flip` | — | 0.0/0.0 | ❌ |
| G3 misbind drop | ≥0.50 | 0.0000 | ❌ |
| G4 context margin | — | ✅（`no_context_m1_max=0.0`）| ✅ |
| G5（D8 定义） | ≥2 seed >0.10 | ✅（2/3）| ✅ |
| G6 boundary | ≥0.95 | ✅ | ✅ |
| `first_byte` | ≥0.529 | **0.7529** | ✅ |

**per-seed `full_rate` = [0.4310, 0.0000, 0.6552]**；**M3 = 0.2907 / 0.1977 / 0.4419**（最佳 0.4419）。

**三项实质进展（系列第一次）**：① **主门首次通过**；② **M3 均值 0.3101 = 表面天花板 0.1735 的 1.79×**；
③ **`no_context_m1_max = 0.0`（三 seed 全 0）⇒ 无材料时不猜**（"确实在用材料"的最强单点证据）。

**明确缺口**：**`M4_flip_pair`/`fact_flip`/`combo_flip` 三 seed 全 0** ⇒ **复制链建成、绑定/取反未建立**。

**路由**：合同 §5 表（K1 过、K2/G2 败）**精确落第二行**
⇒「复制站稳但绑定精度未达 ⇒ 「绑定 = per-item 率连续函数」检验点，**评审决定再放大或收束**」。
建议 **(α) 再放大一档** 或 **(β) 收束转入口评审**；**助手倾向 (β)**（flip 全零跨 seed 一致 ⇒ 结构性，
用"再多点数据"解释说服力不足）。**需用户裁决**，未自行推进。
结项：`plans/reference/M5_R2_D8_MATCHED_DEV_RESULT_20260918.md`。

## R2-D8 (α) 再放大一档：v4 已建，probe 两门失败（2026-09-18）

用户对 matched 路由第二行裁决「**再放大一档**」⇒ 建 **v4**（**64 对象 × 24 色**，
train **4862** 行 = 值行 1536×3 + unknown 128 + combo 126；dev/final 与 v1 逐字节相同）。
builder 加 `--variant v4` + 共享 gate `_assert_scale_pool_clean(objects, colors, first_new_color)`
（**只有索引 ≥12 的双字需字符纯净**；单字只需与 dev/final 值字无交集）——
**v1/v2/v3 逐字节复现**，51 项契约测试全过。

**v4 probe（seed 917，21.4 min）**：机制层**全过且更强**（`copy_M1=1.0`、多字值 2304/2304、
intact 0.956、misbind 0.122、**`bias_end=2.828`** = v3 的 4.3×）；
但**两门失败**，性质不同：
1. **`within_wall_cap`**（21.4 > 20 min）⇒ **纯"门未适配规模"**（上限是 v3 档 9 min 定的，无机制含义）；
2. **`loss_increases_bounded_15pct`** ⇒ **不是抖动而是实质中期失稳**：
   `4.187 → 0.055 → 0.473(+763%) → … → 0.274`，**且 `copy_M1` 同步 1.0→0.825、epoch25 才回 1.0**
   （**确实丢过已学到的复制行为**）；最终恢复、末值最优（未发散）。
   最可能是 **lr 未随规模调整**（每 epoch micro-batch 160→608），但**证据不足以与"3.8× 规模下机制不稳"分开**。

**纪律**：合同 §4 ⇒ **matched 未启动**，未放宽任何冻结门。**(a2) 补证正在跑**（另两 seed，42 min），
判据预先定好：**2/2 复现 ⇒ 系统性（不能直接放行）**；**0/2 ⇒ 偶发（可判抖动放行）**。
不推荐"先调 lr"——它同时改规模与 lr，会让 v4 失去与 v3 的归因力。

### (a2) 结果：**3/3 系统性复现** ⇒ 3.8× 规模在固定 lr 下不可稳定训练

| seed | outcome | `copy_M1` | multibyte | `bias_end` | 最大 loss 跳幅 |
|---|---|---|---|---|---|
| 20260917 | failed | 1.0000 | 1.0000 | 2.828 | +762.9% |
| **20260918** | failed | **0.8470** | **0.6944** | 7.089 | **+1047.9%** |
| 20260919 | failed | 1.0000 | 1.0000 | 4.828 | +124.3% |

`loss_increases_bounded_15pct` **3/3**、`within_wall_cap` **3/3**；
**seed 918 连机制门一起挂**（copy_M1 0.847、multibyte 0.694、late collapse），
且其 loss **未恢复**（`4.232→0.054→…→0.618→0.605→0.464`，末值远高于早期最优）。

**⇒ 规模-稳定性边界（固定 lr=0.01、固定配方）**：
- **v1(174)→v3(1278, 7.34×)**：同门 **1/3**、可判抖动、matched 正常完成；
- **v3→v4(4862, 再 3.8× = 28× v1)**：同门 **3/3**、且**机制层退化**。
⇒ **v3 之内稳定、v4 之外不稳**。

**裁决选项**：(i) 承认"固定配方的规模上限"、v4 判超参域外负结果 ⇒ **转阶段收束/入口评审**；
(ii) 把"规模适配 lr"立为新变量（**需新预注册、分别检验**）；
(iii) 只修 wall_cap 仍放行 matched（**不推荐**：训练不稳时读数无法归因到机制）。
**助手建议 (i)** —— v4 本为检验"绑定 = per-item 率连续函数"，但其前提"训练稳定"已被证否。
