...
const aclRunner = process.platform === 'win32'
  ? fileURLToPath(import.meta.resolve('@taiji/dsh-sandbox-windows-acl/runner'))
  : undefined

if (aclRunner !== undefined && process.argv[2] === aclRunner) {
  process.argv.splice(1, 1)
  await import('@taiji/dsh-sandbox-windows-acl/runner')
} else if (process.env.DSH_PTC_RUNTIME_NODE === '1') {
  Reflect.deleteProperty(process.env, 'DSH_PTC_RUNTIME_NODE')
  await import('@taiji/dsh-ptc-runtime-node/process')
  ...
  const { runCli } = await import('@taiji/dsh/lib/bin.js')
  await runCli()
} else {
  Reflect.deleteProperty(process.env, selectorName)
  const { runSelectedSubprocessRunner } = await import('@taiji/dsh-subprocess-local/runner')
