...
const RELEASE_ENTRY_PACKAGE = '@taiji/dsh'
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@taiji/')) {
        throw new Error(`${manifestPath} must name an @taiji package`)
      }
      if (name === '@taiji/dsh-root') {
...
class InstalledBundleSmoke {
  ...
  run(): void {
    ...
    try {
      ...
      const bin = resolve(consumerRoot, 'node_modules/@taiji/dsh/lib/bin.js')
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@taiji/') || name === '@taiji/dsh-root')) {
