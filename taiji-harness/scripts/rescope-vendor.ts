/**
 * Rescope the vendored Cordis packages into the `@taiji` scope, and undo
 ...
 * `@taiji/dsh-tool-cordis`, and `cordiverse/cordis`, and makes the
...
const RENAMES: readonly Rename[] = [
  { directory: 'cordis', upstream: 'cordis', scoped: '@taiji/cordis' },
  { directory: 'cosmokit', upstream: 'cosmokit', scoped: '@taiji/cosmokit' },
  { directory: 'schemastery', upstream: 'schemastery', scoped: '@taiji/schemastery' },
  { directory: 'loader', upstream: '@cordisjs/plugin-loader', scoped: '@taiji/cordis-plugin-loader' },
  { directory: 'include', upstream: '@cordisjs/plugin-include', scoped: '@taiji/cordis-plugin-include' },
  { directory: 'group', upstream: '@cordisjs/plugin-group', scoped: '@taiji/cordis-plugin-group' },
  { directory: 'timer', upstream: '@cordisjs/plugin-timer', scoped: '@taiji/cordis-plugin-timer' },
  { directory: 'hmr', upstream: '@cordisjs/plugin-hmr', scoped: '@taiji/cordis-plugin-hmr' },
  { directory: 'logger-console', upstream: '@cordisjs/plugin-logger-console', scoped: '@taiji/cordis-plugin-logger-console' },
]
...
const POSTCONDITIONS: readonly PostCondition[] = [
  { file: 'vendor/cordis/package.json', text: '"name": "@taiji/cordis"', count: 1 },
  { file: 'vendor/hmr/package.json', text: '"name": "@taiji/cordis-plugin-hmr"', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '@taiji\\/cordis', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '!== \'@taiji/cordis\'', count: 1 },
  { file: 'scripts/gen-scoped-events.ts', text: '=== \'@taiji/cordis\'', count: 1 },
  { file: 'packages/typert/generator/src/analyzer.ts', text: '!== \'@taiji/cordis\'', count: 2 },
  { file: 'scripts/check-workspace-constraints.ts', text: '?.[\'@taiji/cordis\']', count: 2 },
  { file: 'packages/boot/app-boot/tsdown.config.ts', text: '[\'@taiji/cordis-plugin-include\']', count: 1 },
  { file: 'tsconfig.base.json', text: '"@taiji/cordis-plugin-loader": ["./vendor/loader/src"]', count: 1 },
  // The vendored README owns this required entry; reject its deletion or duplication.
  { file: 'vendor/README.md', text: '17. **`@taiji` rescope**', count: 1 },
...
const EXACT_EDITS: readonly ExactEdit[] = [
  {
    ...
    replace: "import type Schema from '@taiji/schemastery'",
    expect: 1,
  },
  {
    ...
    replace: 'const MERGE_HEAD = /declare module [\'"](?:@taiji\\/cordis|\\.\\/context\\.ts)[\'"]/',
    expect: 1,
  },
  {
    ...
    replace: `    const peer = manifest.peerDependencies?.['@taiji/cordis']
    const dev = manifest.devDependencies?.['@taiji/cordis']

    if (!peer) errors.push(\`\${label}: @taiji/cordis must be a peerDependency\`)
    if (!dev) errors.push(\`\${label}: @taiji/cordis must also be a devDependency\`)
    if (peer && dev && peer !== dev) {
      errors.push(\`\${label}: @taiji/cordis peer (\${peer}) and dev (\${dev}) ranges must match\`)`,
    expect: 1,
  },
  {
...
`,
  ...
  },
  {
    ...
    find: '      if (!isVendored && !name.startsWith(\'@taiji/\')) {',
  ...
  },
  {
    ...
    replace: 'vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@taiji/cordis` is a peerDependency (+ dev) of every harness package.',
    expect: 1,
  },
  {
    // The client purity gate reads `@taiji/` as "another plugin package".
    ...
    replace: `/**
 * Vendored framework libraries: rescoped into @taiji, so the gate below
 ...
 */
const VENDORED_LIBRARY = /^@taiji\\/(cosmokit|schemastery)(\\/|$)/
...
/** Generated descriptor/codec contribution with no shared runtime identity. */`,
  ...
  },
  {
    ...
    replace: "import { Context } from '@taiji/cordis'",
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \\`@taiji\\` scope. All are MIT-licensed',
    expect: 1,
  },
  {
...
\${vendored.map(row => \`| \\\`\${row.npmName}\\\` | \\\`\${row.upstreamName}\\\` | [\${row.sourceDirectory}](\${row.sourceDirectory}/) | MIT |\`).join('\\n')}`,
  ...
  },
  {
    ...
    replace: 'parseVendoredRows(\'| `cordis/` | `@taiji/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\\n\')',
    expect: 1,
  },
  {
