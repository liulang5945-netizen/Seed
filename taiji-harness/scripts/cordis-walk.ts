...
const MERGE_HEAD = /declare module ['"](?:@taiji\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@taiji/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@taiji/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@taiji/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@taiji/cordis'` Context merge
...
/**
 * Every event name a `declare module '@taiji/cordis'` Events merge declares in one
