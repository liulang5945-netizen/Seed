 ...
 * `@taiji/dsh-*` listed 49 candidate globs and `@taiji/dsh-*\/invariant`
 ...
 * @returns Every directory whose manifest names a `@taiji/dsh-` package and that carries `src`.
 */
 ...
 * `@taiji/dsh-<directory>`. Packages named after something other than
 ...
 *
 * @returns Declared names of every `@taiji/dsh-` package carrying a `src` directory.
 */
...
export function mappedSpecifiers(text: string): Set<string> {
  ...
  for (const match of text.matchAll(/^\s*"(@taiji\/dsh-[^"/]+)":/gm)) {
    const key = match[1]
...
function handWrittenSpecifiers(text: string): Set<string> {
  ...
  const keys = new Set<string>()
  for (const match of outside.matchAll(/^\s*"(@taiji\/[^"]+)":/gm)) {
    const key = match[1]
    if (key !== undefined) keys.add(key)
  }
  return keys
