...
describe('release families', () => {
  it('publishes all current experimental packages', () => {
    ...
    expect(members
      ...
      '@taiji/dsh-experimental-webworker-packer',
  ...
  it('excludes private applications from the publish set', () => {
    ...
    roots.push(root)
    write(join(root, 'apps/public/package.json'), '{"name":"@taiji/dsh-public","version":"0.0.1"}\n')
    write(join(root, 'apps/private/package.json'), '{"name":"@taiji/dsh-private","version":"0.0.1","private":true}\n')

    expect(releaseFamily('dsh').members(root).map(entry => entry.name)).toEqual(['@taiji/dsh-public'])
  })
  ...
  it('bumps private dsh workspaces without adding release tags', () => {
    ...
    const published = member('packages/core/published', '@taiji/dsh-published')
  ...
  it('names one tag for the whole dsh family and one per vendored package', () => {
    ...
    const cli = member('apps/cli', '@taiji/dsh')
    const cordis = { ...member('vendor/cordis', '@taiji/cordis'), version: '4.0.1' }
  ...
  it('rejects a family whose members disagree on the shared version', () => {
    const dsh = releaseFamily('dsh')
    const members = [member('apps/cli', '@taiji/dsh'), { ...member('apps/web', '@taiji/dsh-web-frontend'), version: '0.0.2' }]
  ...
  it('accepts independent vendored versions and rejects an unpublishable one', () => {
    const vendor = releaseFamily('vendor')
    const members = [
      { ...member('vendor/cordis', '@taiji/cordis'), version: '4.0.1' },
      { ...member('vendor/cosmokit', '@taiji/cosmokit'), version: '1.8.2' },
    ]
  ...
  it('publishes a dependency before its consumer, and orders ties by name', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@taiji/dsh-consumer', { dependencies: { '@taiji/dsh-library': 'workspace:^' } }),
      member('packages/a/library', '@taiji/dsh-library'),
      member('packages/a/zebra', '@taiji/dsh-zebra'),
    ]
  ...
  it('reports a runtime dependency cycle instead of emitting an arbitrary order', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@taiji/dsh-left', { dependencies: { '@taiji/dsh-right': 'workspace:^' } }),
      member('packages/a/right', '@taiji/dsh-right', { dependencies: { '@taiji/dsh-left': 'workspace:^' } }),
    ]
  ...
  it('publishes a peer before its consumer', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@taiji/dsh-consumer', { peerDependencies: { '@taiji/dsh-zebra': 'workspace:^' } }),
      member('packages/a/zebra', '@taiji/dsh-zebra'),
    ]
  ...
  it('orders around a peer cycle rather than refusing to publish, and reports the edge it dropped', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@taiji/dsh-left', { peerDependencies: { '@taiji/dsh-right': 'workspace:^' } }),
      member('packages/a/right', '@taiji/dsh-right', { peerDependencies: { '@taiji/dsh-left': 'workspace:^' } }),
    ]
    ...
    expect(plan.droppedPeerEdges).toEqual([
      { consumer: '@taiji/dsh-right', peer: '@taiji/dsh-left' },
    ])
  })
  ...
  it('honours an install edge even when a peer cycle surrounds it', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/base', '@taiji/dsh-base', { peerDependencies: { '@taiji/dsh-consumer': 'workspace:^' } }),
      member('packages/a/consumer', '@taiji/dsh-consumer', {
        dependencies: { '@taiji/dsh-base': 'workspace:^' },
        peerDependencies: { '@taiji/dsh-base': 'workspace:^' },
      }),
    ]
    ...
    expect(plan.droppedPeerEdges).toEqual([
      { consumer: '@taiji/dsh-base', peer: '@taiji/dsh-consumer' },
    ])
  })
  ...
  it('refuses an order that would publish a consumer before a dependency it installs', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/alpha', '@taiji/dsh-alpha', { peerDependencies: { '@taiji/dsh-bravo': 'workspace:^' } }),
      member('packages/a/bravo', '@taiji/dsh-bravo', { peerDependencies: { '@taiji/dsh-charlie': 'workspace:^' } }),
      member('packages/a/charlie', '@taiji/dsh-charlie', { dependencies: { '@taiji/dsh-alpha': 'workspace:^' } }),
    ]
    ...
    expect(() => { dsh.publishOrder(members) }).toThrow(/no publish order honours @taiji\/dsh-charlie -> @taiji\/dsh-alpha/)
  })

  it('ignores devDependencies when ordering', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/alpha', '@taiji/dsh-alpha', { devDependencies: { '@taiji/dsh-zebra': 'workspace:^' } }),
      member('packages/a/zebra', '@taiji/dsh-zebra'),
    ]
  ...
  it('applies the harness payload policy to dsh and keeps upstream payloads for vendored packages', () => {
    const dsh = releaseFamily('dsh')
    const vendor = releaseFamily('vendor')
    const harness = member('packages/a/library', '@taiji/dsh-library')
    const vendored = member('vendor/cordis', '@taiji/cordis')
  ...
  it('drives the installed entry only for the family that publishes one', () => {
    expect(releaseFamily('dsh').installedEntry).toEqual({ packageName: '@taiji/dsh', binPath: 'lib/bin.js' })
...
describe('payload change judgement', () => {
  const sourceShipping = member('vendor/cosmokit', '@taiji/cosmokit', {
    files: ['lib/index.js', 'lib/types/**/*.d.ts', 'src'],
  })
  const buildOutputOnly = member('vendor/cordis', '@taiji/cordis', {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@taiji/dsh-library', { files: ['lib/index.js'] }),
