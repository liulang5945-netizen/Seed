...
const WORKSPACE_ROOT_PACKAGE = '@taiji/dsh-root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@taiji/')) throw new Error(`${normalized} must name an @taiji package`)
...
class DshFamily extends ReleaseFamily {
  ...
  readonly installedEntry = { packageName: '@taiji/dsh', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@taiji/', '')}-v`
  }
