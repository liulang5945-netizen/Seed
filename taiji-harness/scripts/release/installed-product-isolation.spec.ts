...
const roots: string[] = []
const experimental = '@taiji/dsh-experimental-example'
...
describe('installed default-product isolation', () => {
  it('ignores development dependencies and unrelated installed experimental packages', () => {
    const root = fixture()
    const entry = writePackage(root, '@taiji/dsh', {
      dependencies: { core: '1.0.0' },
      devDependencies: { [experimental]: '1.0.0' },
    })
    writePackage(root, 'core', { peerDependencies: { '@taiji/dsh': '1.0.0' } })
  ...
  it('skips the entry package\'s optional bundles, which must be installed, and still rejects other experimental edges', () => {
    const root = fixture()
    const entry = writePackage(root, '@taiji/dsh', { dependencies: { core: '1.0.0', [experimental]: '1.0.0' } })
    writePackage(root, 'core', {})
    writePackage(root, experimental, { dependencies: { '@taiji/dsh-experimental-inner': '1.0.0' } })
    ...
    const missing = writePackage(root, '@taiji/dsh-missing', { dependencies: { absent: '1.0.0' } })
    expect(() => verifyInstalledProductIsolation(missing, ['absent'])).toThrow('optional bundle is missing: @taiji/dsh-missing -> absent')
  })

  it('rejects experimental identities hidden behind an installed alias', () => {
    const root = fixture()
    const entry = writePackage(root, '@taiji/dsh', { dependencies: { safeName: 'file:../prototype' } })
  ...
  it('follows nested installations rather than an unrelated hoisted package', () => {
    const root = fixture()
    const entry = writePackage(root, '@taiji/dsh', { dependencies: { core: '1.0.0' } })
    writePackage(root, 'core', {})
  ...
  it('permits missing optional packages and optional peers but rejects missing dependencies', () => {
    ...
    writePackage(root, '@taiji/dsh', { dependencies: { missing: '1.0.0' } })
