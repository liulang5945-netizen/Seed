...
describe('Python runtime executable builder CLI', () => {
  it('keeps the single-file dispatcher on the Python packaging surface', () => {
    ...
    expect(cliManifest.dependencies).not.toHaveProperty('@taiji/dsh-subprocess-local')
    expect(cliManifest.devDependencies).toHaveProperty('@taiji/dsh-subprocess-local')
    expect(runtimeManifest.dependencies).toHaveProperty('@taiji/dsh-subprocess-local')
    expect(bootstrap).toContain("import('@taiji/dsh/lib/bin.js')")
    expect(bootstrap).toContain('await runCli()')
    expect(bootstrap).toContain("import('@taiji/dsh-subprocess-local/runner')")
