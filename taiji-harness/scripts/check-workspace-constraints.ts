...
const vendoredPackages = new Set([
  ...
  '@taiji/cosmokit',
  '@taiji/schemastery',
  '@taiji/cordis-plugin-loader',
  '@taiji/cordis-plugin-include',
  '@taiji/cordis-plugin-group',
  '@taiji/cordis-plugin-timer',
  '@taiji/cordis-plugin-hmr',
  '@taiji/cordis-plugin-logger-console',
])
const publicNativePackages = new Set([
  '@taiji/node-addon-system',
  '@taiji/node-addon-system-darwin-arm64',
  '@taiji/node-addon-system-darwin-x64',
  '@taiji/node-addon-system-linux-arm64',
  '@taiji/node-addon-system-linux-x64',
])
...
const publicationSourceAllowlist: Readonly<Record<string, readonly string[]>> = {
  '@taiji/node-addon-system': ['src/main.c', 'src/flock.c'],
}
...
const experimentalPackageNamePrefix = '@taiji/dsh-experimental-'
...
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  '@taiji/dsh': ['lib/*.js', 'lib/types/*.d.ts'],
  ...
  '@taiji/dsh-web-frontend': ['dist', '!dist/**/*.map', '!dist/preview.html', '!dist/preview'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  ...
  '@taiji/dsh-app-boot': ['lib/worker/profile-resolution-bootstrap.js'],
  ...
  '@taiji/dsh-client-ui-primitives': ['lib/**/*.css'],
  '@taiji/dsh-client-ui-dockkit': ['lib/**/*.css'],
  '@taiji/dsh-client-ui-sidebar-documentpreview': ['lib/client.*.js'],
  '@taiji/dsh-client-ui-sidebar-terminal': ['lib/client.*.js'],
  '@taiji/dsh-client-web': ['lib/**/*.css', 'lib/apply-injections.js'],
  '@taiji/dsh-client-ui-theme': ['lib/styles'],
  // The CPython side ships as source .py files, published as-is rather than built.
  '@taiji/dsh-experimental-ptc-runtime-python': ['py/**/*.py'],
  '@taiji/dsh-experimental-speech-to-text-sensevoice': ['runtime/assets.json'],
  // The isolated Node bootstrap is a separately launched bundle.
  '@taiji/dsh-ptc-runtime-node': ['lib/process.js'],
  // The Host entry starts its sibling Worker by URL rather than a package export.
  '@taiji/dsh-experimental-inspector': ['lib/worker.js'],
  // Creator's composition guidance travels with the declaration package.
  '@taiji/dsh-agent-preset': ['skills'],
  // The Web Host mounts the default-off settings owner independently of each
  // Agent-scoped delegation-tool instance.
  '@taiji/dsh-tool-subagent': ['lib/model-selection-settings.js'],
  ...
  '@taiji/dsh-session-persistence-jsonl': ['lib/worker.cjs'],
  // The argv-prefix runner entry ships beside the lib as its own bundle;
  // sandbox-local resolves it through the package's ./runner export. tsdown
  // also shares its generated FFI code through a hashed runtime chunk.
  '@taiji/dsh-sandbox-windows-acl': ['lib/runner.js', 'lib/types-*.js'],
  '@taiji/dsh-skill-badge': ['assets'],
  '@taiji/dsh-skill-office': ['assets'],
  '@taiji/dsh-subprocess': ['lib/control.js'],
  ...
  '@taiji/dsh-experimental-webworker-packer': ['bin.js', 'lib/repository-*.js'],
  ...
  '@taiji/dsh-headless': ['lib/json-stream-*.js'],
}
...
export function checkDshFamilyVersion(manifest: PackageManifest, expected: string | undefined): string | undefined {
  ...
  if (name !== '@taiji/dsh' && name?.startsWith('@taiji/dsh-') !== true) return undefined
...
export function checkWorkspaceManifest({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (manifest.name?.startsWith('@taiji/')) {
  ...
  if (dir.startsWith('apps/') && dir !== desktopApplicationDirectory && manifest.name?.startsWith('@taiji/')) {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@taiji/dsh-')) {
    const peer = manifest.peerDependencies?.['@taiji/cordis']
...
export function checkExperimentalDependencyIsolation(
  ...
  for (const { dir, manifest } of manifests) {
    ...
    const offered = manifest.name === '@taiji/dsh' ? new Set(optionalBundles) : new Set<string>()
