...
describe('tierExternalDeps', () => {
  it('limits the LibreOffice exception to its reviewed package identity and MPL terms', () => {
    ...
    for (const dependency of [
      ...
      { name: '@taiji/dsh-libreoffice-kit', license: 'MPL-2.0' },
  ...
  it('tiers by declaring area, not by the declaring section name', () => {
    const { manifests, names } = workspace({
      ...
      'packages/test-support/loader-smoke/package.json': { name: '@taiji/dsh-loader-smoke', dependencies: { 'smoke-helper': '^1' } },
      'packages/test-support/client-runtime/package.json': { name: '@taiji/dsh-client-test-runtime', dependencies: { 'test-lib': '^1' } },
      ...
      'packages/mcp/mcp-client/package.json': { name: '@taiji/dsh-mcp-client', dependencies: { 'protocol-sdk': '^1' }, devDependencies: { 'protocol-fixture-server': '^1' } },
      'apps/cli/package.json': { name: '@taiji/dsh-cli', dependencies: { 'cli-lib': '^1', '@taiji/dsh-mcp-client': 'workspace:^' } },
    })
  ...
  it('keeps a package runtime when any shipping area declares it, and excludes workspace links', () => {
    const { manifests, names } = workspace({
      ...
      'packages/interaction/tui/package.json': { name: '@taiji/dsh-tui', dependencies: { shared: '^1', '@taiji/dsh-cli': 'workspace:^' } },
      'apps/cli/package.json': { name: '@taiji/dsh-cli' },
    })

    expect(tierExternalDeps(manifests, names).get('shared')).toBe(true)
    expect(tierExternalDeps(manifests, names).has('@taiji/dsh-cli')).toBe(false)
  })
})
...
describe('parseVendoredRows', () => {
  ...
  it('yields nothing when the table columns change, so the generator fails loud', () => {
    expect(parseVendoredRows('| `cordis/` | `@taiji/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\n')).toEqual([])
  })
