...
function declaration(
  ...
  return {
    name: short.startsWith('@') ? short : '@taiji/dsh-client-' + short,
...
describe('source package uses', () => {
  ...
  it('counts type imports, module augmentations, dynamic imports, and JSX', () => {
    const uses = collectSourcePackageUses('feature.tsx', [
      "import type { A } from '@taiji/dsh-a/subpath'",
      "declare module '@taiji/dsh-client-ui-slots' {}",
      "const load = () => import('@taiji/dsh-b/remote')",
...
describe('package modes', () => {
  ...
  it('requires every preloaded external to have a parser preload row', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      + '"@taiji/dsh-client-bootstrap/client" has no matching PARSER_PRELOAD_IDS row in '
...
describe('module requests', () => {
  it('rejects runtime requests from one client feature package to another dynamic row', () => {
    const ui = declaration('ui', {
      external: ['@taiji/dsh-client-slots/client'],
      runtimeSourceUses: {
        '@taiji/dsh-client-slots': ['packages/client/ui/src/client/index.ts'],
      },
    })
    ...
    expect(collectClientPackageViolations(facts([], { declarations: [ui, slots] }))).toEqual([
      ...
      + '"@taiji/dsh-client-slots/client"; import shared types only or call an injected Cordis service',
    ])
  })

  it('rejects stale externals and accepts a runtime import outside client feature packages', () => {
    const gateway = {
      ...declaration('@taiji/dsh-api-gateway'), manifest: 'packages/api/gateway/package.json',
    }
    const stale = { ...declaration('@taiji/dsh-api-stale', {
      external: ['@taiji/dsh-api-gateway/client'],
    }), manifest: 'packages/api/stale/package.json' }
    const live = { ...declaration('@taiji/dsh-api-live', {
      external: ['@taiji/dsh-api-gateway/client'],
      runtimeSourceUses: {
        '@taiji/dsh-api-gateway': ['packages/api/live/src/client/index.ts'],
      },
      runtimeSourceSpecifiers: {
        '@taiji/dsh-api-gateway/client': ['packages/api/live/src/client/index.ts'],
      },
    }), manifest: 'packages/api/live/package.json' }
    expect(collectClientPackageViolations(facts([], {
      declarations: [gateway, stale, live],
    }))).toEqual([
      stale.manifest + ': dsh.client.external "@taiji/dsh-api-gateway/client"'
  ...
  it('requires the exact external subpath to be imported at runtime', () => {
    ...
    const subject = { ...declaration('@taiji/dsh-api-session-controller', {
      external: ['@taiji/dsh-api-gateway/client'],
      runtimeSourceUses: {
        '@taiji/dsh-api-gateway': ['packages/api/session-controller/src/client/index.ts'],
      },
      runtimeSourceSpecifiers: {
        '@taiji/dsh-api-gateway/remote': ['packages/api/session-controller/src/client/index.ts'],
      },
    ...
    expect(collectClientPackageViolations(facts([], { declarations: [gateway, subject] }))).toEqual([
      subject.manifest + ': dsh.client.external "@taiji/dsh-api-gateway/client"'
      + ' has no runtime import or re-export in production source; remove the stale declaration',
    ])
  })
  ...
  it('rejects duplicates, empty values, self-requests, and missing suppliers', () => {
    const ui = declaration('ui', {
      external: ['', '@taiji/dsh-client-ui', '@taiji/dsh-missing', '@taiji/dsh-missing'],
      inject: ['', '@taiji/dsh-a', '@taiji/dsh-a'],
    })
  ...
  it('rejects synchronous module-request cycles but ignores inject cycles', () => {
    const a = { ...declaration('@taiji/dsh-api-a', {
      external: ['@taiji/dsh-api-b'],
      inject: ['@taiji/dsh-api-b'],
      runtimeSourceUses: { '@taiji/dsh-api-b': ['packages/api/a/src/client.ts'] },
      runtimeSourceSpecifiers: { '@taiji/dsh-api-b': ['packages/api/a/src/client.ts'] },
    }), manifest: 'packages/api/a/package.json' }
    const b = { ...declaration('@taiji/dsh-api-b', {
      external: ['@taiji/dsh-api-a'],
      inject: ['@taiji/dsh-api-a'],
      runtimeSourceUses: { '@taiji/dsh-api-a': ['packages/client/b/src/client.ts'] },
      runtimeSourceSpecifiers: { '@taiji/dsh-api-a': ['packages/client/b/src/client.ts'] },
...
describe('manifest declarations', () => {
  ...
  it('fixes malformed declaration entries without changing dependency sections', () => {
    ...
    const subject = pkg('feature', {
      external: ['', 'react', '@taiji/dsh-client-feature', '@taiji/dsh-missing'],
      inject: ['', '@taiji/dsh-agent', '@taiji/dsh-agent'],
      sourceUses: {
        '@taiji/dsh-agent': ['packages/client/feature/src/index.ts'],
        '@taiji/dsh-client-ui-slots': ['packages/client/feature/src/view.tsx'],
      },
      dependencies: {
        [CORDIS]: 'workspace:^',
        '@taiji/dsh-agent': 'workspace:*',
      },
      peerDependencies: {
        '@taiji/dsh-client-ui-slots': 'workspace:^',
        '@taiji/cordis-plugin-loader': 'workspace:^',
      },
      devDependencies: {},
    })
    const slots = declaration('ui-slots', { dynamic: false })
    ...
    expect(fixed.dsh.client).toMatchObject({
      external: ['@taiji/dsh-missing'],
      inject: ['@taiji/dsh-agent'],
    })
