...
const roots: string[] = []
const experimental = '@taiji/dsh-experimental-prototype'
const core = '@taiji/dsh-core'
const base = '@taiji/dsh-base'
...
function fixture(): string {
  ...
  roots.push(root)
  write(root, 'apps/cli/package.json', { name: '@taiji/dsh', dependencies: { [core]: 'workspace:^' } })
  write(root, 'apps/cli/src/bin.ts', 'export {}\n')
  write(root, 'apps/web/package.json', { name: '@taiji/dsh-web-frontend' })
  write(root, 'apps/web/index.html', '<script type="module" src="/src/main.ts"></script>')
  write(root, 'apps/web/src/main.ts', 'export {}\n')
  write(root, 'python/sdk-runtime/package.json', { name: '@taiji/dsh-python-runtime' })
  ...
  write(root, preset, [{ insert: [{ name: '@taiji/dsh-agent-preset', config: { id: 'standard', plugins: [{ name: core }] } }] }])
  write(root, 'packages/preset/agent-preset/package.json', { name: '@taiji/dsh-agent-preset' })
...
describe('default product isolation', () => {
  ...
  it('ships an optional bundle switched off: its graph is outside the product, its name stays out of imports and defaults', () => {
    const root = fixture()
    const layer = '@taiji/dsh-experimental-layer'
  ...
  it('requires each optional bundle to be a runtime dependency that declares a bundle patch', () => {
    ...
    expect(verifyDefaultProductIsolation(root).failures.join('\n')).toContain(`@taiji/dsh dependencies -> ${experimental}`)
  })
  ...
  it('rejects experimental names absent from the inventory and paths with another package name', () => {
    const root = fixture()
    manifest(root, 'apps/cli/package.json', { dependencies: { '@taiji/dsh-experimental-missing': '*' } })
    ...
    const failures = verifyDefaultProductIsolation(root).failures.join('\n')
    expect(failures).toContain('@taiji/dsh-experimental-missing')
  ...
  it.each([
    ...
    [{ name: '@taiji/cordis-plugin-group', config: [{ name: experimental }] }],
    [{ name: '@taiji/cordis-plugin-include', config: { patches: [{ insert: [{ name: experimental }] }] } }],
  ...
  it('follows Include files while ignoring ordinary plugin config data', () => {
    ...
    expect(verifyDefaultProductIsolation(root).failures).toEqual([])
    write(root, patch, [{ name: '@taiji/cordis-plugin-include', config: { path: './nested.yml' } }])
