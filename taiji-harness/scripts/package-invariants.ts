...
function checkManifest(
  ...
  if (owner.packageName === '@taiji/dsh-invariants') return
  ...
  const peerRange = manifest.peerDependencies?.['@taiji/dsh-invariants']
  if (developmentOnlyInvariant ? peerRange !== undefined : peerRange !== expectedRange) {
    addViolation(violations, owner.manifestPath, developmentOnlyInvariant
      ? '@taiji/dsh-invariants must not be a peerDependency under this package dependency policy'
      : '@taiji/dsh-invariants must be a workspace:^ peerDependency')
  }
  if (manifest.devDependencies?.['@taiji/dsh-invariants'] !== expectedRange) {
    addViolation(
      violations,
      owner.manifestPath,
      `@taiji/dsh-invariants must be a ${expectedRange} devDependency`,
    )
  }
}
...
function checkBuild(
  ...
  if (hasCompanion
    && owner.packageName !== '@taiji/dsh-invariants'
