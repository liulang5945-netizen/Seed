...
function fixture(name = '@taiji/dsh-core'): {
  root: string
  ...
  roots.push(root)
  const installDir = join(root, 'global', 'node_modules', '@taiji', 'dsh')
  const installed = join(installDir, 'node_modules', name)
  const installAnchor = pkg(installDir, '@taiji/dsh', 0, { [name]: '*' })
...
describe('runtime resolution', { concurrent: false }, () => {
  it('computes an immutable runtime resolution without writing profile packages', async () => {
    ...
    expect(resolution.entries.find(entry => entry.name === '@taiji/dsh-core')).toMatchObject({
    ...
    expect(createRequire(join(installationOnly.profilesDir, 'entry.cjs'))('@taiji/dsh-core'))
      .toEqual({ marker: 1 })
  })
  ...
  it.each([
    ...
    if (origin === 'bundle') {
      pkg(dirname(f.installAnchor), '@taiji/dsh', 0)
    ...
    expect(resolution.entries.find(entry => entry.name === '@taiji/dsh')?.declarer).toBe(f.installAnchor)
  ...
  it('routes ESM and CommonJS through the same installation entry', async () => {
    ...
    expect(require('@taiji/dsh-core')).toEqual({ marker: 1 })
    expect(require.resolve('@taiji/dsh-core')).toBe(join(f.installed, 'index.cjs'))
    const parent = pathToFileURL(join(f.profile.dir, 'entry.mjs')).href
    expect(resolveFrom('@taiji/dsh-core', parent)).toBe(pathToFileURL(join(f.installed, 'index.js')).href)
    expect(resolveFrom('@taiji/dsh-core', parent, { type: 'javascript' }))
      .toBe(pathToFileURL(join(f.installed, 'index.js')).href)
    expect(resolveFrom('@taiji/dsh-core', parent)).toBe(pathToFileURL(join(f.installed, 'index.js')).href)
    expect(import.meta.resolve('@taiji/dsh-core', parent)).toBe(pathToFileURL(join(f.installed, 'index.js')).href)
    expect(await importFrom('@taiji/dsh-core', parent)).toMatchObject({ marker: 1 })
  })
  ...
  it('intercepts directly above an application-owned profile outside the shared profiles directory', async () => {
    ...
    pkg(join(f.root, 'node_modules', '@taiji/dsh-core'), '@taiji/dsh-core', 2)
    const profile = {
      ...f.profile,
      dir: profileDir,
      patchPath: join(profileDir, 'cordis.patch.yml'),
    ...
    expect(require('@taiji/dsh-core')).toEqual({ marker: 1 })
    expect(require.resolve('@taiji/dsh-core', { paths: [profileDir] }))
      .toBe(join(f.root, 'node_modules', '@taiji/dsh-core', 'index.cjs'))
    const parent = pathToFileURL(join(profileDir, 'entry.mjs')).href
    expect(resolveFrom('@taiji/dsh-core', parent)).toBe(pathToFileURL(join(f.installed, 'index.js')).href)
    expect(await importFrom('@taiji/dsh-core', parent)).toMatchObject({ marker: 1 })
    expect(registration.packageDir('@taiji/dsh-core', parent)).toBe(f.installed)
    expect(createRequire(join(f.root, 'outside.cjs'))('@taiji/dsh-core')).toEqual({ marker: 2 })
  })

  it('does not reuse a default route for explicit CommonJS paths', async () => {
    const f = fixture()
    const alternative = join(f.root, 'alternative')
    const alternativePackage = join(alternative, 'node_modules', '@taiji/dsh-core')
    pkg(alternativePackage, '@taiji/dsh-core', 2)
    const registration = installRuntimeInterception(await resolutionOf(f))
    registrations.push(registration)
    const require = createRequire(join(f.profile.dir, 'entry.cjs'))
    expect(require.resolve('@taiji/dsh-core')).toBe(join(f.installed, 'index.cjs'))
    expect(require.resolve('@taiji/dsh-core', { paths: [alternative] })).toBe(join(alternativePackage, 'index.cjs'))
    expect(thrownError(() => require.resolve('@taiji/dsh-core', { paths: [f.profile.dir] })))
      .toMatchObject({ code: 'MODULE_NOT_FOUND' })
    expect(thrownError(() => require.resolve('@taiji/dsh-core', {
    ...
    file(join(invalid, 'node_modules', '@taiji/dsh-core', 'package.json'), '{')
    expect(() => { require.resolve('@taiji/dsh-core', { paths: [invalid, f.profile.dir] }) })
  ...
  it('keeps earlier explicit CommonJS paths ahead of a profile-local failure', async () => {
    ...
    const local = join(f.profile.dir, 'node_modules', '@taiji/dsh-core')
    file(join(local, 'package.json'), JSON.stringify({
      name: '@taiji/dsh-core', version: '2.0.0', exports: './missing.cjs',
    }))
    const alternative = join(f.root, 'alternative')
    const selected = join(alternative, 'node_modules', '@taiji/dsh-core', 'index.cjs')
    pkg(dirname(selected), '@taiji/dsh-core', 3)
    const require = createRequire(join(f.profile.dir, 'entry.cjs'))
    const paths = [alternative, f.profile.dir]
    expect(require.resolve('@taiji/dsh-core', { paths })).toBe(selected)
  ...
  it('preserves a missing legacy main error before an explicit profile path', async () => {
    const f = fixture()
    const invalid = join(f.root, 'invalid')
    const selected = join(invalid, 'node_modules', '@taiji/dsh-core')
    file(join(selected, 'package.json'), JSON.stringify({ name: '@taiji/dsh-core', main: './missing.cjs' }))
    const require = createRequire(join(f.profile.dir, 'entry.cjs'))
    const paths = [invalid, f.profile.dir]
    const nativeError = thrownError(() => require.resolve('@taiji/dsh-core', { paths }))
    const registration = installRuntimeInterception(await resolutionOf(f))
    registrations.push(registration)
    const runtimeError = thrownError(() => require.resolve('@taiji/dsh-core', { paths }))
  ...
  it('preserves a missing legacy main error for explicit paths when a runtime entry is unavailable', async () => {
    const f = fixture()
    const selected = join(f.root, 'node_modules', '@taiji/dsh-core')
    file(join(selected, 'package.json'), JSON.stringify({ name: '@taiji/dsh-core', main: './missing.cjs' }))
    const alternative = join(f.root, 'alternative')
    pkg(join(alternative, 'node_modules', '@taiji/dsh-core'), '@taiji/dsh-core', 2)
    const resolution = await resolutionOf(f)
    rmSync(f.installed, { recursive: true })
    const require = createRequire(join(f.profile.dir, 'entry.cjs'))
    const paths = [f.profile.dir, alternative]
  ...
  it('keeps a profile-local package ahead of the resolution', async () => {
    ...
    file(join(f.profile.dir, 'package.json'), JSON.stringify({
      ...
      private: true,
      dependencies: { '@taiji/dsh-core': '*', 'linked-local': '*' },
    }))
    const localResolution = join(f.profile.dir, 'node_modules', '@taiji/dsh-core')
    pkg(localResolution, '@taiji/dsh-core', 2)
    ...
    const resolution = await resolutionOf(f)
    expect(resolution.localPackageNames).toEqual(['@taiji/dsh-core', 'linked-local'])
    const registration = installRuntimeInterception(resolution)
    registrations.push(registration)
    const require = createRequire(join(f.profile.dir, 'entry.cjs'))
    expect(require('@taiji/dsh-core')).toEqual({ marker: 2 })
    expect(require.resolve('@taiji/dsh-core', { paths: [f.profile.dir] }))
      .toBe(join(localResolution, 'index.cjs'))
    const parent = pathToFileURL(join(f.profile.dir, 'entry.mjs')).href
    expect(resolveFrom('@taiji/dsh-core', parent)).toBe(
      pathToFileURL(join(f.profile.dir, 'node_modules', '@taiji/dsh-core', 'index.js')).href,
    )
  })
  ...
  it('preserves an npm alias package self-reference', async () => {
    ...
    file(f.installAnchor, JSON.stringify({
      ...
      type: 'module',
      exports: { import: './index.js', require: './index.cjs' },
      dependencies: { '@taiji/dsh-core': '*', 'real-name': '*' },
    }))
    ...
    expect(() => createRequire(join(invalidScope, 'inside.cjs')).resolve('@taiji/dsh-core'))
      .toThrow(/Invalid package config/u)
    expect(() => resolveFrom('@taiji/dsh-core', pathToFileURL(join(invalidScope, 'inside.mjs')).href))
      .toThrow(/Invalid package config/u)
  ...
  it('keeps package imports aliases on the resolution route', async () => {
    ...
    file(join(f.profile.dir, 'package.json'), JSON.stringify({
      name: 'dsh-profile-web',
      private: true,
      imports: { '#@taiji/dsh-core': '@taiji/dsh-core' },
    }))
    const resolution = await createRuntimeResolution({
      installAnchor: f.installAnchor,
      profile: f.profile,
    ...
    expect(() => require.resolve('#@taiji/dsh-core')).toThrow(/Cannot find module/u)
    expect(() => resolveFrom('#@taiji/dsh-core', nativeParent)).toThrow(/Cannot find package/u)

    const registration = installRuntimeInterception(resolution)
    registrations.push(registration)
    const runtimeParent = pathToFileURL(join(nested, 'entry-runtime.mjs')).href
    expect(require.resolve('#@taiji/dsh-core')).toBe(join(f.installed, 'index.cjs'))
    expect(resolveFrom('#@taiji/dsh-core', runtimeParent)).toBe(pathToFileURL(join(f.installed, 'index.js')).href)
  })
  ...
  it('imports package aliases through the resolution and reports the original importer', async () => {
    ...
    file(join(f.profile.dir, 'package.json'), JSON.stringify({
      name: 'dsh-profile-web',
      private: true,
      imports: { '#library': '@taiji/dsh-core', '#missing': '@taiji/dsh-core/missing' },
    }))
    const registration = installRuntimeInterception(await resolutionOf(f))
    registrations.push(registration)
    const parent = pathToFileURL(join(f.profile.dir, 'entry.mjs')).href
  ...
  it('does not inherit package imports across node_modules', async () => {
    const f = fixture()
    file(join(f.profile.dir, 'package.json'), JSON.stringify({
      name: 'dsh-profile-web', private: true, imports: { '#@taiji/dsh-core': '@taiji/dsh-core' },
    }))
    const nested = join(f.profile.dir, 'node_modules', 'manifestless', 'entry.cjs')
    file(nested, '')
    const require = createRequire(nested)
    const nativeMessage = thrownMessage(() => require.resolve('#@taiji/dsh-core'))
    const registration = installRuntimeInterception(await resolutionOf(f))
    registrations.push(registration)

    expect(thrownMessage(() => require.resolve('#@taiji/dsh-core'))).toBe(nativeMessage)
  })
  ...
  it('falls through a missing local CommonJS subpath to the resolution', async () => {
    ...
    file(join(f.profile.dir, 'package.json'), JSON.stringify({
      name: 'dsh-profile-web',
      private: true,
      dependencies: { '@taiji/dsh-core': '*' },
    }))
    file(join(f.installed, 'package.json'), JSON.stringify({
      name: '@taiji/dsh-core', version: '1.0.0', type: 'module', main: './index.cjs',
    }))
    ...
    const local = join(f.profile.dir, 'node_modules', '@taiji/dsh-core')
    file(join(local, 'package.json'), JSON.stringify({
      name: '@taiji/dsh-core', version: '2.0.0', type: 'module', main: './index.cjs',
    }))
    file(join(local, 'index.cjs'), 'module.exports = { marker: 2 }\n')
    const resolution = await resolutionOf(f)
    expect(resolution.localPackageNames).toEqual(['@taiji/dsh-core'])
    const registration = installRuntimeInterception(resolution)
    registrations.push(registration)

    expect(createRequire(join(f.profile.dir, 'entry.cjs')).resolve('@taiji/dsh-core/only-install.cjs'))
      .toBe(join(f.installed, 'only-install.cjs'))
    rmSync(join(local, 'package.json'))
    expect(createRequire(join(f.profile.dir, 'entry.cjs')).resolve('@taiji/dsh-core/legacy-install.cjs'))
  ...
  it('treats null exports as legacy CommonJS package resolution', async () => {
    ...
    file(join(f.installed, 'package.json'), JSON.stringify({
      name: '@taiji/dsh-core', version: '1.0.0', type: 'module', exports: null,
    }))
    const installedSubpath = join(f.installed, 'only-install.cjs')
    file(installedSubpath, 'module.exports = { marker: 8 }\n')
    const local = join(f.profile.dir, 'node_modules', '@taiji/dsh-core')
    file(join(local, 'package.json'), JSON.stringify({
      name: '@taiji/dsh-core', version: '2.0.0', type: 'module', exports: null,
    }))
    const resolution = await createRuntimeResolution({
      installAnchor: f.installAnchor,
      profile: f.profile,
    ...
    expect(() => require.resolve('@taiji/dsh-core/only-install.cjs')).toThrow(/Cannot find module/u)

    const registration = installRuntimeInterception(resolution)
    registrations.push(registration)
    expect(require.resolve('@taiji/dsh-core/only-install.cjs')).toBe(installedSubpath)
  })
  ...
  it('stops a local CommonJS probe before the resolution fallback position', async () => {
    ...
    file(join(f.profile.dir, 'node_modules', '@taiji/dsh-core', 'package.json'), JSON.stringify({
      name: '@taiji/dsh-core', version: '2.0.0', main: './index.cjs',
    }))
    file(join(f.profile.dir, 'node_modules', '@taiji/dsh-core', 'index.cjs'), 'module.exports = {}\n')
    file(join(f.root, 'node_modules', '@taiji/dsh-core', 'package.json'), JSON.stringify({
      name: '@taiji/dsh-core', version: '3.0.0', exports: './index.cjs',
    }))
    file(join(f.root, 'node_modules', '@taiji/dsh-core', 'index.cjs'), 'module.exports = {}\n')
    const require = createRequire(join(f.profile.dir, 'entry.cjs'))
    expect(thrownError(() => require.resolve('@taiji/dsh-core/only-install.cjs')))
  ...
  it('keeps a legacy CommonJS package without a manifest ahead of the resolution', async () => {
    const f = fixture()
    file(join(f.profile.dir, 'node_modules', '@taiji/dsh-core', 'index.js'), 'module.exports = { marker: 2 }\n')
    const registration = installRuntimeInterception(await resolutionOf(f))
    registrations.push(registration)
    expect(createRequire(join(f.profile.dir, 'entry.cjs'))('@taiji/dsh-core')).toEqual({ marker: 2 })
  })

  it('keeps a manifestless local CommonJS subpath ahead of the resolution', async () => {
    const f = fixture()
    const localSubpath = join(f.profile.dir, 'node_modules', '@taiji/dsh-core', 'sub.cjs')
    file(localSubpath, 'module.exports = { marker: 2 }\n')
    const require = createRequire(join(f.profile.dir, 'entry.cjs'))
    expect(require.resolve('@taiji/dsh-core/sub.cjs')).toBe(localSubpath)
  ...
  it('keeps a local extensionless CommonJS package ahead of the resolution', async () => {
    ...
    const require = createRequire(join(f.profile.dir, 'entry.cjs'))
    expect(require.resolve('@taiji/dsh-core')).toBe(local)
  ...
  it('keeps a local legacy main outside its package directory ahead of the resolution', async () => {
    const f = fixture()
    const local = join(f.profile.dir, 'node_modules', '@taiji/dsh-core')
    const outside = join(f.profile.dir, 'node_modules', '@taiji', 'outside.cjs')
    file(join(local, 'package.json'), JSON.stringify({ name: '@taiji/dsh-core', main: '../outside.cjs' }))
    file(outside, 'module.exports = { marker: 2 }\n')
    const require = createRequire(join(f.profile.dir, 'entry.cjs'))
    expect(require.resolve('@taiji/dsh-core')).toBe(outside)
  ...
  it('keeps a missing local legacy main error ahead of the resolution', async () => {
    const f = fixture()
    const local = join(f.profile.dir, 'node_modules', '@taiji/dsh-core')
    file(join(local, 'package.json'), JSON.stringify({ name: '@taiji/dsh-core', main: './missing.cjs' }))
    const require = createRequire(join(f.profile.dir, 'entry.cjs'))
    const nativeError = thrownError(() => require.resolve('@taiji/dsh-core'))
    const registration = installRuntimeInterception(await resolutionOf(f))
    registrations.push(registration)
    const runtimeError = thrownError(() => require.resolve('@taiji/dsh-core'))
  ...
  it('keeps a profile-local CommonJS package file ahead of the resolution', async () => {
    const f = fixture()
    file(join(f.profile.dir, 'node_modules', '@taiji/dsh-core.js'), 'module.exports = { marker: 2 }\n')
    const registration = installRuntimeInterception(await resolutionOf(f))
    registrations.push(registration)
    expect(createRequire(join(f.profile.dir, 'entry.cjs'))('@taiji/dsh-core')).toEqual({ marker: 2 })
    expect(resolveFrom('@taiji/dsh-core', pathToFileURL(join(f.profile.dir, 'entry.mjs')).href)).toBe(
  ...
  it('preserves the ESM and CommonJS behavior of an empty local package directory', async () => {
    const f = fixture()
    mkdirSync(join(f.profile.dir, 'node_modules', '@taiji/dsh-core'), { recursive: true })
    const registration = installRuntimeInterception(await resolutionOf(f))
    registrations.push(registration)
    expect(createRequire(join(f.profile.dir, 'entry.cjs'))('@taiji/dsh-core')).toEqual({ marker: 1 })
    expect(() => resolveFrom(
      '@taiji/dsh-core', pathToFileURL(join(f.profile.dir, 'entry.mjs')).href,
  ...
  it('keeps the resolution ahead of packages above the shared fallback position', async () => {
    ...
    const require = createRequire(join(f.profile.dir, 'entry.cjs'))
    expect(require('@taiji/dsh-core')).toEqual({ marker: 1 })
    expect(await importFrom('@taiji/dsh-core', pathToFileURL(join(f.profile.dir, 'entry.mjs')).href))
  ...
  it('continues the original ancestor lookup after a resolution subpath miss', async () => {
    ...
    file(join(profileDir, 'node_modules', '@taiji/dsh-core', 'package.json'), JSON.stringify({
      name: '@taiji/dsh-core', version: '2.0.0',
    }))
    file(join(f.installed, 'package.json'), JSON.stringify({
      name: '@taiji/dsh-core', version: '1.0.0', type: 'module', main: './index.cjs',
    }))
    const ancestorSubpath = join(home, 'node_modules', '@taiji/dsh-core', 'sub.cjs')
    ...
    const require = createRequire(join(profileDir, 'entry.cjs'))
    expect(() => require.resolve('@taiji/dsh-core')).toThrow(/Cannot find module/u)
    expect(require.resolve('@taiji/dsh-core/sub.cjs')).toBe(ancestorSubpath)
  ...
  it('uses later explicit CommonJS paths when the profile has no matching package', async () => {
    ...
    const alternative = join(f.root, 'alternative')
    const alternativeSubpath = join(alternative, 'node_modules', '@taiji/dsh-core', 'sub.cjs')
    file(alternativeSubpath, 'module.exports = { marker: 4 }\n')
    const registration = installRuntimeInterception(await resolutionOf(f))
    registrations.push(registration)

    expect(createRequire(join(f.profile.dir, 'entry.cjs')).resolve('@taiji/dsh-core/sub.cjs', {
  ...
  it('occupies the interception layer package directory for a resolution hit and continues above it on a CommonJS subpath miss', async () => {
    const f = fixture()
    file(join(f.installed, 'package.json'), JSON.stringify({
      name: '@taiji/dsh-core', version: '1.0.0', main: './index.cjs',
    }))
    // Ordinary profile lookup replaces the shared copy with the runtime entry; explicit paths still read the shared copy.
    const shared = join(f.root, 'profiles', 'node_modules', '@taiji/dsh-core')
    file(join(shared, 'package.json'), JSON.stringify({ name: '@taiji/dsh-core', version: '9.0.0' }))
    file(join(shared, 'sub.cjs'), 'module.exports = { marker: 9 }\n')
    file(join(shared, 'sub.js'), 'module.exports = { marker: 9 }\n')
    const home = join(f.root, 'node_modules', '@taiji/dsh-core')
    file(join(home, 'package.json'), JSON.stringify({ name: '@taiji/dsh-core', version: '3.0.0' }))
    ...
    const require = createRequire(join(f.profile.dir, 'entry.cjs'))
    expect(require.resolve('@taiji/dsh-core')).toBe(join(f.installed, 'index.cjs'))
    expect(require.resolve('@taiji/dsh-core/sub.cjs')).toBe(join(home, 'sub.cjs'))
    expect(require('@taiji/dsh-core/sub.cjs')).toEqual({ marker: 3 })
    expect(require.resolve('@taiji/dsh-core/sub.cjs', { paths: [f.profile.dir] })).toBe(join(shared, 'sub.cjs'))
    ...
    const parent = pathToFileURL(join(f.profile.dir, 'entry.mjs')).href
    expect(resolveFrom('@taiji/dsh-core', parent)).toBe(pathToFileURL(join(f.installed, 'index.cjs')).href)
    await expect(importFrom('@taiji/dsh-core/sub.js', parent)).rejects.toMatchObject({ code: 'ERR_MODULE_NOT_FOUND' })
  ...
  it('treats a profile symlink into .dsh-module-fallback as an ordinary local package', async () => {
    const f = fixture()
    file(join(f.profile.dir, 'package.json'), JSON.stringify({
      name: 'dsh-profile-web', private: true, dependencies: { '@taiji/dsh-core': '*' },
    }))
    const target = join(f.root, 'old-dsh', 'node_modules', '@taiji', 'dsh-core')
    pkg(target, '@taiji/dsh-core', 9)
    const owned = join(f.profile.dir, '.dsh-module-fallback', 'node_modules', '@taiji/dsh-core')
    const projected = join(f.profile.dir, 'node_modules', '@taiji/dsh-core')
    ...
    const require = createRequire(join(f.profile.dir, 'entry.cjs'))
    const parent = pathToFileURL(join(f.profile.dir, 'entry.mjs')).href
    expect(require('@taiji/dsh-core')).toEqual({ marker: 9 })
    expect(require.resolve('@taiji/dsh-core')).toBe(join(target, 'index.cjs'))
    expect(resolveFrom('@taiji/dsh-core', parent)).toBe(pathToFileURL(join(target, 'index.js')).href)
    expect(await importFrom('@taiji/dsh-core', parent)).toMatchObject({ marker: 9 })
    expect(registration.packageDir('@taiji/dsh-core', parent)).toBe(projected)
  ...
  it.each(lookupMatrix())('$title', async (matrixCase) => {
    ...
    const name = { installation: '@taiji/dsh-core', bundle: 'bridge', unlisted: 'left-pad' }[kind]
    const f = fixture()
  ...
  it('does not attach routed CommonJS anchors to the importing module', async () => {
    ...
    expect(internal.Module._resolveFilename('@taiji/dsh-core', parent, false)).toBe(join(f.installed, 'index.cjs'))
  ...
  it('reports routed CommonJS failures with the native require stack', async () => {
    ...
    const nativeError = thrownError(() => nativeRequire.resolve('@taiji/dsh-core/missing.cjs'))
    const registration = installRuntimeInterception(resolution)
    registrations.push(registration)
    const runtimeRequire = createRequire(join(f.profile.dir, 'entry-runtime.cjs'))
    const runtimeError = thrownError(() => runtimeRequire.resolve('@taiji/dsh-core/missing.cjs'))
  ...
  it('reports routed ESM failures from the original importer', async () => {
    ...
    expect(thrownMessage(() => resolveFrom('@taiji/dsh-core/private', parent)))
  ...
  it('leaves an invalid resolution manifest error to Node', async () => {
    ...
    const parent = pathToFileURL(join(f.profile.dir, 'entry.mjs')).href
    expect(() => resolveFrom('@taiji/dsh-core', pathToFileURL(f.installAnchor).href))
      .toThrow(/Invalid package config/u)
    const registration = installRuntimeInterception(resolution)
    registrations.push(registration)

    expect(() => resolveFrom('@taiji/dsh-core', parent)).toThrow(/Invalid package config/u)
  })
  ...
  it('does not fall back after the resolution selects a missing exports target', async () => {
    const f = fixture()
    file(join(f.installed, 'package.json'), JSON.stringify({
      name: '@taiji/dsh-core', version: '1.0.0', type: 'module', exports: './missing.cjs',
    }))
    pkg(join(f.root, 'node_modules', '@taiji/dsh-core'), '@taiji/dsh-core', 2)
    const registration = installRuntimeInterception(await resolutionOf(f))
    registrations.push(registration)

    expect(() => createRequire(join(f.profile.dir, 'entry.cjs')).resolve('@taiji/dsh-core'))
      .toThrow(/Cannot find module/u)
  })
  ...
  it('does not fall back after the resolution selects a missing legacy main', async () => {
    const f = fixture()
    file(join(f.installed, 'package.json'), JSON.stringify({
      name: '@taiji/dsh-core', version: '1.0.0', type: 'module', main: './missing.cjs',
    }))
  ...
  it('rejects changing an existing package mapping without publishing it', async () => {
    ...
    registration.replace({
      ...first,
      entries: first.entries.map(entry => entry.name === '@taiji/dsh-core'
    ...
    expect(() => {
      registration.replace({ ...first, localPackageNames: ['@taiji/dsh-core'] })
    }).toThrow(/requires a process restart/u)
    expect(() => {
      registration.replace({ ...first, entries: first.entries.filter(entry => entry.name !== '@taiji/dsh-core') })
    }).toThrow(/requires a process restart/u)
    expect(() => {
  ...
  it('leaves non-package and out-of-scope metadata lookups to native resolution', async () => {
    ...
    expect(registration.packageDir('@taiji/dsh-core/private', profileParent)).toBe(f.installed)
  ...
  function linkedPluginFixture(peerDependencies: Record<string, string> | undefined = { '@taiji/dsh-core': '*' }): {
    ...
    pkg(installDir, '@taiji/dsh', 0, { '@taiji/dsh-core': '*', '@taiji/dsh-util': '*' })
    pkg(join(installDir, 'node_modules', '@taiji', 'dsh-util'), '@taiji/dsh-util', 2)
    ...
    const writeManifest = (peers: Record<string, string> | undefined): void => {
      file(join(linkedRoot, 'package.json'), JSON.stringify({
        ...
        type: 'module',
        exports: { import: './index.js', require: './index.cjs' },
        imports: { '#core': '@taiji/dsh-core' },
        dependencies: { 'zod': '*', '@taiji/dsh-util': '*', 'helper': '*' },
        ...(peers === undefined ? {} : { peerDependencies: peers }),
        devDependencies: { '@taiji/dsh-core': '*' },
      }))
    }
    ...
    pkg(join(linkedRoot, 'node_modules', '@taiji', 'dsh-core'), '@taiji/dsh-core', 21)
    pkg(join(linkedRoot, 'node_modules', 'zod'), 'zod', 22)
    pkg(join(linkedRoot, 'node_modules', '@taiji', 'dsh-util'), '@taiji/dsh-util', 23)
    ...
    pkg(helper, 'helper', 25, { sibling: '*' }, { '@taiji/dsh-core': '*' })
  ...
  it.each([
    ...
    pkg(installDir, '@taiji/dsh', 0, Object.fromEntries(queries.map(({ name }) => [name, '*'])))
  ...
  it('occupies only the peer names of a linked plugin at its own node_modules', async () => {
    ...
    await expectResolution('@taiji/dsh-core', f.installed, 1)
    // A cache-busting query on the importer, as hot module replacement re-imports it, changes nothing.
    expect(resolveFrom('@taiji/dsh-core', `${parent}?t=1`)).toBe(pathToFileURL(join(f.installed, 'index.js')).href)
    ...
    await expectResolution('@taiji/dsh-util', join(linkedRoot, 'node_modules', '@taiji', 'dsh-util'), 23)
    ...
    await expectResolution('@taiji/dsh-core', f.installed, 1, helperImporter)
  ...
  it('continues above a linked plugin after a CommonJS subpath miss in an occupied peer', async () => {
    const { f, linkedRoot } = linkedPluginFixture()
    file(join(f.installed, 'package.json'), JSON.stringify({ name: '@taiji/dsh-core', version: '1.0.0', main: './index.cjs' }))
    const above = join(f.root, 'work', 'node_modules', '@taiji', 'dsh-core')
    file(join(above, 'package.json'), JSON.stringify({ name: '@taiji/dsh-core', version: '3.0.0' }))
    file(join(above, 'sub.cjs'), 'module.exports = { marker: 3 }\n')
    file(join(linkedRoot, 'node_modules', '@taiji', 'dsh-core', 'sub.cjs'), 'module.exports = { marker: 21 }\n')
    const registration = installRuntimeInterception(await resolutionOf(f))
    registrations.push(registration)
    const require = createRequire(join(linkedRoot, 'entry.cjs'))
    expect(require.resolve('@taiji/dsh-core')).toBe(join(f.installed, 'index.cjs'))
    // The occupied name never falls back to the devDependency copy at the same layer.
    expect(require.resolve('@taiji/dsh-core/sub.cjs')).toBe(join(above, 'sub.cjs'))
    expect(require('@taiji/dsh-core/sub.cjs')).toEqual({ marker: 3 })
  })
  ...
  it('reads a linked plugin manifest at every resolution', async () => {
    ...
    const registration = installRuntimeInterception(await resolutionOf(f))
    registrations.push(registration)
    const devCopy = join(linkedRoot, 'node_modules', '@taiji', 'dsh-core')
    const require = createRequire(join(linkedRoot, 'entry.cjs'))
    const parent = pathToFileURL(join(linkedRoot, 'entry.mjs')).href
    expect(require.resolve('@taiji/dsh-core')).toBe(join(devCopy, 'index.cjs'))
    expect(resolveFrom('@taiji/dsh-core', parent)).toBe(pathToFileURL(join(devCopy, 'index.js')).href)
    expect(registration.packageDir('@taiji/dsh-core', parent)).toBe(devCopy)

    writeManifest({ '@taiji/dsh-core': '*' })
