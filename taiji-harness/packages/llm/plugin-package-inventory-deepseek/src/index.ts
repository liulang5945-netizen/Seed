 ...
 * @module @taiji/dsh-plugin-package-inventory-deepseek
 */
...
import type { Entry, EntryTree } from '@taiji/cordis-plugin-loader'
...
async function collectActivePluginPackages(
  ...
  if (sessionId !== undefined && ctx.get('agentPresets') !== undefined) {
    ...
    if (agent !== undefined) {
      ...
      const { standingMountFor } = await import('@taiji/dsh-agent-preset-registry')
