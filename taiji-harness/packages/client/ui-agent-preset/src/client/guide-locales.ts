...
export const guideEn = {
  ...
  guideCordisUsage: [
    '### Add a UI',
    '> Create a Taiji Harness plugin that adds a project notes entry to the sidebar. Let me browse the Markdown files in this workspace and preview a selected note. Install it and verify that the page opens.',
...
export const guideZh: Record<PresetGuideKey, string> = {
  ...
  guideCordisExplanation: [
    ...
    '### 插件与模式的关系',
    '插件为 Taiji Harness 增加能力，例如工具、服务连接或界面入口。模式是一份 Agent 预设，用来选择任务可用的工具，并约定 Agent 的工作方式。自定义模式中也可以使用自己开发的插件。',
    '### 怎样让成果生效',
    '可以要求 Agent 完成安装并验证实际效果。插件可能即时加载，也可能需要重启，取决于修改内容；新建的模式在创建新任务时选择。',
  ].join('\n\n'),
  guideCordisUsage: [
    '### 添加一个界面',
    '> 帮我写一个 Taiji Harness 插件，在侧栏增加「项目笔记」入口，列出当前工作区的 Markdown 文件，点击后能预览内容。完成安装并验证页面能打开。',
