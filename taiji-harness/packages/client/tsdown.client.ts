 ...
 * Everything else under @taiji/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^(?:@taiji\/dsh-(?:file-reference|session|llm|tools|brand|deque|output-retention|typert-protocol|util-crypto|util-values|util-workspace-path)(?:\/|$)|@taiji\/dsh-token-meter\/client$|@taiji\/dsh-native-command\/types$|@taiji\/dsh-host-open-in-app\/shared$|@taiji\/dsh-plugin-manager\/registry$|@taiji\/dsh-agent-preset-registry\/display$|@taiji\/dsh-spill-policy\/notice$)/
...
const GENERATED_REMOTE = /^@taiji\/dsh-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
...
function clientConfig(id: string, entry: string, clientBanner?: (fileName: string) => string | undefined): UserConfig {
  ...
  return {
    ...
    plugins: [{
      ...
      // inline, and every other @taiji value import is a build error — a
      ...
      resolveId(source: string) {
        if (!source.startsWith('@taiji/')) return null
...
function clientInputIsolation(id: string): {
  ...
  const experimental = id.startsWith('@taiji/dsh-experimental-')
