...
工具需要 `ctx.terminals`——必须挂载一个后端——以及用于指引章节的系统提示词服务。后台发送还额外要求任务服务及其面向模型的控制器（`@taiji/dsh-tool-jobs`）。
...
- **后台模式要求任务接口面**——`run_in_background` 同时需要 `@taiji/dsh-jobs` 及其面向模型的控制器（`@taiji/dsh-tool-jobs`）；缺少时会拒绝该参数。
