 ...
 *
 * @module @taiji/dsh-tool-fs-search/search-core
 */
...
import { ItemRetainer, TextRetainer } from '@taiji/dsh-output-retention'
import type { RetainedItems } from '@taiji/dsh-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@taiji/dsh-subprocess'
 ...
 * `@taiji/dsh-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`@taiji/dsh-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@taiji/dsh-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@taiji/dsh-tool-fs-search/presentation} `grepSearchMeta`)
