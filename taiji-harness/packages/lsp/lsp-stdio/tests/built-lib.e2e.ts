...
/**
 * Keyless built-artifact smoke: plain Node imports `@taiji/dsh-lsp` and
 * `@taiji/dsh-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@taiji/dsh-lsp')
      const LspLocal = await import('@taiji/dsh-lsp-stdio')
      const { default: LocalFileSystem } = await import('@taiji/dsh-fs-local')
      const { default: LocalSubprocessRuntime } = await import('@taiji/dsh-subprocess-local')
