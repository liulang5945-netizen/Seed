...
import * as CommandInvariant from '@taiji/dsh-commands/invariant'
import InvariantRegistry, { InvariantError } from '@taiji/dsh-invariants'
import SessionStore, { SessionId, SessionSeq, type Session } from '@taiji/dsh-session'
import { CommandId } from '@taiji/dsh-commands'
...
describe('command lifecycle invariants', () => {
  ...
  it.each([-1, 1.5, 1])('rejects invalid or non-prior sourceEventSeq %s', async (sourceEventSeq) => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@taiji/dsh-commands',
    }))
  })
