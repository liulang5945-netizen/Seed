...
import { SESSION_FORMAT_VERSION, SessionLogOffset, SessionSeq } from '@taiji/dsh-session'
...
import { createUserMessage, MessageId } from '@taiji/dsh-llm'
import { snapshotSubagentDescriptor } from '@taiji/dsh-subagent'
import { createInboxStub, mountAgentLoopTestDependencies, mountAgentLoopTestHarness } from '@taiji/dsh-agent-loop-testkit'
import type { Agent, Inbox } from '@taiji/dsh-agent'
import type { SessionEvent, SessionHeader, SessionId } from '@taiji/dsh-session'
...
describe('Remote Agent and Session lookup policy', () => {
  ...
  it('deduplicates a cold resume across Agent and Session parameters', async () => {
    ...
    const resumedSession = { id: sessionId, header: meta, events: [] } as unknown as import('@taiji/dsh-session').Session
