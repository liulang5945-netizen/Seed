...
async function load(mode?: string): Promise<{ ctx: Context; root: string; model: ScreenshotModel }> {
  ...
  const modules = new Map<string, unknown>([
    ['@taiji/dsh-computer-use', ComputerUse],
    ...
    ['@taiji/dsh-experimental-computer-use-cua-driver-mcp', Provider],
  ])
  const configPath = join(root, 'cordis.yml')
  await writeFile(configPath, JSON.stringify([...modules.keys()].map(name => ({
    id: name === '@taiji/dsh-experimental-computer-use-cua-driver-mcp' ? 'computer-use-driver' : undefined,
    name,
    config: name === '@taiji/dsh-experimental-computer-use-cua-driver-mcp'
      ? {
      ...
      : name === '@taiji/dsh-attachment-local'
        ? { dshHome: root }
        : name === '@taiji/dsh-agent-loop' ? { agents: [] } : {},
  }))))
