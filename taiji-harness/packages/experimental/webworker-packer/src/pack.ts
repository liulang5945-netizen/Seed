 ...
 * @module @taiji/dsh-experimental-webworker-packer/src/pack
 */
...
export { DEFAULT_ROOT } from '@taiji/dsh-experimental-webworker-runtime'
...
function sweepImage(
  ...
  for (let entry = queue.shift(); entry !== undefined; entry = queue.shift()) {
    ...
    try {
      ...
      const external = importer.startsWith('node_modules/') && !importer.startsWith('node_modules/@taiji/')
...
function materialize(
  ...
  for (let entry = queue.shift(); entry !== undefined; entry = queue.shift()) {
    ...
    for (const field of ['dependencies', 'peerDependencies'] as const) {
      ...
      // (@taiji/cordis is a peerDependency of every harness package),
