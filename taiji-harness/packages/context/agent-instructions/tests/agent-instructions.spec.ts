...
import * as AgentInstructions from '@taiji/dsh-agent-instructions'
import LlmRuntime, { createUserMessage, ToolCallId, type Message, type MessageSource, type StreamChunk } from '@taiji/dsh-llm'
import type { ContextFormed } from '@taiji/dsh-llm'
import SessionStore, { SessionId, SessionSeq, type SessionEvent, type SurfaceIntent, type UserMessage } from '@taiji/dsh-session'
...
import AgentLoop, { turnBoundaryProjectionDefinition } from '@taiji/dsh-agent-loop'
import { FileSystem, FsTargetKey, FsVersion } from '@taiji/dsh-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
} from '@taiji/dsh-agent-instructions'
import {
...
describe('workspace context instruction discovery', () => {
  ...
  it('labels the default DSH home as ~/.dsh when HOME points at the configured default', async () => {
    ...
    try {
      ...
      const isolated = await import('@taiji/dsh-agent-instructions')
