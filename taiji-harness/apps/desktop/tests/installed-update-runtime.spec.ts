  ...
  it('retags only release-owned packages and dependencies while preserving source and third-party versions', async () => {
    await fixture(async (directory) => {
      ...
      for (const version of versions) {
        ...
        expect(descriptor.sharedPackages.find(entry => entry.name === '@taiji/cordis')?.version).toBe(source.version)
        expect(descriptor.sharedPackages.find(entry => entry.name === '@taiji/dsh')?.version).toBe(version)
        const metadata = JSON.parse(await readFile(join(runtime, 'node_modules/@taiji/dsh/package.json'), 'utf8')) as {
          dependencies: Record<string, string>
        }
        expect(metadata.dependencies['@taiji/dsh-desktop-host']).toBe(version)
        expect(metadata.dependencies['@taiji/cordis']).toBe(source.version)
      }
