...
function desktopRelease(): DesktopRelease {
  ...
  if (version !== dshVersion) {
    throw new Error(`desktop runtime: Electron ${version} must bind the same version of @taiji/dsh, found ${dshVersion}`)
  }
...
async function main(): Promise<void> {
  try {
    ...
    writeFileSync(join(DSH_OUTPUT_ROOT, 'package.json'), `${JSON.stringify({
      name: '@taiji/dsh-desktop-runtime', private: true, version: release.version, type: 'module',
