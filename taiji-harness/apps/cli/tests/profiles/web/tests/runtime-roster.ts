...
import type {} from '@taiji/dsh-client-modules'
...
export function experimentalRuntimeReferences(roster: RuntimeRoster): string[] {
  ...
  return [...references].filter(reference => reference.includes('@taiji/dsh-experimental-')
    || (reference.startsWith('file:') && (fileURLToPath(reference).replaceAll('\\', '/').includes('/packages/experimental/')
      || modulePackage(reference)?.startsWith('@taiji/dsh-experimental-')))).sort()
}
