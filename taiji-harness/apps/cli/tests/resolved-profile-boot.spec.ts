...
vi.mock('@taiji/dsh-app-boot', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@taiji/dsh-app-boot')>()
  return {
    ...actual,
    boot: vi.fn(),
...
vi.mock('@taiji/dsh-http-proxy', () => ({ installProxyFromEnvironment: vi.fn() }))
