...
M4.V2.R0 已完成：连续成长量尺合同已版本化、内容寻址，R7/R10/R12 已生成只读语义审计；M4.V2.R1 已在真实 v10 joint-training checkpoint 上完成零变化迁移 smoke；R2 已完成 fast/slow 局部状态、真实 wake replay、三臂对照和 3 个 course seed/order 的 S/G formal Gate；R3 已把一个 zero-gated adaptive residual bridge 接入真实 observation→prediction/credit 主路径，gate=0、local credit、lesion、fresh restore 和 rollback smoke 全通过；R4 已完成 pressure/decision 主路径接线、zero-impact candidate artifact、独立 shadow materialization 和一次 candidate-only residual training smoke，bare/fresh restore、续步、parent 冻结和 rollback 均已覆盖；候选尚未通过短课程、matched-capacity 对照或准入。校准超上限时已正确 fail-closed，修正后的 epsilon=`0.0176304`；整体仍保持 `can_promote=false`。当前唯一下一步是 **M4.V2.R4：运行短 S/G shadow canary，生成 pressure-driven、frozen、R3 fixed-capacity、random growth 与 matched-capacity fixed-large 的对照报告**。Skill/MCP、客户端热插拔、provider、视觉和 CUDA 均保留在统一计划的解冻顺序中。
