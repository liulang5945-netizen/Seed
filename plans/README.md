...
| [03 详细推进方案](active/roadmap/03_CURRENT_EXECUTION.md) | 暂停状态、唯一队首、统一任务切片设计、串行出口与停止点 |
...
| [唯一完整VISION](reference/VISION_FUTURE_TECHNOLOGY.md) | §8.4–8.9细化能力形成：任务分布、表示/目标、跨时间信用、自由运行与适应后表现；仍是待评审候选 |
...
1. 当前不运行研究。唯一下一步是评审并冻结一个最小任务族的能力形成合同；不再重复已出现后继报告的P3b结案，不追加同质训练或先清全部技术债。
2. 下一研究设计围绕同一任务消费链展开：工程接线、联合行为贡献、反馈学习与原生语言分别验收；R2语言缺口不删除，也不让全部语言门阻止早期隔离集成。没有同时启动在线、扩面或VISION支线。
