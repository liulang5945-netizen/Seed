...
| [**P3b 结项报告结构与允许表述预案**](reference/M5_P3B_RESULT_REPORTING_PLAN_20260915.md) | **写于 `stages_recorded = 0` 时**（本地 01:38），不含任何结果。把结项最容易犯的错——"看着数决定怎么写"——提前堵死：**表 A** 逐 tick 双臂分数 + 主效应判定列（必须同时给 `main_effect_verdict()` 的标签，不许只写差值）、**表 B** J1–J5 逐条 × 逐臂（J4 的 A/H 分支永久写 `untested`）、**表 C** 停止条件轨迹、**表 D** 八项"缺一项即不完整"的披露清单；数值口径唯一（`tally.machine_normalised`，`null` 记 `untested` 不记 0 不删行）；**9 条禁止表述**（"未检测到效应"≠"无关"、单边提升≠分布、on-stream 与 holdout 差≠效应、能力断言须 B 人工复核已填、"先止损"≠"分布更差"、禁止任何"CI 已绿"）；结项最小动作清单 6 步。预案自身按冻结件纪律处理：缺结构另立文档，不回改 |
