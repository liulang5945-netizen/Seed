...
**P0 等 replay 配对诊断已完成；P1.1 数据合同已修复并通过；P2 pilot 未晋级；P2.1 输出/行动链诊断、P2.2 安全 bridge、P2.3 continuation 数据合同/targeted learning、P2.4 retention、P2.5 novel-composition probe、P2.6 novel K2 learning、P2.7 independent holdout generalization、P3.0 checkpoint/interrupt-resume contract、P3.1 single-cell 状态整合预检、P3.2 K→G owner-transfer 预检、P3.3 G candidate data-signal canary、P3.3 G-only learning Gate、P3.4 行为信号 Gate、P3.5 reobserve-aware G-only learning Gate、P3.6 独立行为 holdout/保持 Gate、P4.0 固定容量压力扫描、P4.1 公平容量合同预检和 P4.2 隔离训练/公平容量归因 Gate 已完成。** P3.6 在 2 个全新 project、4 条全新 path 上保持 trained-G utility `4.0`、zero-step `2.65`，行为目标命中 `4/4` 对 `1/4`；P4.0 在 5 个新案例、width `2/4/8/12` 上观察到 residual `0.32/0/0.54/0.59`，序列长度 `1/4/16` utility 均为 `0.6375`；P4.1 为 20 个集合建立 9 维候选集上下文、22 参数 fixed-large reference 与 context-lesion effective 13 参数 reference，所有参考 checkpoint 独立恢复且零 fit；P4.2 两个 seed 的 fixed-small/context-aware-small 平均 holdout utility 均为 `0.68`，fixed-large 为 `0.65875`，没有稳定容量收益，训练后旧行为保持 Gate 失败。归因继续为 `inconclusive`，`growth_admitted=false`、`can_promote=false`；唯一下一步改为 P4.3 保持约束下的增量学习 Gate，不能把保持退化直接写成“需要扩张神经元”。CUDA、客户端外围和 promotion 继续冻结。

阶段已按“研究审计完成、模型能力尚未验收”收束；本轮完成了 P2 小预算训练 pilot、P2.1–P2.7、P3.0–P3.6、P4.0 压力扫描、P4.1 合同预检与 P4.2 隔离训练，但没有 promotion 成绩。**P4.0 只证明固定 G 在候选宽度变化下存在选择压力，P4.1 只证明公平输入/恢复边界已准备，P4.2 进一步证明当前训练臂没有稳定容量收益且会破坏旧行为保持；下一步只执行 P4.3 保持约束下的增量学习 Gate。** P5 及客户端/CUDA 保留为后续路线，不并行开工；完整成果分类、停止条件及文档边界均在当前计划中。
