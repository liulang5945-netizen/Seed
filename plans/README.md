...
- 主线已从 provider watchdog/interaction-group 深挖重排为 Workbench Closure W0–W7；W0 的 native 合同、只读环境、core router、runtime capability projection 和审计链已开始落地，但前端尚未消费 native projection，因此仍不能宣称第一个真实纵切片完成；随后才允许推进 IDE 语言选择、写入/终端、MCP、自主循环和产品残留迁移。
- 2026-08-29 W0 首批纵切片已验证：`ActionIntent → ToolCall → WorkbenchEnvironment → Outcome` 能在真实仓库文件上执行，过期 snapshot、路径越界和错误值域均 fail-closed；原有 structured-tool 回归、OpenAPI、编译和 lint 通过。当前遗留边界是 `WorkspaceView/MonacoEditor` 仍使用 Legacy 文件 API，前端 native capability/event projection、checkpoint continuation 和 packaged-client canary 尚未完成。
