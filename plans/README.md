...
- M2-1 已排除 F2→F1 的错误读出串扰；M2-2a～2c 已建立 content-addressed 的 phase-A/phase-B 真实语料隔离、可恢复 sequence-only runner 与 seed 11 checkpoint fresh-process 预检；M2-2d/e 又实测排除了“缺少 byte replay”解释。M2-2f 已完成 F1 predictive readout 隔离、v8/v1→v9/v2 断点迁移，并在 clean revision 的真实 seed 11 course 与 fresh-process 复验中证明 action/memory/identity readout 全部逐位受保护、B4 保持 `1.0`、old/new 四项 BPB 均略改善；但 B2 从 `0.870` 降至 `0.813`，完整 F5 因而仍失败。memory field payload 未变，证据将剩余干扰收窄为 F1 对共享 fabric 表征/检索线索的持久更新。M2-2g 已完成可恢复的单变量合同：F1 可只训练独立 predictive readout 而不写 shared fabric，且该选择进入 v3 checkpoint、corpus digest、resume mismatch gate 和报告；旧 v1/v2 checkpoint 一律解释为 fabric-plastic 并已用真实 v2 `last.pt` 只读复验。随后同一旧 parent 的 v3 seed 11 predictor-only 1 MiB course 与全新进程复验均通过：B2/B4 保持，old/new 四项 BPB 均改善，shared fabric 和行动性读出逐位不变。这是「共享 fabric 持久写入造成 B2 干扰」的正向单变量证据，但还不是三 seed F5 晋级或最终成长架构。M2-2h 现已完成代码级所有权合同：F1 新增独立 receptor map 与零初始稀疏时间残差，局部误差只可写该 residual 与 F1 decoder；`taiji-native-v10`/state v7、joint v4 checkpoint、corpus digest、phase audit、resume mismatch 和 v8/v9/v1～v3 迁移均被显式化。定向回归已证明 private context 会成长、shared fabric/F4/F2 不写、断点可恢复；尚未产生新的 1 MiB 数值结论。当前唯一开发步骤是从同一 v3 seed 11 parent 在 clean revision 建立 v4 private-context predictor-only preflight，并用全新进程只读恢复复核，再决定是否允许正式 course；不得把永久冻结误作解决方案。
