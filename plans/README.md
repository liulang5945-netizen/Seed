...
1. 本轮只完善开发指导，仓库研究主线保持R2；P1失败后的二次评审与唯一入口见03。必要局部实验按[02 §2.5](active/roadmap/02_GATES_AND_CI.md)有条件准入：明确判别价值、对照、累计预算及停止/返回动作；已授权且未触发停止线可按约定推进，不逐次全架构审批。当前已停止配置不会因本轮规则修订自动重开，A–E也不依次试遍。
