...
**唯一推荐下一步：把 WP-1 已确认取值写成路线 B 冻结版预注册（新文件）——待用户许可，本轮不启动。** **2026-09-15 取值已定案（全取上限档）**：D1 = 主判据 `all_singleton_oracle`（最强参照，`required 1.65`）｜D2 = `create` 行全部 3 条语言路由｜D3 = 旧 1.65 留作历史、新判据逐格公布 `required`｜D4 = 旧 144/88 降级为开发回归、新面独立冻结｜**D5 = 落地 `m4_failure_handoff`**｜N2 = 落地前预注册 14 个消费文件（5 个真判断点）并把 `all_members_blocked` 定为一类终局结果｜**N1a = 开**（升为 WP-6）｜N1a-2 = T2 所需成员间证据通道单列。其中 **D5 的选项已有通过四项伪影审计、在 12 context / 6 变体 / 5 种子下复现、并在全部 11 个可表达结构上定位了增益范围的实测支撑** —— 不落地，H2 协作主张在当前机制下不可达；落地前仍须先处理 **N2**（N1 使其更必要：三格各产生 24 次该停止原因），并按 N1 的界限读证据（**N1a** binder 仍未突破，是唯一能把独立结构因素抬过 1 的杠杆；**N1b/N1c** 样本规模已由 WP-1.5 扩面**闭合**：每格 6 context / 7 种子）。**因 D1 取最强参照而实测 `+2.000` 正好贴 `ceiling_gain 2.0`（零余量），WP-1.5 满档扩面是冻结版落盘前的必过门——该门已于 2026-09-15 通过**（四条出口全过、两跑字节相同；扩面后逃生通道 `required` 由 2/2 变 5/6，k 维度首次有余量，但增益维度仍零余量）。
执行顺序已按工作包修订：**WP-1 决策（2026-09-15 取值已确认）→ WP-1.5 扩面（只读，冻结前必过门）→ 路线 B 冻结版预注册 → WP-2 停止原因语义 → WP-3 落地（五条可机检出口：反事实 ≡ 实现 / 冻结面仍 0 回归 2 改善 / 达标面 `interleaved>0` 且增益 `>1.65` / 全量失败集合不新增（基线 851/27/0/1）/ 历史报告只加"规则版本"指针不改数字）→ WP-4 B1 入场（五件探针 + 训练前六门）→ WP-5 下游链**；**WP-6 binder 议题（N1a → N1a-2）**挂在 WP-3 之后，把现有证据冻结为 binder v1 基线再动冻结面。**2026-09-15 修订（上限优先，取值已确认）**：用户偏好上限更高而非成本更低的方案 ⇒ D1 取**最强参照** `all_singleton_oracle`（口径更正：实测 `+2.000` 本来就是对该参照算的，`required 1.65`；但 `+2.000 = ceiling_gain 2.0` ⇒ **零余量**）、D2 取 create 行**全部 3 条**语言路由、D5 **落地**、N1a **开**并升为正式包。**授权边界**：用户同时明确"先不开始实际推进"⇒ 冻结版预注册、WP-2、WP-3、WP-4、WP-6 **一律待另行许可**。**WP-1.5 只读扩面已于 2026-09-15 跑完**（上限档每格 6 context × 7 种子；探针经 `--contexts-per-cell` / `--seed-offsets` 驱动、**默认值不变** ⇒ 无旗标仍逐字节复现归档报告；34 项合同测试钉住默认值与"有效性门不因扩面而放松"）：四条出口全过、两跑字节相同 ⇒ **冻结前必过门已通过**，但**未据此启动任何后续包**。当前无进行中的实验。（WP-2 的消费表**已由** [N2 处置审查](reference/M5_B0_N2_STOP_REASON_DISPOSITION_20260914.md) **交付**：实时消费面 **14 个文件**、其中 **5 个真判断点**，加固轮记录的 11 为历史值；剩冻结版语义预注册。）D1–D5 已明确**不等于**可以启动 B1 训练；不直接加特征列重跑旧 Gate。详见[推进计划修订](reference/M5_B0_CLOSEOUT_AND_PLAN_REVISION_20260914.md)。
...
| [B0 复合任务候选与路线 B 预注册草案](reference/M5_B0_ROUTE_B_PREREGISTRATION_DRAFT_20260913.md) | **未冻结**（已由上方冻结版接管取值）；H1 路由 / H2 协作 / H3 排序分列判据；候选已由探针收敛；§10.2 记 2026-09-15 参照口径更正与上限优先修订 |
