...
**P0 等 replay 配对诊断已完成；P1.1 数据合同已修复并通过；P2 pilot 未晋级；P2.1 输出/行动链诊断、P2.2 安全 bridge、P2.3 continuation 数据合同/targeted learning、P2.4 retention、P2.5 novel-composition probe、P2.6 novel K2 learning、P2.7 independent holdout generalization、P3.0 checkpoint/interrupt-resume contract、P3.1 single-cell 状态整合预检、P3.2 K→G owner-transfer 预检、P3.3 G candidate data-signal canary、P3.3 G-only learning Gate、P3.4 行为信号 Gate、P3.5 reobserve-aware G-only learning Gate 和 P3.6 独立行为 holdout/保持 Gate 已完成。** P3.6 没有新增训练：在 2 个全新 project、4 条全新 path 上，trained-G utility 为 `4.0`、zero-step 为 `2.65`，行为目标命中为 `4/4` 对 `1/4`；3 条 reobserve 均安全投影为可往返、不可执行的 `workspace.list`，P2.7 Workbench 保持 `4/4`，五类旧类、安全出口、K digest 和独立恢复均通过。`can_promote=false` 继续保持；唯一下一步改为 P4 容量压力与继承式结构成长 Gate，不能把 P3.6 的局部泛化误写成通用智能，也不能追加同质 G epoch。CUDA、客户端外围和 promotion 继续冻结。

阶段已按“研究审计完成、模型能力尚未验收”收束；本轮完成了 P2 小预算训练 pilot、P2.1–P2.7、P3.0–P3.6，但没有 promotion 成绩。**P3.6 Gate 已通过：新 project/path 上 trained-G 行为 utility 和 target hit 均超过 zero-step，五类旧类、安全出口、P2.7 Workbench `4/4`、K digest 与 checkpoint 独立恢复保持；下一步只执行 P4 容量压力与继承式结构成长 Gate。** P5 及客户端/CUDA 保留为后续路线，不并行开工；完整成果分类、停止条件及文档边界均在当前计划中。
