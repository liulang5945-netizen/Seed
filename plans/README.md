...
**P0 等 replay 配对诊断已完成；P1.1 数据合同已修复并通过；P2 pilot 未晋级；P2.1 输出/行动链诊断、P2.2 安全 bridge、P2.3 continuation 数据合同/targeted learning、P2.4 retention、P2.5 novel-composition probe、P2.6 novel K2 learning、P2.7 independent holdout generalization、P3.0 checkpoint/interrupt-resume contract、P3.1 single-cell 状态整合预检、P3.2 K→G owner-transfer 预检、P3.3 G candidate data-signal canary、P3.3 G-only learning Gate、P3.4 行为信号 Gate、P3.5 reobserve-aware G-only learning Gate、P3.6 独立行为 holdout/保持 Gate、P4.0 固定容量压力扫描、P4.1 公平容量合同预检、P4.2 隔离训练/公平容量归因 Gate、P4.3 保持约束增量学习 Gate 和 P4.4 保持身份/结构校准 Gate 已完成。** P3.6 在 2 个全新 project、4 条全新 path 上保持 trained-G utility `4.0`、zero-step `2.65`，行为目标命中 `4/4` 对 `1/4`；P4.0 在 5 个新案例、width `2/4/8/12` 上观察到 residual `0.32/0/0.54/0.59`，序列长度 `1/4/16` utility 均为 `0.6375`；P4.1 为 20 个集合建立 9 维候选集上下文、22 参数 fixed-large reference 与 context-lesion effective 13 参数 reference，所有参考 checkpoint 独立恢复且零 fit；P4.2 没有稳定容量收益且暴露旧行为保持问题；P4.3 两个训练臂在 fresh retention 都达到 utility `0.68`，但 new-only 与 rehearsal-mix 完全相同，`rehearsal_specific_gain=false`、出口为 `signal_insufficient`；P4.4 在同构但全新身份的 sibling retention 上复现 P4.2 历史退化的 5 个 arm/seed，P4.3 两臂/两 seed 也退化，出口为 `retention_failure_reproduced`。`growth_admitted=false`、`can_promote=false`；唯一下一步改为 P4.5 保持约束与更新规则对照 Gate，不能把保持退化直接写成“需要扩张神经元”。CUDA、客户端外围和 promotion 继续冻结。

阶段已按“研究审计完成、模型能力尚未验收”收束；本轮完成了 P2 小预算训练 pilot、P2.1–P2.7、P3.0–P3.6、P4.0 压力扫描、P4.1 合同预检、P4.2 隔离训练、P4.3 保持约束增量学习和 P4.4 保持身份与结构校准，但没有 promotion 成绩。**P4.0 只证明固定 G 在候选宽度变化下存在选择压力，P4.1 只证明公平输入/恢复边界已准备，P4.2 暴露容量归因与旧行为保持问题，P4.3 显示 fresh retention 通过但 rehearsal 没有可归因增益，P4.4 证明保持退化可跨同构新身份复现；下一步只执行 P4.5 保持约束与更新规则对照 Gate。** P5 及客户端/CUDA 保留为后续路线，不并行开工；完整成果分类、停止条件及文档边界均在当前计划中。
