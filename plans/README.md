...
> 2026-09-11 依据 main 044b04b6 的源码、checkpoint 与报告复审。唯一执行顺序：[当前计划](active/roadmap/03_CURRENT_EXECUTION.md)。
...
**P0 等 replay 配对诊断已完成；P1.1 数据合同已修复并通过；P2 pilot 未晋级；P2.1 输出/行动链诊断、P2.2 安全 bridge、P2.3 continuation 数据合同/targeted learning、P2.4 retention、P2.5 novel-composition probe、P2.6 novel K2 learning、P2.7 independent holdout generalization、P3.0 checkpoint/interrupt-resume contract、P3.1 single-cell 状态整合预检、P3.2 K→G owner-transfer 预检、P3.3 G candidate data-signal canary、P3.3 G-only learning Gate 和 P3.4 行为信号 Gate 已完成。唯一下一步：P3.5 reobserve-aware G-only learning Gate。** P3.4 在冻结 K 且不调用 fit 的情况下，构造了 40 条 train、10 条 validation、每条 2–6 个候选的行为 artifact；40/50 条出现 K-only 行为分歧，30 条以 typed `reobserve` 为行为目标，信号 Gate 通过。10 条 B/D 场景是静态候选语义与实时 Workbench 观测不一致造成的零 utility margin 平局，只能作为诊断，不能进入 fit。P3.4 仍 `can_promote=false`；下一步只允许在非零 margin 样本上学习，并把 `reobserve` 投影为安全的 `workspace.list` 型 typed abstention，不把它当作已执行成功。结构增长、promotion、CUDA 和外围路线继续冻结。

阶段已按 601413cd 收束为“研究审计完成、模型能力尚未验收”；本轮完成了 P2 小预算训练 pilot、P2.1–P2.7、P3.0–P3.3，以及 P3.4 行为信号 canary，但没有 promotion 成绩。**P3.4 Gate 已通过工程与数据条件：K1/K2 冻结、40/10 split、2–6 候选、40 条 K-only 分歧、30 条 reobserve 目标、checkpoint/独立恢复和运行时标签隔离均通过；10 条零边际平局已被标记为不可训练样本。下一步只执行 P3.5 reobserve-aware G-only learning Gate。** P4–P5 保留为后续路线，不并行开工；完整成果分类、停止条件及文档边界均在当前计划中。
