...
| [**P3b 双臂新颖度配对修订（增补件）**](reference/M5_P3B_NOVELTY_MATCHED_ARMS_AMENDMENT_20260915.md) / [实验臂 manifest](manifests/p3b_dialogue_fresh_manifest.json) / [对照臂 manifest](manifests/p3b_all_fresh_manifest.json) | 冻结预注册 §2.2 的混淆表**漏了第三源：数据新颖度**。实测起点 `seed_beta.pt` 的上一环指纹是**另一个**语料 ⇒ 状态已在原始流上消耗前 **11,199,800** 符号，按原文"整条原始流"起跑的两臂各有 **23.69% / 22.27%** 预算在重放已见数据。处置：两臂改由 `build_p3b_arm_corpus.py` **同一 skip、同一源窗口起点**产出，只差行规则（对话密度 **1.000 对 0.2356**，共享前缀 61,249 = 预算 0.13%）；skip 由检查点血缘推导、**无法定界即拒绝产出**；两臂语料都与 manifest 强绑。**J1–J5 与 §4 一字未改**，冻结件只加前进指针；本文写于第一个检查点之前（当时 `stages_recorded=0`）⇒ 改动不可能由结果驱动。§4 预先声明结果读法（0.15 分辨率、进度日志不作效应、`holdout_surprise` 只是格式熟悉度、止损不对称只读共同 tick、**J4 的 A/H 分支判不了**——CAP-0 runner 把 A/F/H 整维记 `not_executed`（07 §5：不记 0 也不记通过），结项只能记 `untested`） |
| [**P3b 结项报告结构与允许表述预案**](reference/M5_P3B_RESULT_REPORTING_PLAN_20260915.md) | **写于 `stages_recorded = 0` 时**（本地 01:38），不含任何结果。把结项最容易犯的错——"看着数决定怎么写"——提前堵死：**表 A** 逐 tick 双臂分数 + 主效应判定列（必须同时给 `main_effect_verdict()` 的标签，不许只写差值）、**表 B** J1–J5 逐条 × 逐臂（J4 的 A/H 分支永久写 `untested`）、**表 C** 停止条件轨迹、**表 D** 八项"缺一项即不完整"的披露清单；数值口径唯一（`tally.machine_normalised`，`null` 记 `untested` 不记 0 不删行）；**10 条禁止表述**（"未检测到效应"≠"无关"、单边提升≠分布、on-stream 与 holdout 差≠效应、能力断言须 B 人工复核已填、"先止损"≠"分布更差"、**禁用仪器里的 `best_stage`（它只按 D 一个维度取最大，是挑数钩子）**、禁止任何"CI 已绿"）；结项最小动作清单 6 步（J2 必须写成"**通过的阶段数 / 共同阶段数**"，不得只给末阶段单次结论）。预案自身按冻结件纪律处理：缺结构另立文档，不回改 |
| [**CAP-0 加载策略决策简报（未拍板、未改码）**](reference/CAP0_LEGACY_LOADER_DECISION_BRIEF_20260915.md) | 训练产物 `seed_beta.pt`（v8）**装不上产品入口**（`taiji/model.py:2732` `enabled identity organ checkpoint payload is missing`），而 v8/v9 是 `LEGACY_CHECKPOINT_FORMATS` **声明支持**的格式、同函数**其余 9 处**legacy 载荷都有 `is None and not is_legacy_checkpoint` 宽容分支与迁移注释——**只有器官这一支漏了** ⇒ 定性为正确性缺陷而非有意严格。选项上限排序 **(a) 补迁移 > (c) 明示降级 > (b) 重导出**（b 已被第二道 config 相等守卫证否）。**已实测拆分**：放宽后能装载、tick 真实读回，但输出与 `tick=2` 逐字相同 ⇒ 装载缺陷 ≠ 能力缺口。**现在不能落地**：改 `taiji/model.py` 会中途换掉在跑双臂的评测语义，且 P3a 基线的 `chain` 字段被 `test_cap0_baseline_contract.py:325` 钉住 ⇒ 落地时机 = campaign 之后。**关键警告**：钉这一支的测试全部**读已提交报告**、从不重跑，且诊断文字里的行号是**硬编码字面串**（DEBT-I5）⇒ 若只改代码不再生 `legacy_load` / `inventory` 两份报告与该行号断言，就会得到"代码已放宽、测试仍绿"的最坏假绿状态；5 步落地清单与 6 条出口见 §5 |
| [**P3b 阶段核验记录（滚动）**](reference/M5_P3B_STAGE_VERIFICATION_LOG_20260915.md) | 结项表 D 要"逐阶段"核三件事：快照 `metadata.tick` **等于**文件名 tick（标签即状态）、两臂 `corpus_fingerprint` **不同**（真的吃了不同语料）、阶段报告评测面四字段与 C/D/E 题序**与 P3a 全等**。最多 94 次，凭回忆拼不出来 ⇒ **发生当时记一行、只增不改**，记错就加更正行不覆盖。阶段 1（tick 17,000,000）已过：两臂 tick 相符、指纹分别为 `p3b_dialogue_fresh` / `p3b_all_fresh`、字节数 88,303,741 对 88,314,093、`tick_gap=0`；同阶段一并记下"哪些差**不是**效应"（on-stream 两臂不同数据、holdout 差 +0.0013 远低 0.15）与 196.6/199.3 符号/s 的同机负载事实 |
| [**P3b 若要第二次跑，怎样才判得动（决策简报，未冻结）**](reference/M5_P3B_SECOND_CAMPAIGN_DECISION_BRIEF_20260916.md) | 起草于**阶段 2 落地之前**（看过新数据再定读法就是事后拟合）。核算显示本次 campaign **结构上判不动**：单臂可停 ⇒ 只剩 1 个共同 tick，而预声明读法要"连续 2 个共同 tick 同方向"；且每维 20 题 ⇒ 量子 0.05，C 维基线 **0/14** 说明某些维度根本没有可上升的东西。给三个按强度排序的修法：**A′（推荐先做）**冻结一份足够大的**未见字节集**、每阶段做**成对 `mean_surprise`** 比较（连续分辨率、不生成故便宜，不耗训练机时）、**B** 固定 N 个共同检查点且**只按成对停止**（单臂退化只登记）、**C** 只把 `material` 抬到与分辨率同源（不推荐单用）。**A（扩题面到每维 100 题）的前提已被实测否证**：机检题按期望答案长度分桶后，E 维 **19/20 题**本来就只期望 1–2 字符 token 却仍只得 3 题 ⇒ 零分是**水位问题不是题目问题**，扩题只会把 0 多量几遍（§2b 负结果）。§6 列出新预注册必须预先声明的 6 条（共同 tick 定义、主效应唯一形式、停止三类、分辨率随题量推导、头寸先验、不做显著性检验）。**不改任何冻结件**；B 需用户新的机时授权 |
| [**P3b 结果（阶段 1 定稿 + 实验臂待停）**](reference/M5_P3B_RESULT_20260916.md) | **文档自身标注"不是结项"**：对照臂结论已固定，实验臂阶段 2 行全部记 `pending`。表 A 给共同 tick 17,000,000 的双臂分与判定列（照抄 `summarize_p3b.py` 标签：C/D/E 均 `not_detected_at_this_resolution`，`matched_ticks=1`）；表 A′ 逐题定位说明**+0.05 全部来自 E20 一题的存活差异**（两臂共同失去 E07 ⇒ 对差分贡献 0），并含同快照二次打分全等 ⇒ −0.10 是状态变化不是抖动；表 B 把 J2 写成**通过阶段数/共同阶段数 = 0/1**、J4 的 A/H 分支永久 `untested`；表 C 是停止轨迹（对照 `material` 已停 / 实验 `regression_kind=null` 续跑）并预告 `persistent` 的结局；表 D 八项披露（评测面四字段与题序全等、快照 tick 标签相符、两臂指纹与字节数、受保护检查点 sha256 与 DEBT-I7 根因、作废首跑、**远端 CI 未查询故全文禁"CI 已绿"**、机时 2.22%/3.91% 不做速率承诺）。§3 逐条自查十条禁令、**未使用 `best_stage`**；§6 记录 A′ 仪器已落地但**结果未产出故不含数字** |
