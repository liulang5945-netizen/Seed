...
- 主线已从 provider watchdog/interaction-group 深挖重排为 Workbench Closure W0–W7；第一个真实纵切片必须完成 `Taiji ActionIntent → ToolCall → Seed WorkbenchEnvironment → Outcome → UI/audit`，随后才允许推进 IDE 语言选择、写入/终端、MCP、自主循环和产品残留迁移。
