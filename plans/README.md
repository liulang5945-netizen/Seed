...
- 2026-08-29 W1 首批语言闭环已落地：`ProgrammingLanguageRegistry` 以内容寻址规则统一扩展名、shebang、内容、manifest、邻近文件、可选 LSP 和 toolchain 证据；`programming_language_id/editor_language_id`、confidence、provenance、registry revision 与用户覆盖进入 Workbench state。`editor.set_language` 已接入 Taiji-native action：高置信且与证据一致时自动允许，低置信、语言冲突或 `.h` 等歧义场景返回 `ask_user`，显式用户覆盖可撤销且按文件 digest 失效；checkpoint 会恢复但不会把旧覆盖套到新内容。Monaco 已提供“自动检测”撤销入口。后端 Workbench 合同 `8 passed`、Monaco 回归 `10 passed`，前端 lint `0 errors`、构建通过。
