...
- 主线已从 provider watchdog/interaction-group 深挖重排为 Workbench Closure W0–W7；W0 的 native 合同、只读环境、core router、runtime capability projection、审计链和前端 projection 已开始落地，但 checkpoint continuation 与 packaged-client canary 尚未通过，因此仍不能宣称第一个真实纵切片完成；随后才允许推进 IDE 语言选择、写入/终端、MCP、自主循环和产品残留迁移。
- 2026-08-29 W0 首批纵切片已验证：`ActionIntent → ToolCall → WorkbenchEnvironment → Outcome` 能在真实仓库文件上执行，过期 snapshot、路径越界和错误值域均 fail-closed；原有 structured-tool 回归、OpenAPI、编译和 lint 通过。当前遗留边界已收敛为 checkpoint continuation 与 packaged-client/real-workbench canary。
- 2026-08-29 W0 前端投影已接线并回归：`WorkspaceView` 使用 native capability 懒加载目录，`MonacoEditor` 使用 native read 打开文件，`editor.open` outcome 可通过统一 audit projection 驱动 IDE；写入、终端、重命名仍明确停留在 Legacy 未接入边界。剩余 W0 证据收敛为 checkpoint continuation 与 packaged-client/real-workbench canary。
