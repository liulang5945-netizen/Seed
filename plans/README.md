...
**P0 等 replay 配对诊断已完成；P1.1 数据合同已修复并通过；P2 pilot 未晋级；P2.1 输出/行动链诊断、P2.2 安全 bridge、P2.3 continuation 数据合同/targeted learning、P2.4 retention、P2.5 novel-composition probe、P2.6 novel K2 learning、P2.7 independent holdout generalization、P3.0 checkpoint/interrupt-resume contract、P3.1 single-cell 状态整合预检、P3.2 K→G owner-transfer 预检、P3.3 G candidate data-signal canary、P3.3 G-only learning Gate、P3.4 行为信号 Gate、P3.5 reobserve-aware G-only learning Gate、P3.6 独立行为 holdout/保持 Gate 和 P4.0 固定容量压力扫描已完成。** P3.6 在 2 个全新 project、4 条全新 path 上保持 trained-G utility `4.0`、zero-step `2.65`，行为目标命中 `4/4` 对 `1/4`；P4.0 在 5 个新案例、width `2/4/8/12` 上观察到 residual `0.32/0/0.54/0.59`，序列长度 `1/4/16` utility 均为 `0.6375`，K digest、G lineage 和独立恢复均通过，但没有批准结构成长。`can_promote=false` 继续保持；唯一下一步改为 P4.1 压力归因与公平容量对照 Gate，先区分候选集上下文合同缺陷和固定容量瓶颈，不能把压力扫描直接写成“需要扩张神经元”。CUDA、客户端外围和 promotion 继续冻结。

阶段已按“研究审计完成、模型能力尚未验收”收束；本轮完成了 P2 小预算训练 pilot、P2.1–P2.7、P3.0–P3.6 与 P4.0 压力扫描，但没有 promotion 成绩。**P4.0 Gate 只证明固定 G 在候选宽度变化下存在选择压力，未证明拓扑扩张必要；下一步只执行 P4.1 matched fixed-large/lesion 与上下文合同归因。** P5 及客户端/CUDA 保留为后续路线，不并行开工；完整成果分类、停止条件及文档边界均在当前计划中。
