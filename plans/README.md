...
2026-08-24 审计确认：GitHub `main` 远程读写已经恢复；Taiji 核心没有 Transformer/tokenizer/autograd 依赖。容量硬编码第三阶段已完成：`CapacityPolicy` + 参数预算现在同时规划区域/突触结构与 episodic memory 的时间、episode 编码维度，训练画像也会同步放大，旧 policy JSON 保持兼容。Transformer 清理边界第十阶段已完成：桌面启动依赖清单迁到 `seed_platform.dependencies`，核心清单不再自动安装 Transformer/RAG/Agent 依赖，只有显式 `SEED_ENABLE_LEGACY=1` 才启用 Legacy 清单；平台 API、旧 Cortex CLI、可选 Agent 路由和聊天请求 fallback 均已门控。当前唯一下一步是收口 `api/training/recommend.py` 的请求时 Legacy 数据集检查；禁止直接删除目录造成产品壳断裂。
