...
归档日期：2026-09-17；来源：ad812961 时的 plans/active/roadmap/03_CURRENT_EXECUTION.md。

本文完整保留旧正文，仅修正迁移后的相对链接。所有“当前/下一步/授权”均为历史表述，不再发出执行指令。数值与归因更正以[结果总览](../../reference/PROJECT_RESULTS_AND_CONCLUSIONS_20260917.md)为准；当前唯一顺序见[03执行计划](../../active/roadmap/03_CURRENT_EXECUTION.md)。特别是“监督结构性缺席所以失败必然”“语言完全缺失”和 v1 win 0/3 等旧表述不作为当前结论。
...
2026-09-17用户确认新增[统一认知闭环验收原则](../../active/roadmap/02_GATES_AND_CI.md#21-统一认知闭环验收原则2026-09-17用户确认)：优先证明同一模型在真实任务中的记忆、世界预测与规划共同改善行为；局部机制、模块齐全和界面可用不能代替联合能力证据。语言验收保留，但不是整个认知架构的唯一出口。该决定仅更新验收标准，不启动整合支线、不解除暂停、不改变下列唯一收尾动作；后续路线若调整须另行明确范围与预算，不能自动增加并行缺口。
...
> 本文是唯一执行顺序来源；[01](../../active/roadmap/01_SCOPE_AND_PHASES.md)管总阶段，[02](../../active/roadmap/02_GATES_AND_CI.md)管晋级，[07](../../active/roadmap/07_MINI_MODEL_DELIVERY.md)管整模型验收。
...
>
> 本文件仍是**唯一执行顺序来源**；[01](../../active/roadmap/01_SCOPE_AND_PHASES.md) 管总阶段，
> [02](../../active/roadmap/02_GATES_AND_CI.md) 管晋级，[07](../../active/roadmap/07_MINI_MODEL_DELIVERY.md) 管整模型验收。
...
[技术债](../../active/roadmap/05_TECH_DEBT_REGISTER.md)按最新日期解释：旧27项SystemExit是环境守卫伪影，不是未定位架构缺陷；3.10数值敏感、仪器语义、隔离问题独立跟踪。本次仅运行不写checkpoint的文档/身份检查，不全量冒险重测。
