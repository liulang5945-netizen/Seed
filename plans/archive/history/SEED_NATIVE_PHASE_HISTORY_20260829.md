# 归档：Seed / Taiji Native v1 阶段推进记录（截至 2026-08-29）

> 本文件保留 P0–P7 的完整滚动实现历史，不提供当前执行顺序。当前范围和阶段门槛见 [01_SCOPE_AND_PHASES.md](../../active/roadmap/01_SCOPE_AND_PHASES.md)，即时动作只看 [03_CURRENT_EXECUTION.md](../../active/roadmap/03_CURRENT_EXECUTION.md)。下文“当前/下一步”均是历史语境。

> 本文由原总路线图按职责拆分而来。原始行号：1–495；本文件属于当前计划资料或历史证据，具体身份以本文件所在目录为准。
> 这是当前阶段顺序和阶段退出门槛的详细定义；它不承担逐次执行日志。
...
长期目的见 [TAIJI_CORE_REQUIREMENTS.md](../../active/TAIJI_CORE_REQUIREMENTS.md)，权威设计见 [TAIJI_NATIVE_ARCHITECTURE_V1.md](../../active/TAIJI_NATIVE_ARCHITECTURE_V1.md)。旧 raw-byte substrate 路线完整保存在 [旧路线归档](SEED_DEVELOPMENT_ROADMAP_PRE_NATIVE_V1_20260825.md)，其中 R1–R5 不再是当前研究执行顺序。
