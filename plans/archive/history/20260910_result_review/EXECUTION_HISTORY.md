...
> 仅作历史证据，执行顺序由 [当前计划](../../../active/roadmap/03_CURRENT_EXECUTION.md) 替代。
...
>
> 当前主线：**M4.V2 继承式成长重构**。M4.R0～R12 已作为 F1 fixed-capacity 实验证据归档，但其对“容量、分布和全部学习规则”的过度外推已撤销。当前架构依据为 [Taiji 继承式成长架构 v2](../../../active/architecture/TAIJI_CONTINUAL_DEVELOPMENT_V2.md)，历史证据边界见 [M4 固定容量证据链复盘](../../../reference/M4_FIXED_CAPACITY_EVIDENCE_REVIEW_2026_09_09.md)。
>
...
parity 口径修正与修复路线已冻结：[M4V2_B3_K_C_PARITY_CALIBER_REVISION_20260910.md](../../../reference/M4V2_B3_K_C_PARITY_CALIBER_REVISION_20260910.md)。机器证据钉死口径：parent K1/K2 = 2080/5040 步，fixed-large replica 与 widened 每通道都只新增 3+3=6 步（3 train experiences × 1 epoch），**两臂真实新增均为 12 步/cell 且恰好相等**——旧口径的 14,252 中 14,240 是继承步数，掩盖了真实预算语义。新口径冻结：inherited/new 分离、双臂只用 `new_update_steps` 比较、旧 JSON 保留 + 只读重述审计、硬门替换。

**归因修正（用户质疑触发，§6 已记录）**：原「parent 收敛→梯度≈0→顺序通道无信息」被证伪（delta norms 0.196/0.030 非零）。真机制经四步实验钉死：course1 的 A+A 两个 experience 构成 **delta 等价类**（类型化绑定下掩码可见学习信号逐位相同，input_digest 差异只在掩外特征），model31/course1 的 anchored 排列 `[1,0,2]` 恰好只交换等价成员——「非恒等排列≠非等效排列」，防恒等兜底有等价类盲区。衍生事实：**三个 model seed 的 parent K worker 权重完全相同**（state diff=0.0，digest 差异全在 metadata）——9-cell 矩阵的 model 维度对 K workers 无独立性，独立样本实际只有 3 个 course，触碰「重复 worker 不算独立样本」红线。修复路线 n_new=150 方向保留但理由更换为「打破等价类 + 排列空间不可碰撞」，并新增硬前提：manifest v2 声明目标 multiset 无重复类型；统计单元改为 course（n=3）或补建互异 parent workers（另行决策）。
...
**parity formal 预注册已冻结：[M4V2_B3_K_C_PARITY_FORMAL_PREREGISTRATION_20260910.md](../../../reference/M4V2_B3_K_C_PARITY_FORMAL_PREREGISTRATION_20260910.md)。** 预注册准备期的 digest 固定发现并修正第二个独立性问题（首轮 v2 的类序列不随 course_seed 变化 → 9 cell 权重逐位相同）：类块内排列随课程变化（ABC/ACB/BAC），新增**课程独立性硬门**（state_dict digest 排除 metadata 后两两互异，双臂同查），重跑 9/9 通过且 ensemble digest 恰 3 份（每课程一份）——独立样本结构机器成立。冻结内容：评分输入 digest（sealed artifact sha256 `8989…`、v2 artifacts 对 build report 校验）；评分语义（6 分量 MSE + combined，与 v1 同口径；candidate readout = 权重相加，与 logit 域通道和数学等价——诚实声明；fixed-large = 概率平均）；判据 G1 质量门（validation delta < 0 于 ≥2/3 课程）、G2 灾难界（`epsilon_cat = max(0.01, 3×std(validation deltas))` 于 sealed 读取前派生冻结，每课程 ≤ +epsilon_cat）、G3 主判据（sealed 上 candidate < fixed-large 于 ≥2/3 课程且均值更优）；统计单元 course n=3，9 cell 为描述性；停止线含「sealed 读取后禁止任何判据调整」。`can_promote=false` 固定。
...
用户拍板 (a)。**C 阶段 formal v2 预注册已冻结并执行完毕：[M4V2_C_STAGE_FORMAL_V2_PREREGISTRATION_20260910.md](../../../reference/M4V2_C_STAGE_FORMAL_V2_PREREGISTRATION_20260910.md)。** 唯一变更 = G1 操作化修正（学习证据从 3-episode validation 代理改为 sealed 主测量；G1「臂学到东西了吗」与 G3「FS 比 C 好吗」边界显式分离）；其余判据、四臂与 v4 artifacts 零变更；结果映射修正后覆盖全分支（含 v1 的「G1 败 + G3/G4 过」缺口）。执行：sealed v5 materialize（task_seed=151，D/R/A 覆盖，对 v1~v4 sealed 与 train/validation 不交校验过）→ pre-sealed epsilon 先冻结（epsilon_cat=0.01、epsilon_ni=0.003846）→ 四臂 sealed 评分（`sealed_read_count=1`）。**结果四门全过**：G1（v2 sealed 口径）3/3（C −0.12879/−0.12875/−0.12876、FS −0.13070/−0.13043/−0.13078）；G2 远离灾难界；G3 弱类 3/3（FS −0.1939~−0.1941 vs C −0.1913，每课程好 ~0.0022）；G4 整体非劣 3/3 且 FS 实际更优 ~0.0018（亦优于 XL −0.12868）。`formal_passed=true`；按 §5 冻结映射：**FS（fast/slow+replay）收束为 K 相位默认学习机制候选**；`can_promote=false` 不变。C 阶段学习机制比较正式闭合，v1 的映射缺口被操作化修正消解。
...
- [Taiji 核心需求](../../../active/TAIJI_CORE_REQUIREMENTS.md)：CR-1～CR-10，长期不变目标。
- [Taiji Native Architecture v1](../../../active/TAIJI_NATIVE_ARCHITECTURE_V1.md)：完整认知架构与 A0～A9。
- [Taiji 继承式成长架构 v2](../../../active/architecture/TAIJI_CONTINUAL_DEVELOPMENT_V2.md)：M4 v2 的状态、主路径、学习与结构准入设计。
