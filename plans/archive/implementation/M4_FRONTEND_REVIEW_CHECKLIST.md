...
## 8. 2026-08-24 Taiji 品牌图形与原生内容对齐

- [x] 导入用户提供的水墨神经网络太极图，统一用于客户端内 logo、窗口、任务栏、托盘及托盘通知。
- [x] logo 在客户端内整体缓慢流转，并在减少动态效果偏好下自动静止。
- [x] 桌面客户端使用圆润外框；窗口内容边界与圆角 mask 保持一致。
- [x] 首页对话示例、设置、知识库、训练说明和 Taiji 状态页与当前 Taiji 原生字节输入、状态推进、局部可塑性和动作闭环一致，移除旧 Transformer/ResonanceField 占位表述。
