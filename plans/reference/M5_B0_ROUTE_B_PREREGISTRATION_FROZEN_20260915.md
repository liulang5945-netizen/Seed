...
不等于 `safety_stops`、不可单独推出「成员无能力」。消费面 **17 个文件 / 10 处判断点（4 live gate + 6 测试断言）**，
...
> **进度注（2026-09-15 追加，上表各行原文保留不删）**：第 **2** 项已闭合（[N2 冻结版](M5_B0_N2_STOP_REASON_PREREGISTRATION_FROZEN_20260915.md)
> I1–I8 + 双向测试，清单现值 17 文件 / 10 处判断点）；第 **3** 项已闭合（HANDOFF-M4 写进 gate，
> 五条出口判定见[落地结果](M5_B0_M4_LANDING_RESULT_20260915.md)——**注意**：闭合的是"gate 里的组合规则"，
> 产品机制仍无实现，该项的这半个风险不在本条范围内）；第 **4** 项已闭合（成员序/`STEP_CAP`/模板已写入本文「任务规格」）；
> 第 **7** 项已闭合（pre-WP-3 复采 1408 / 0 失败 / 6 跳过，登记册 2026-09-15 节）。
> 第 **1** 项**仍未闭合**：落地后指纹仍 3/11，独立结构因素仍为 1；第 **5**、**6** 项保持开放。
