...
**legacy_load 一侧的顺序依赖**：其封存样本里 `default_control_current_guard.tick = 36`，而盘上默认基座
已被改写成 tick=2（见债册 I7/I9）⇒ 拿它当参照会因**基座**而非仪器漂移而红。
须等写侧隔离生效后重新采样一份，再按同一形状落地（`--report` 已补好，采样即可）。
