...
> 另：因 **D1 确认取最强参照** `all_singleton_oracle`（`required 1.65`）而本文实测 `+2.000` 正好等于 `ceiling_gain 2.0`（**零余量**），
> §7.2 的 **N1b/N1c 扩面已从"可选"升为"冻结版落盘前必过门"**，取上限档**每格 6 context / 7 种子**。
> 取值见[推进计划修订 §2「WP-1 取值已确认」](M5_B0_CLOSEOUT_AND_PLAN_REVISION_20260914.md)。
> **执行已授权（2026-09-15 解除限制）**：路线 B 冻结版预注册已落盘，WP-1.5 四条出口全过 ⇒ **WP-2 语义冻结与 WP-3 落地推进中**；WP-4（训练）与 WP-6（改 binder）仍按各自前置排队。

## 10. WP-1.5 扩面重跑结果（2026-09-15，上限档）

> 规模：**每格 6 context**（原 2）、**7 个种子偏移** `0 101 202 303 404 505 606`（原 3）⇒ 66 context / 7 种子。
> 仪器：同一支 `probe_taiji_b0_structure_space.py`，经 `--contexts-per-cell` / `--seed-offsets` 两个旗标驱动；
> **默认值未改**，故无旗标运行仍逐字节复现本文 §5 的 20260913 归档报告。
> 报告：[扩面报告](../../reports/taiji_b0_structure_space_probe_wide_20260915.json)（新文件，未覆盖旧报告）。

### 10.1 实测

| 格 | 联合必需 | 冻结规则 | M4 | delta | 交错 |
|---|---|---|---|---|---|
...
| `patch__observation` / `__override` / `__mismatch` | 否 | `−2.000` | `−2.000` | `+0.000` | `0 → 0` |
| `none__observation` / `__override` / `__mismatch`、`create__none`、`patch__none` | 否 | `+0.000` | `+0.000` | `+0.000` | `0 → 0` |

- **11 格全部 `measurable`，无一格被有效性门剔除**（`not_measurable: none`）⇒ 扩面没有靠放宽门换来样本。
- `positive every seed` = 三个 create 格 ⇒ **7 个偏移逐个为正**，无一种子依赖。
- `unexplained changes: none`、`m4 regresses cells: none`、`combination-only prediction contradictions: none`。
- `patch` 行两规则同为 `−2.000` ⇒ 仍然**没有把不可达变成可达**。
- T1 / T3 复测仍为 `inexpressible_under_frozen_binder`（与 §2 一致，扩面不改可表达性）。

### 10.2 四条出口判据

| # | 出口 | 结果 |
|---|---|---|
| 1 | 三格 `create × 语言` 在**每个**偏移仍 `> 0` | **过**（7/7 偏移 × 3 格） |
| 2 | `unexplained_changes` 仍为空 | **过**（`none`） |
| 3 | 两次重跑 JSON 字节相同 | **过**：两次 sha256 均为 `b09b3e62a0679c27bee1f482c2ef7bf69a9908386c3acb349bb9dace2d487b64` |
| 4 | 若某偏移转负则如实发布为界限收缩 | **未触发**（无偏移转负，无需发布收缩） |

- **实测机时**：两轮连跑 01:05:09 → 01:29:11 = **24 分钟**（单轮约 **12 分钟**），低于计划里 20–30 分钟的外推。

### 10.3 这次扩面**闭合了什么、没闭合什么**

- **闭合 L4（样本规模），并且顺带把"k 维度"从零余量变成有余量**：原每格 2 context / 3 种子 ⇒ 现 6 context / 7 种子。
  逐格核对扩面报告的 `reference_requirements_audited`（三个 create 格一致）：

  | 参照 | `required` 组合必需 context | `available` | `feasible` | `ceiling_gain` | `max_clearable_reference` |
  |---|---|---|---|---|---|
  | `all_singleton_oracle`（1.5，D1 主判据） | **5**（扩面前 2） | **6**（扩面前 2） | `true` | `2.0` | `1.85` |
  | `best_observed_fixed_pair`（1.0） | 4 | 6 | `true` | `2.0` | `1.85` |
  | `best_fixed_singleton`（0.5） | 2 | 6 | `true` | `2.0` | `1.85` |

  ⇒ **逃生通道在 k 维度上首次有余量**：需要 5 个、可用 6 个（扩面前是 2/2，一个都不能少）。
  9 个「格 × 参照」组合全部 `feasible: true`（3 格 × 3 参照）。
- **但"增益维度"仍然零余量，且这是判据的形状而非采样偶然**：`ceiling_gain` 与 context 数**无关**，扩面前后都是 `2.0`，
  而实测增益恰好 `+2.000` ⇒ 贴着上限。之所以不是"样本恰好贴边"：6 个 context **全部**产生真实交错（`0 → 6`，无一退化为回退链），
  7 个种子偏移下增益恒为 `+2.000`，`unexplained` / `regress` 皆空。
  **含义**：D1 取最强参照时，达标不是"勉强过线"，而是"把可清的组合必需 context 全清了"；
  但也因此**没有任何缓冲**——冻结版必须把这一点写成显式风险条款（WP-1 取值确认表 D1 行已要求）。
- **没有闭合 L1（结构独立性）**：`3 distinct fingerprints over 11 cells` 与扩面前**完全相同**。
