...
## §10 v6 追记（2026-09-20，目标调度判停）

[v6 合同 §G1](M5_R2_CONTENT_BINDING_CONTRACT_V6_FROZEN_20260920.md)（λ_pair 8000 起线性撤除至 12000，256/256×16000 更新×B/A）。**判定：无合格候选（copy 峰值 0.64 < 0.90），v6 判停。** 但调度假设部分成立：

- **学习稳定性显著改善**：final loss 0.154/0.182（v5 同期 1.395/1.314）——撤压后 CE/copy 恢复训练；
- **A 臂 relation margin 保持饱和（0.011@16000）**且 copy 恢复至 0.540——调度避免了 v5 的全面崩塌；
- **但 B 臂 obj margin 撤压后回升（0.771）**、copy 仍未达线；anneal 边界处 copy 瞬时不稳（B@12000: 0.000）。

**v1–v6 完整弧线终点**：绑定对比全部可饱和（慢）、margin-copy 存在可调度的张力（调度改善稳定性与保持但不足以达线）、自由生成泛化恒定脱耦（24 次运行、~128,000 更新、copy 天花板 0.65）。**继续同架构放大已无依据**；剩余路径＝数据配方重构或架构级改动（均需新合同）。证据：[v6 聚合](../../reports/r2_content_binding_v6/calibration_selection_20260920.json)。
