...
## §6 修订（2026-09-19，L2 测量缺陷修正——意图不变、形态可检验化；撤回受影响判定）

第四轮运行（lang_confirm 族）：L1/L3/L4 过且消融有真实区分度（full=1.0 vs simple_strategy=0.0、disable_memory=0.0），但 **L2 失败**。归因：L2 的失败注入为**单次**（首选成员仅首次尝试失败，rev 0 立即重试成功，不产生浪费）——未实例化 HANDOFF-M4 假设的前提（成员持续阻塞）；且 executed_actions 指标无法表达失败尝试与重试 tick 的资源浪费。属仪器设计缺陷，非机制负结果：

1. **注入语义修正**：首个可绑定成员的执行**持续失败，直至另一成员成功执行**（与 P5.2b 原始语境的"成员持续阻塞"一致）；
2. **L2 度量修正**：`full 臂总步数（含失败尝试）≤ disable_selection 臂总步数`——同一意图（让位比盲重试省资源）的可检验形态（原 executed_actions 口径无法计失败成本，rev 0 全失败时反而为 0）；
3. L1/L3/L4 与全部阈值零改动；第四轮的 L2 判定撤回，重跑判定。
