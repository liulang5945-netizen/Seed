...
## 17 Logo 第二轮：先忠实复现，再换星象（应所有者反馈，2026-09-21）

所有者反馈：v1/v2 都不满意——**得先复现原本 social preview 的细节**，再用星象代表神经元；
用单个北斗太生硬，不如原版。

### 17.1 先解剖原图（不再目测，逐像素量）

- 原图 1280×640，**左右双栏**：左半是文字排版（SEED 眉题 / Taiji 大标题 / 红色下划线 /
  特性列表 / mono 页脚），右半才是标志。此前一直把右半当全部，是第一轮走偏的根源。
- 底色 **#FAFBF6**（占 87%），墨色近 #101216，灰环 #AAABA6。
- 盘心 (1023.5, 318)、R = 159.5（在 640 宽的右半里占 0.249 —— 盘其实不大，留白很多）。
- **双眼都在经典位 (0, ∓0.5R)，半径 ≈ 0.144R（≈1/6.9，比经典的 1/8 粗）**——这是它
  「现代感」的来源；黑鱼头在下、白鱼头在上。
- 右上有一圈**灰色偏移环**（+0.055R，细）。
- 两张 mesh 是**不规则网状**（不是星座）：黑鱼上 13 个白节点、白鱼上 10 个黑节点 +
  1 颗红点（(0.418,-0.674)，带一条红边）。节点由**连通域检测**自动提取（面积/填充率/
  长宽比过滤掉盘缘弧段），不再手抄坐标。

### 17.2 产物（`design/logo-candidates/`）

| 文件 | 内容 |
|---|---|
| `repro-faithful.png` | 忠实复现：粗眼太极 + 偏移环 + 两张 mesh（检测到的节点 + k 近邻连线）+ 红点 |
| `star-imagery.png` | **星象版**：同一太极，mesh 换成星图——星等差（1 亮 + 多暗）、连线更细更少（k=2）、加 26 颗背景碎星（确定性散布，颜色按原图该点明暗自动选） |
| `compare-original-repro-stars.png` | 三联：原图右半 / 复现版 / 星象版 |
...
参照物是 `frontend/dist/social-preview.png`：极简扁平黑太极 + 两半各铺一张节点连线图 + 单点红。

构思：**北斗七星 = 七个神经元节点，连线 = 突触**。斗四星落在黑鱼、柄三星扫向白鱼；
红点落在天枢（斗口第一星），呼应 social-preview 的单点红；太极双鱼眼各加一圈细描边，
让「眼」也参与「星」的叙事。

产物（`design/logo-candidates/`，全部 PIL 4x 超采样矢量绘制，**非 AI 生图**——北斗的星形与
相对位置必须精确，生图给不出可复现的几何）：

| 文件 | 用途 |
|---|---|
| `v1-logo-transparent.png` | 透明底通用 Logo |
| `v2-icon-squircle.png` | 奶油圆角砖（应用图标 / 快捷方式） |
| `all-candidates.png` | 两版总览 |

**待所有者选定后**，再由选中版生成 `icon.ico` 与托盘/窗口图标（当前的圆角 `icon.ico`
仍是旧水墨太极方案，未替换）。

### 16.1 顺带修掉的两处壳缺陷（所有者截图定位）

**现象**：双击快捷方式启动打包版，停在「Seed / 界面加载中，请稍候…」占位屏，
窗口右侧出现一条竖向长条。

1. **占位屏 = 我注入的 3 秒兜底**（`#app` 为空时替换 body），说明 **Vue 未挂载**。
