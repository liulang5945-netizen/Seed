...
## §7 标定判定（2026-09-19，用户批准预算后执行）

标定扫描（replay_weight ∈ {0, 0.25, 0.5, 1.0}，总耗时 623s）全部完成。**判定：replay_weight=0.25 为准入候选——四条预注册线全过**，产品 report 自身的 admitted 标志首次在真实语料上翻为 true：

| 指标 | 控制组 (0.0) | **选定 (0.25)** | 线 |
|---|---|---|---|
| procedural retention | 0.4189（逐位复现 P5.1g，仪器连续性） | **0.6053** | ≥0.5 ✓ |
| 独立切片准确率（新 16 记录，行 344–361，不相交、0 剪枝） | 0.92 | **0.92** | −0.367≥0.15 ✓ |
| semantic retention | 1.0→0.000369 | 1.0→0.000369 | 零退化 ✓ |
| lesion / checkpoint 往返 | 0.0 / 保留 | 0.0 / 保留 | ✓ |
