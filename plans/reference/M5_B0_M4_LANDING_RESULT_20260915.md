...
- **核查口径（避免误判）**：封条 sha256 针对的是**工作区文件字节**。本仓库 `core.autocrlf=true`，
  所以 `git show HEAD:reports/…` 给出的是 LF 规范化后的 blob，与工作区哈希**必然不同**
  （本轮同一份扩面报告：工作区 `b09b3e62…` = 封条值；blob `f527b78a…`）——这**不是篡改证据**。
  复核只能按本节的方式读工作区文件（`test_b0_rule_revision_seal_contract` 即如此）。
