...
| E3-3 | [Native evolution and client embodiment route](../active/roadmap/63_TAIJI_NATIVE_EVOLUTION_AND_EMBODIMENT_20260831.md) | 当前阶段：只接收真实 `WorldTransition` 状态—动作—结果证据，驱动现有局部 WorldDynamicsLearner；digest-only 经验不得伪造状态，holdout/retention 只测量，先不改结构、不接 provider |
