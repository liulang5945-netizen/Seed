...
> 事实快照：2026-08-31。本文件只描述当前代码事实、能力边界和证据入口，不决定执行顺序。当前动作见 [03_CURRENT_EXECUTION.md](../active/roadmap/03_CURRENT_EXECUTION.md)，后续阶段见 [62_POST_S51_PROJECT_CONVERGENCE_20260831.md](../active/roadmap/62_POST_S51_PROJECT_CONVERGENCE_20260831.md)。
...
- 普通 `/api/chat/stream` 的旧路径仍不能在没有预制结构化 intent 时自主完成 IDE 任务；新的 P2-8 入口已不要求调用方提供 `ActionIntent`，P2-9/P2-10/P2-11 已收回只读多步与 IDE 语言链的最终绑定，但自然语言受控写入仍待 P2-12。
...
- P2-7 已证明 bounded、content-addressed 的 TaskDecomposition 绑定当前 Goal，不能携带 capability/tool/intent 字段，Taiji 可按每个语义步骤做非执行 grounding，native checkpoint roundtrip 保持一致；P2-8 已证明 Taiji 会先建立当前感知，再从已验证 provider evidence 和 live affordance 形成 ActionIntent 并执行一条有界 Workbench 任务，低置信和执行字段 fail-closed，checkpoint/save-load 通过；P2-9/P2-10 已收回声明式单步、多步的最终 `parameter_bindings` ownership；P2-11 又证明 IDE 语言 ID 由当前文件/Workbench 证据派生，provider 不提交最终语言绑定，并经过 Outcome/checkpoint/recovery；P2-12 仍需收回自然语言写入参数 ownership。
...
- P3-3 已证明现有 provider watchdog、rotation、fallback、checkpoint/restart rebinding 的确定性 packaged-client 集成 seam；本机缺少真实 provider 模型目录，因此真实模型质量与加载链保持 `asset-unverified`。
...
| P3-3 | [Packaged provider lifecycle canary](../../reports/taiji_w7_p3_3_packaged_provider_lifecycle_20260831.json) | 确定性 rotation/watchdog/fallback/checkpoint/restart rebinding 集成 seam 通过；真实 provider 模型资产缺失，保持 `asset-unverified` |
