...
最近一次已记录的完整回归为 Python `550 passed, 6 skipped`、前端 `42 files / 233 passed`，覆盖率约 `45.13%`；native/API contract、Ruff、Black、核心 mypy 和生产构建均通过。后续改动不得只更新数字，必须把对应报告、Gate 和失败边界一起保留。
