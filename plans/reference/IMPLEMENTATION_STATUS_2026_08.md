...
- **W7-R1 S0 provider watchdog（2026-08-29）**：健康记录改为以 `artifact_id + artifact_digest` 内容寻址，防止同 ID 内容替换继承旧失败计数；健康/失败阈值/单次回退/cooldown/native fallback/checkpoint roundtrip 的 S0 评测通过，报告见 [taiji_w7_r1_provider_watchdog_20260829.json](../../reports/taiji_w7_r1_provider_watchdog_20260829.json)，定向 provider 回归 `20 passed`。当前只完成 S0，S1 checkpoint replay 与 S2 packaged-client provider 观测仍未完成。
