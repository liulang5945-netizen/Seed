...
- interaction-group、真实 Workbench、结构增长、自然语言执行链、provider 边界和客户端 host 已形成大量受限 Gate；这些结果继续作为机制与安全证据，但其训练样本和任务族过小，不能证明基础智能。当前不再沿 E6/E8 扩展外围，而是执行 M0 五项基础能力真实性基线，并在测量链可信后立即进入 M1 foundation 训练。
...
- 最近一次已记录的远端全绿基线早于当前 R5C-S18–S52 大批改动；当前代码的全量 CI 状态保持“未验证”，M0-0 将先显式核对已知 OpenAPI snapshot 漂移，再开始新增能力代码。
...
4. 当前执行顺序、优先级、日程和下一步永远只由 `03_CURRENT_EXECUTION.md` 决定。
