...
| E5-1 | [E5-1 client snapshot integration report](../../reports/taiji_w7_e5_1_client_snapshot_integration_20260901.json) / [client extension routes](../../api/routes_client_extensions.py) / [Vue client extension facade](../../frontend/src/composables/useClientExtensions.js) | 已完成实际接线：8/8 Gate 通过，API 与 Workbench capability snapshot 绑定，prepare/commit 两阶段发布、stale fail-closed、Vue 声明式 slot projection、App 注入和 Legacy/Taiji 执行隔离通过；旧 `/api/plugins`、marketplace/upload 尚未收敛 |
| E5-2 | [Native evolution and client embodiment route](../active/roadmap/63_TAIJI_NATIVE_EVOLUTION_AND_EMBODIMENT_20260831.md) | 当前阶段：把旧 `/api/plugins`、marketplace/upload、`routes_agent_mcp.py` 的 Legacy manager surface 迁移到 Seed-owned runtime 或明确 tombstone；完成后才进入真实第三方 MCP/plugin |
