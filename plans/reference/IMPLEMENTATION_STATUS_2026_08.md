...
- **W7-G0 合同冻结（2026-08-29）**：五份 R1–R5 manifest 已进入 `plans/manifests/`，由 `tests/test_w7_gate_manifests.py` 的 3 个结构测试看守；它们冻结后续实现的真实输入/输出、trace、资源预算、checkpoint、red proof、holdout/lesion、失败隔离、rollback 和越界边界。R1/R2/R3/R5 为 `contract_frozen`，R4 明确为 `hardware-blocked`；这不是任何后续能力已完成的声明。当前执行入口是 W7-R1 provider watchdog。
...
- **W7-R1 S0/S1 provider watchdog（2026-08-29）**：健康记录改为以 `artifact_id + artifact_digest` 内容寻址，防止同 ID 内容替换继承旧失败计数；S0 的健康/失败阈值/单次回退/cooldown/native fallback 通过，S1 又用真实 `TSKV8Adapter.native_checkpoint()` 验证 artifact、digest、registry、健康计数和阈值后的下一次探针 lineage 可恢复。报告见 [S0](../../reports/taiji_w7_r1_provider_watchdog_20260829.json) 与 [S1](../../reports/taiji_w7_r1_provider_watchdog_s1_20260829.json)，定向 provider 回归 `20 passed`。当前只完成 S0/S1，S2 packaged-client provider 观测仍未完成。
