...
R4 已完成 pressure、candidate、shadow materialization、candidate-only causal training 以及首个五臂短 S/G 技术 canary：`reports/taiji_m4v2_r4_shadow_canary_20260909.json`。五臂的 checkpoint/fresh restore/rollback/matched-capacity 技术 Gate 均通过，但 pressure-driven growth 没有稳定优于 R3 fixed-capacity，candidate lesion 在 S/G 方向不一致；因此该结果只证明“对照和因果审计管线可运行”，不证明结构成长有效，整体仍保持 `can_promote=false`。当前唯一下一步是增加逐 tick candidate activity、projection credit、candidate-vs-parent residual 和 lesion trace，并只修订 candidate credit/activation 边界后复跑同一五臂课程；在 growth efficacy、旧能力非劣、资源预算、fresh restore 与 rollback 全部通过前，不允许进入 R5 learned router。Skill/MCP、provider、客户端和 CUDA 继续遵守执行计划的解冻顺序。
