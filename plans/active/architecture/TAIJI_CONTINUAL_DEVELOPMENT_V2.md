...
R4 已完成第一子步：`AdaptiveResidualGrowthPressure` 与 `AdaptiveResidualGrowthTrigger` 已把 residual/conflict/utility pressure、parent digest、重复 evidence、预算和 checkpoint 状态固定为可审计合同；该合同仍是 evidence-only，不会自行改拓扑。当前唯一下一步是把 pressure 观测由 R3 bridge 的真实 observation tick 产生，再沿这一条 bridge 入口提议零影响候选；候选进入 shadow 后还必须与 frozen、R3 fixed-capacity、random growth、同等最终参数量 fixed-large 对照，并通过 growth lesion、旧能力非劣、资源预算、fresh restore 与 rollback，才允许进入 R5 learned router。Skill/MCP、provider、客户端和 CUDA 继续遵守执行计划的解冻顺序。
