...
R4 已完成 pressure、candidate、shadow materialization、candidate-only causal training 以及首个五臂短 S/G 技术 canary：`reports/taiji_m4v2_r4_shadow_canary_20260909.json`。随后加入逐 tick candidate activity、eligibility、projection credit、candidate-vs-parent residual、projection update 和 lesion trace；候选 projection 使用已有 eligibility trace 后，pressure candidate 的平均 candidate/parent residual ratio 从首轮约 `0.00380` 提升到约 `0.00981`，G lesion delta 为 `+0.00474`，但 S lesion delta 为 `-0.00027`，所以 credit 通路已被证明可工作，growth efficacy 仍未通过。进一步审计发现首轮五臂的 matched-capacity 训练边界并不一致，已校正为：五臂统一从 `pressure_parent` 进入，fixed-capacity 与 growth efficacy 使用同一 parent-bridge 学习边界，candidate-only 版本单独保留为冻结 parent 的技术 smoke。校正后技术 Gate 全通过；pressure growth 相对 fixed-capacity 在 G 改善约 `0.04233`、S 退化约 `0.00033`，相对 fixed-large 仍未占优，S lesion 仍为负。固定边界的 3 model-seed × 3 course-seed/order 矩阵证明 pressure growth 已相对 fixed-capacity 稳定改善（G `9/9`、S 均值改善约 `0.04915`），但相对 fixed-large 在 S 为 `0/9` 占优。单-anchor pressure birth 随后使相对 fixed-capacity 的 S/G 均 `9/9` 改善，相对 fixed-large 的 S `8/9` 占优、G `5/9` 占优，S lesion `9/9` 为正、G lesion `6/9` 为正；这证明出生初始化能修复 S，却会把 candidate 过度专门化到单一出生模式。当前唯一下一步是把单 anchor 收敛为固定 fan-in/预算内的 top-2 activity+eligibility birth mixture，保存 anchor identities、归一化权重和 parent digest；pressure-driven 与 candidate-only 使用 mixture，random-growth/fixed-large 保持随机，其他 parent、pressure、课程、预算、projection eligibility 和 Gate 全部冻结，先复跑 canonical 再复跑 9-cell。整体仍保持 `can_promote=false`，不进入 R5 learned router。Skill/MCP、provider、客户端和 CUDA 继续遵守执行计划的解冻顺序。
