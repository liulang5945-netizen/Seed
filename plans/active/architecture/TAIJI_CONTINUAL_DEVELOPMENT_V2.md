...
R4 已完成前三个边界子步中的前两个：`AdaptiveResidualGrowthPressure` 与 `AdaptiveResidualGrowthTrigger` 把 residual/conflict/utility pressure、parent digest、重复 evidence、预算和 checkpoint 状态固定为可审计合同；`Taiji.observe` 已在 R3 bridge 的真实 predictive observation→prediction/credit tick 产生 pressure，并把可重建的 decision 暴露给模型；`should_propose=true` 已固化为 content-addressed zero-impact candidate artifact，稳定 unit identity、proposal evidence、topology diff、source/parent digest 和预算均可恢复。decision/candidate 仍不会改变真实 bridge 拓扑。当前唯一下一步是从 bare parent 做独立 shadow materialization，先证明旧支持、权重和 runtime state 不变、候选 gate=0 且 fresh restore 一致，再允许 shadow 训练；候选进入 shadow 后还必须与 frozen、R3 fixed-capacity、random growth、同等最终参数量 fixed-large 对照，并通过 growth lesion、旧能力非劣、资源预算、fresh restore 与 rollback，才允许进入 R5 learned router。Skill/MCP、provider、客户端和 CUDA 继续遵守执行计划的解冻顺序。
