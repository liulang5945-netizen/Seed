...
R4 已完成 pressure、candidate、shadow materialization、candidate-only causal training 以及首个五臂短 S/G 技术 canary：`reports/taiji_m4v2_r4_shadow_canary_20260909.json`。随后加入逐 tick candidate activity、eligibility、projection credit、candidate-vs-parent residual、projection update 和 lesion trace；候选 projection 使用已有 eligibility trace 后，pressure candidate 的平均 candidate/parent residual ratio 从首轮约 `0.00380` 提升到约 `0.00981`，G lesion delta 为 `+0.00474`，但 S lesion delta 为 `-0.00027`，所以 credit 通路已被证明可工作，growth efficacy 仍未通过。进一步审计发现首轮五臂的 matched-capacity 训练边界并不一致，已校正为：五臂统一从 `pressure_parent` 进入，fixed-capacity 与 growth efficacy 使用同一 parent-bridge 学习边界，candidate-only 版本单独保留为冻结 parent 的技术 smoke。校正后技术 Gate 全通过；pressure growth 相对 fixed-capacity 在 G 改善约 `0.04233`、S 退化约 `0.00033`，相对 fixed-large 仍未占优，S lesion 仍为负。固定边界的 3 model-seed × 3 course-seed/order 矩阵证明 pressure growth 已相对 fixed-capacity 稳定改善（G `9/9`、S 均值改善约 `0.04915`），但相对 fixed-large 在 S 为 `0/9` 占优。单-anchor pressure birth 修复了 S 但专门化 G；top-2 birth mixture 的 9-cell 结果为：相对 fixed-capacity 的 S `9/9`、G `8/9` 改善，S lesion `9/9` 为正但 G lesion 仅 `2/9` 为正且均值为负，相对 fixed-large 的 S `8/9`、G `4/9` 占优。utility trace 显示 canonical pressure candidate 在训练期 G 的平均 residual·feedback 约 `0.0409`，但 G holdout lesion 为 `-0.00555`，说明全局 utility 不能完成跨上下文选择。exact counterfactual、direct exact gate credit、dual-state、three-source gate 与 aligned pressure birth 均已按正确时序和 canonical→9-cell 闭合：aligned output 使 formal G lesion 均值约 `-0.000008`、`7/9` non-worse，S 仍 `9/9` 正且均值 `+0.07515`，但 pressure 对 fixed-large 的 G 仍平均劣化 `0.00287`，所以这是“功能伤害基本消除”而不是“结构增长晋级”。最后的 birth homeostasis 已对 incoming/recurrent/output candidate mapping 做 deterministic weighted-second-moment norm matching；formal 中 G candidate lesion 均值 `+0.003081`（`6/9` 胜出、`9/9` 不劣），S 均值 `+0.080387`（`9/9` 胜出），相对 fixed-capacity 的 G/S 均为 `9/9` 改善，但相对 fixed-large 的 G 仍均值劣化 `0.001002`、仅 `4/9` 胜出。因此 R4 的因果贡献和技术闭合已成立，压力驱动生长优于等量预分配的结构性 Gate 未成立；`can_promote=false`，homeostasis 只留在 shadow 实验路径。R4 收束审计进一步确认独立 shadow、默认 parent checkpoint/forward 未被候选污染，R4 定向 6 项测试与本轮文件 ruff/black 通过；全仓 native、tracked ruff/B/SIM 和 core mypy 的交付审计仍需分别看待：本轮 tracked source 的主 ruff/B/SIM 已为 0，core mypy 已从 `175 errors / 24 files` 收敛到 `0 errors / 95 files`，但全量 native 仍有既有失败与 Windows 临时目录权限噪声，故仓库交付层尚未闭合。M7 第一批安全修复未改变 R4 或默认 parent。`context/delayed memory` 的失败已确认是旧测试继续走 action readout，而 `learn_bytes()` 已按 M2 合同训练 F1 predictive readout；测试现已显式选择 predictive，且原差异阈值通过。`test_synapse_longevity.py` 的失败也已归因为旧测试继续使用默认 action readout，而 `learn_bytes()` 的现行合同训练 F1 predictive readout；没有修改模型或放宽阈值。longevity 测试现已显式选择 predictive、关闭 memory/identity 干扰，decoder mass 保持且 surprise 从 `2.481` 降至约 `1.356`。`Workbench neutral baseline` 已完成归因：新的 task-boundary 合同要求显式 token，运行时拒绝无 token 是安全行为；旧 longitudinal evaluator 没有打开 boundary。评估器现已为每个 episode 打开覆盖实际 capability 的 boundary，并让 neutral、正常动作和 recovery 共享同一 token；Workbench longitudinal、interaction-group Workbench 与 structural bridge 相关回归共 `24 passed`。下一步在仓库可写 basetemp 下重新运行完整 `tests/taiji_native`，刷新修复后的失败账本并按首个真实失败继续归因；R4 shadow/默认 parent 不变，R5 learned router、Skill/MCP/provider、客户端与 CUDA 继续冻结。
