...
R4 已完成 pressure、candidate、shadow materialization、candidate-only causal training 以及首个五臂短 S/G 技术 canary：`reports/taiji_m4v2_r4_shadow_canary_20260909.json`。随后加入逐 tick candidate activity、eligibility、projection credit、candidate-vs-parent residual、projection update 和 lesion trace；候选 projection 使用已有 eligibility trace 后，pressure candidate 的平均 candidate/parent residual ratio 从首轮约 `0.00380` 提升到约 `0.00981`，G lesion delta 为 `+0.00474`，但 S lesion delta 为 `-0.00027`，所以 credit 通路已被证明可工作，growth efficacy 仍未通过。进一步审计发现首轮五臂的 matched-capacity 训练边界并不一致，已校正为：五臂统一从 `pressure_parent` 进入，fixed-capacity 与 growth efficacy 使用同一 parent-bridge 学习边界，candidate-only 版本单独保留为冻结 parent 的技术 smoke。校正后技术 Gate 全通过；pressure growth 相对 fixed-capacity 在 G 改善约 `0.04233`、S 退化约 `0.00033`，相对 fixed-large 仍未占优，S lesion 仍为负。固定边界的 3 model-seed × 3 course-seed/order 矩阵随后完成：pressure growth 相对 fixed-capacity 在 G 为 `9/9` 改善、S 均值改善约 `0.04915`，G lesion `8/9` 为正、S lesion `7/9` 不为负；但相对 fixed-large 在 S 为 `0/9` 占优。因此当前唯一下一步不是继续调学习率，而是只改变 pressure candidate 的出生初始化边界：用同一 `pressure_parent` 的 native activity/eligibility 形成可审计 birth anchor；pressure-driven 与 candidate-only smoke 使用 anchor，random-growth/fixed-large 保持随机出生；其余 parent、pressure、课程、预算、projection eligibility 和 Gate 全部冻结，先复跑 canonical，再复跑相同 9-cell 矩阵。整体仍保持 `can_promote=false`，不进入 R5 learned router。Skill/MCP、provider、客户端和 CUDA 继续遵守执行计划的解冻顺序。
