...
R4 已完成第一、第二个边界子步：`AdaptiveResidualGrowthPressure` 与 `AdaptiveResidualGrowthTrigger` 把 residual/conflict/utility pressure、parent digest、重复 evidence、预算和 checkpoint 状态固定为可审计合同；`Taiji.observe` 已在 R3 bridge 的真实 predictive observation→prediction/credit tick 产生 pressure，并把可重建的 decision 暴露给模型。该 decision 仍是 evidence-only，不会自行改拓扑。当前唯一下一步是把 `should_propose=true` 固化为 zero-impact candidate artifact；候选进入 shadow 后还必须与 frozen、R3 fixed-capacity、random growth、同等最终参数量 fixed-large 对照，并通过 growth lesion、旧能力非劣、资源预算、fresh restore 与 rollback，才允许进入 R5 learned router。Skill/MCP、provider、客户端和 CUDA 继续遵守执行计划的解冻顺序。
