...
R4 已完成前三个边界子步和最小训练 Gate：`AdaptiveResidualGrowthPressure` 与 `AdaptiveResidualGrowthTrigger` 把 residual/conflict/utility pressure、parent digest、重复 evidence、预算和 checkpoint 状态固定为可审计合同；`Taiji.observe` 已在 R3 bridge 的真实 predictive observation→prediction/credit tick 产生 pressure，并把可重建的 decision 暴露给模型；`should_propose=true` 已固化为 content-addressed zero-impact candidate artifact，稳定 unit identity、proposal evidence、topology diff、source/parent digest 和预算均可恢复；独立 shadow materialization 已复制 parent region 的旧支持、权重和 runtime state，追加新 unit，并以 identity-preserving sparse projection 对齐 parent 输出，gate=0 与 fresh restore Gate 通过；shadow 只在真实候选活动 tick 接收一次 causal residual credit，旧 region/projection/parent readout 冻结，bare/fresh restore、续步和 rollback 通过。decision/candidate/shadow 仍不会改变真实 bridge 拓扑。当前唯一下一步是运行短 S/G shadow canary，建立 pressure-driven、frozen、R3 fixed-capacity、random growth 与 matched-capacity fixed-large 的同预算对照；只有完成 growth lesion、旧能力非劣、资源预算、fresh restore 与 rollback，才允许进入 R5 learned router。Skill/MCP、provider、客户端和 CUDA 继续遵守执行计划的解冻顺序。
