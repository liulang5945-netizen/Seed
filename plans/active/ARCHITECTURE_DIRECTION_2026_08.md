...
> selection Gate 已完成；当前唯一入口是把跨区域 learner 接入真实 percept/world runtime 的
> 在线 credit loop，使 prediction error、资源状态与 holdout transfer 从实际 outcome 自动
> 更新，而不是由评估脚本手工注入。继续禁止固定 action/intent 表。下方长段保留为滚动历史记录。
