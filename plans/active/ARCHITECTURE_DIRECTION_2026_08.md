...
> selection、在线 credit、neuron-growth 和 region-growth Gate 已完成；当前唯一入口是为新生
> 区域补上独立的 post-growth holdout validation：只有新区域及其显式连接在未见输入上带来可
> 重复的目标改善，才允许从 proposal 状态进入长期发展记录。增长依据只看 substrate/区域
> 状态，不读取 action/intent 表。下方长段保留为滚动历史记录。
