...
> 实时覆写（2026-08-26）：neuron growth 与 cross-region wiring Gate 已完成；当前唯一入口
> 是把跨区域连接选择变成 prediction error/资源/transfer 驱动的学习型协作，继续禁止固定
> action/intent 表。下方长段保留为滚动历史记录。
