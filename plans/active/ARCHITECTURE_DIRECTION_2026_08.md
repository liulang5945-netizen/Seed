...
状态已滚动更新：四步连续重规划、3/4/5 步变量 episode、不同失败位置、after-state relation 变化，以及 executive-to-world prediction train/holdout 与 no-online-update calibration control 均已通过 3-seed Gate；当前唯一下一步是把 calibration 接入真实 runtime 的持续环境回写和可恢复 trace。
