...
状态已滚动更新：四步连续重规划、3/4/5 步变量 episode、不同失败位置、after-state relation 变化、executive-to-world prediction train/holdout 与 no-online-update calibration control，以及 runtime calibration trace 的多步连续性/恢复均已通过；当前唯一下一步是让 GoalPlanner 消费已校准的结构化 world prediction/error，并加入 world-model lesion。
