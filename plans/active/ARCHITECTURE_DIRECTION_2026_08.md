...
> 实时覆写（2026-08-26）：neuron growth、cross-region wiring 与 learning-driven route
> selection、在线 credit 与 neuron-growth Gate 已完成；当前唯一入口是把同一 substrate 驱动
> 闭环扩展到区域级生长：持续区域瓶颈只能生成可审计的 region proposal，并经统一 ledger 的
> 预算、checkpoint、lesion、holdout 和 rollback 后才能接入跨区网络。增长依据只看 substrate/
> 区域状态，不读取 action/intent 表。下方长段保留为滚动历史记录。
