...
> 依据只看 substrate/区域状态，不读取 action/intent 表。isolated-region split Gate 也已通过：
> `reports/taiji_region_split_20260826.json` 证明父区域身份、单位身份、局部状态和执行顺序可
> 迁移，并通过 holdout、budget、checkpoint continuation 与 reverse rollback；已有连接的
> split 会 fail-closed。connected split 现已补齐显式稀疏连接迁移和 route learner lineage，并由
...
> runtime evidence 入口。每个网络 tick 为每个区域保存可 checkpoint 的 activity usage、资源压力、
> prediction error、learning gain、holdout transfer 和稳定 evidence ID；无 expected activity 时
> 不伪造 growth supervision，但仍可为 pruning 提供使用率与资源压力。route learner 继续从真实
> target activity 获得 credit，native checkpoint 会恢复 runtime tick、最近观测和跨 tick error
> continuity；tick 本身不直接改拓扑，任何 growth/pruning/split/merge 仍必须经过 holdout、budget、
> trial checkpoint 和 reverse rollback。Gate 报告为 `reports/taiji_runtime_structure_20260826.json`，
> manifest 为 `plans/manifests/taiji_runtime_structure_v1.json`。在其上已建立
> `StructuralProposalCandidate` 队列：split、region/connection prune 和兼容 region merge 候选
> 均携带稳定身份、来源 tick、evidence、priority、参数和 resource cost，并随 native checkpoint
> 恢复；队列去重且有边界，不会绕过 ledger 直接上线。下一入口是将候选显式 materialize 为
> `StructuralTopologyProposal`，统一执行 holdout、budget、trial checkpoint、commit 与 reverse
> rollback，继续禁止 action/intent 查表。candidate materialization 已完成：候选可变成 pending
> topology proposal，仍不改变 live topology，并保留 candidate→proposal 映射以支持 checkpoint
> continuation 和幂等恢复。下一入口是为不同 candidate operation 接通统一的 holdout validator
> dispatch，阻止未验证候选进入 commit。该 dispatch 已完成：split、region prune、connection
> prune、merge candidate 会按 topology role 路由到现有 holdout validator；验证只更新 pending
> proposal 的 validation score/status，不直接提交 live topology。下一入口是将通过 holdout 的
> candidate 交给统一 commit dispatcher，并保留 budget/trial/reverse rollback 的最后闸门。
> commit/rollback dispatcher 已完成：candidate 可按 topology role 进入对应 ledger，依次执行
> validation score、structural budget、trial checkpoint、live topology mutation 与 latest-change
> reverse rollback；Gate 已覆盖 commit 后 topology 改变、回滚后父拓扑恢复和 checkpoint continuation。
> 下一入口是将 candidate queue 的 operation 参数与真实 holdout 数据绑定，形成可批量但仍逐项
> fail-closed 的 maintenance cycle。该 cycle 已完成：按候选独立 materialize、holdout、commit，
> 缺数据/异常/预算不足只影响当前项；每项结果保存 status、proposal_id、validation score 和
> error 并进入 checkpoint，已通过多候选 commit/rollback Gate。当前 cycle 已覆盖
> `add`/直接 neuron birth 已纳入同一 candidate contract：standalone native tick 生成的候选也经过
> 统一 materialize、holdout、budget、trial checkpoint、commit/rollback 和 native checkpoint
> continuation；direct-add 子门禁已写入 `reports/taiji_runtime_structure_20260826.json`。下一入口
> 是为 maintenance cycle 增加显式 candidate dependency/conflict 判定，对竞争变更、父子拓扑
...
> `scripts/training/eval_taiji_native_runtime_profile.py`，报告为
...
> structure Gate。`SparseSynapses` 现在只在设备不一致时转换输入，norm 常量按 device/dtype/limit
...
执行顺序、验证和训练日程只看 [模型优先统一开发计划](roadmap/03_CURRENT_EXECUTION.md)。P1～P7、structural growth、topology ledger 与 neuron growth 等既有结果继续作为历史机制证据，但不再拥有优先级；当前必须先完成 M0 CPU 五项基础能力真实性基线，再立即进入 M1 foundation 训练。继续禁止固定 action/intent 表。
