...
**当前唯一下一步：把 outcome hypothesis 分布接入多步 imagined rollout 的风险敏感规划。** 让每个 world-dynamics rollout step 传递 expected reward/success、ledger uncertainty mode 和 risk budget，规划器按 stochastic/conflicted/unseen 采取不同的风险惩罚；覆盖多步 relation holdout、随机结果与冲突结果 lesion、checkpoint continuation，CUDA 继续暂缓。
