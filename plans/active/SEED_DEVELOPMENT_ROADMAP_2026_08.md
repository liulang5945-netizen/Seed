...
**当前唯一下一步：把 ledger 的 outcome 分布接入 world prediction 与 uncertainty。** 让 `WorldDynamicsLearner.predict()` 同时暴露主假设预测和基于 ledger 的结果不确定性，规划器/adapter 使用同一 uncertainty 语义区分“模型未见”与“环境本身随机”；覆盖概率校准、relation-specific holdout、checkpoint continuation 和 world-model lesion，CUDA 继续暂缓。
