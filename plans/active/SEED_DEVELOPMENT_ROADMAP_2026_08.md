...
**当前唯一下一步：建立 ledger-driven probability calibration Gate。** 用独立 relation-specific holdout 测量 outcome reward/success 的 Brier/NLL、置信度覆盖与 calibration trace，禁止 holdout 反馈污染 ledger；同时验证未知 context 的 unseen uncertainty、随机 context 的 stochastic uncertainty 和 native checkpoint continuation，CUDA 继续暂缓。
