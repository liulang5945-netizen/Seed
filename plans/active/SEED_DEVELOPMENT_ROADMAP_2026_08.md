...
**立即执行 R0：审查并收敛当前未提交的工程加固批次，生成晚于最终代码的完整验证报告，让 `main` 恢复为干净、可复现基线。**

在 R0 完成前不续跑 100M、不改 Taiji 学习方程、不写自定义 CUDA kernel，也不删除 `neuroplex/`。R0 完成后自动进入 R1；R1 的真实 profiler 结果是下一个需要讨论的决策点。
