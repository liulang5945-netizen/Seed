...
**当前唯一下一步：把 registry 的冲突/confidence adjudication 接入真实 adapter `WorldTransition` outcome 闭环。** 让每次真实 transition 的 before/action/after 形成稳定 evidence key，由 registry 对重复一致结果增信、对矛盾结果拒绝并驱动 world learner fail-closed；覆盖跨 episode 重复样本、同一 context 的矛盾 after-state、prediction calibration、registry/network 双 checkpoint continuation 和 relation-specific holdout，CUDA 继续暂缓。
