...
**待人工执行（agent 令牌无 `administration:write`，见 14.10）：在 GitHub 网页 Settings/首页填入以下已定稿内容。**
...
- social preview：GitHub 无 REST 接口，只能网页上传；建议图上直接写 `0% → 94.12%` 与 `no backprop / no attention`。
