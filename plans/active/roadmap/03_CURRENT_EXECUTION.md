...
当前唯一下一步为 **M1-56：replay admission/objective attribution**：不改核心网络、拓扑、默认 target 或 checkpoint，建立 phase-B-aware 的 replay 资格诊断，按每条 phase-A replay episode 记录它与 phase-B **train** 的 cue/event 相似度、action×outcome 组合、association residual、readout margin 变化以及 replay 后对 phase-B train 的即时影响；比较 `all replay`、`replay admission trace-only`（只记录资格不改变写入）和 `no-replay`，资格依据只能来自训练分区，禁止读取 phase-B holdout/retention。报告必须证明 admission 统计与实际 replay 顺序/数量可追溯，并保留四组合计数、fresh-process checkpoint、参数预算、topology digest 与 `holdout_updates=0` Gate。该片先回答“是否存在稳定的、由训练分区证据支持的冲突 replay 子集”，不直接启用过滤或改变学习规则；只有冲突子集跨 seed 稳定，才允许下一片做一次等预算的 admission candidate，否则停止继续调 replay，转向更高质量的 memory 训练数据设计。
