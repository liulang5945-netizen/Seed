...
当前唯一下一步为 **M1-44：event-component gain ablation**：保持 shared/all、最大分离 cue、重叠 action、`memory_units=128`、`experienced` provenance、同一事件顺序和 identity organ 关闭，只比较当前 `memory_event_gain=0.80` 与 cue-dominant `memory_event_gain=0.0`；不改变 association 拓扑、fan-in、decoder 或参数预算。记录 cue/event cosine、event active support、association completion/error、old holdout、old retention、new holdout、BWT、causal gain、new delta、checkpoint continuation、exact roundtrip、holdout side effects 和参数预算。该实验只判断 action/outcome/time/episode/provenance 附加分量是否干扰 cue→action 绑定；即使 cue-only 通过，也必须先进行信息损失评审，不能直接删除事件分量。
