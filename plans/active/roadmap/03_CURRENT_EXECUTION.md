...
**当前唯一动作：E6-1 MCP 客户端候选的 API/registry shadow 接线**。E3-0 已完成 parent/trial/admitted checkpoint 预检，E3-1 已完成 route/interaction credit，E3-2 已完成 procedural memory intake，E3-3 已完成真实 `WorldTransition` 的 world prediction/local update，E3-4 已完成多 seed fixed-capacity 对照、容量压力测量和持续失败触发前置：32 hidden units / 775 parameters 下，native holdout error 均值为 0.2785，跨 seed 标准差 0.0216，retention regression 为 0，容量压力虽为 1.0，但固定容量未持续失败，因此结构候选触发保持 false。E4 已通过 [artifact internalization report](../../../reports/taiji_w7_e4_artifact_internalization_20260901.json)：14 个 train/holdout/retention 语义样本与 4 条对应经验进入 Taiji-owned semantic/procedural/affordance 学习，semantic holdout loss 从 1.0 降至 0.0000399，procedural holdout accuracy 为 1.0、procedural lesion 为 0.25，affordance holdout MSE 从 1.19976 降至约 1.15e-12；跨 scope 身份隔离、外部描述关闭、检疫拒绝、checkpoint roundtrip 和客户端执行边界均通过。E5-0 已通过 [client extension host report](../../../reports/taiji_w7_e5_client_extension_host_20260901.json)：12/12 检查通过；E5-1 已通过 [client snapshot integration report](../../../reports/taiji_w7_e5_1_client_snapshot_integration_20260901.json)：8/8 检查证明 client-extension API 与当前 Workbench capability snapshot 绑定、prepare/commit 两阶段发布、stale fail-closed、Vue facade 的声明式 slot projection 和 App 注入均通过；E5-2 已通过 [legacy plugin surface report](../../../reports/taiji_w7_e5_2_legacy_plugin_surface_20260901.json)：9/9 检查证明旧 `/api/plugins`、marketplace/upload 和 Legacy manager 已统一退役为 410 tombstone，重复路由和前端旧引用已清除；E5-3 已通过 [slot runtime report](../../../reports/taiji_w7_e5_3_client_slot_runtime_20260901.json)：8/8 检查证明本地只读 extension 可按 content digest 挂载、卸载、替换并展示失败/隔离状态；E6-0 已通过 [MCP client capability boundary report](../../../reports/taiji_w7_e6_0_mcp_client_capability_boundary_20260901.json)：10/10 检查证明候选只含声明式 schema/权限/网络 scope/credential reference/资源预算和证据 digest，registry 漂移、外部调用、凭据访问和执行源均 fail-closed。下一步只把该候选投影到 Seed-owned API/registry 的 shadow 状态，不连接真实第三方 MCP、不激活客户端器官、不把客户端执行器写入 Taiji。
...
- **客户端插件：E5-0/E5-1/E5-2/E5-3/E6-0 已完成，E6-1 待推进。** `seed_platform/client_extension_host.py`、`api/routes_client_extensions.py`、`frontend/src/composables/useClientExtensions.js` 和 `frontend/src/components/ClientExtensionSlot.vue` 已接入声明式 client snapshot 两阶段发布与真实 slot projection；旧 `/api/plugins`、marketplace/upload 已统一 410 tombstone，重复入口和前端旧引用已清除；E6-0 已冻结 MCP client capability candidate 的独立 schema、policy 和 no-side-effect shadow 合同。当前仍不能声称真实第三方热插拔，下一步只做 Seed-owned API/registry shadow 接线。
