...
当前唯一下一步的正文见第 6 节执行流水的末条；本节只保留该步的 owner 边界，避免同一事实出现两份维护。M2-2a～2e 已完成真实语料隔离、可恢复 continuation、no-replay 与 exact-replay 对照；M2-2f 已在 clean revision 的 seed 11 course 中完成结构性验证但 B2 keep 未通过；M2-2g 已在同一旧 parent 的 clean seed 11 1 MiB course 与 fresh-process 复验中形成正向单变量证据：predictor-only 不写 shared fabric 时，B2/B4 完整保持且语言四项 BPB 均改善。

**M2-2h 代码级所有权合同已完成（20260905，尚无新的 1 MiB 数值结论）**：`BytePredictiveContext` 成为 F1 的独立 owner。它有独立的 cortical receptor map 与零初始、固定 fan-in 的时间递归 residual；当前 tick 的 F1 decoder error 在 decoder 写入前反投影，并以“生成该预测时实际使用的上一时刻 private context”作为 eligibility trace 更新 residual。它只能读取 shared fabric 的前向活动，持久写入被物理限制在 private residual 和 F1 decoder；shared fabric、ByteMotor、memory field 与 identity value 继续由既有 Gate 逐位保护。容量（fan-in/ratio）、学习率、residual gain 和 RNG stream 全部进入 `TaijiConfig`/`CapacityPolicy`，不以隐式魔数伪装成长机制。
