...
进入 **W7-R2-S0 interaction-group 与恢复归因确定性模拟**：R1 的 S0/S1/S2 已通过，沿已冻结的 [R2 manifest](../../manifests/taiji_w7_r2_interaction_group_v1.json) 从真实 W2/W3 trace 构造最小贡献、互补、冲突和恢复归因样本；本步不得预设“规划神经元/记忆神经元”等角色，也不得把交互分析直接写回运行时结构。

## 后续唯一顺序

1. 完成 W7-R1：provider watchdog 的 S0/S1/S2（已完成）。
2. 执行 W7-R2-S0；通过后再进入 R2-S1，之后按 R3 → R4 → R5 推进；每个方向先做可证伪 Gate，再接入真实运行时，最后才更新产品展示。
