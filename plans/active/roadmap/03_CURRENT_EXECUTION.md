...
**当前唯一下一步：为 E8 的 bounded replay 写 red——容量上限下的旧能力保持判据。** E8 的 Gate 含「多周期净能力收益、旧能力保持、checkpoint 大小/延迟预算、污染隔离」，其中唯一可在无 CUDA、无外部治理输入的条件下先行闭合的是 replay 采样合同。owner 已由实读确定为 [taiji/evolution_training.py](../../../taiji/evolution_training.py)：它已拥有 `NativeEvolutionTrainer.consolidate()`、三分区互斥校验与 `consumed_experience_ids`，但当前 `_experience_tuple()` 是无界的——训练集有多少就全量成例，没有任何容量上限，也没有「失败与纠正样本必须被保留」的采样优先级，因此 E8 的「旧能力保持」目前无判据可测。red 的形态：在 [test_evolution_training.py](../../../tests/taiji_native/test_evolution_training.py) 新增一个 bounded replay 用例，当训练经验数超过容量上限时，采样必须保留失败样本与 `retention` 分区证据、不得被新成功样本挤空，且采样选择内容寻址以便跨重启复现。不新建模块（避免与 `config.py` 已有的 `replay_*` 参数造成第二套 replay 机制），不得 import `seed_platform`。CI 暂缓、E6-5b 搁置、CUDA 阻塞与下文登记的 openapi 基线红均不阻塞本步。
...
- **提交/推送：本轮已提交，暂不推送。** E6-0 为 `90616a7`，E6-1 为 `f8abd0a`，E6-2 为 `68ab08b`，E6-4 为 `9c31fa3`，E6-5a 为 `4d72ce8`，S34 批次时序修复与 E6-5b 搁置为 `a86851f`，命名边界红收口为 `0ca96f7`，57 个已完成分片归档与引用改写为 `699e7a6`，`62`/`63` 活动内容并入骨干与归档为 `3e76a55`，E7 前四条 Gate 与 `seed_platform` 边界硬化为 `cd67e35`，E7 第五条消融归属 Gate 闭合为 `53224a4`；不执行远端同步。
