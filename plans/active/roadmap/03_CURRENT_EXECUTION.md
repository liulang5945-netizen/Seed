...
当前唯一下一步为 **M1-29：identity organ 的跨任务/容量增长 admission review**：在不打开默认 gate 的前提下，把已通过的 native organ 放入更大 B2/B5 课程与受控 cue 相似度/容量压力测试，验证 route 的边界是否随 capacity 成长、相似 cue 是否正确拒绝/分裂、replacement 后旧 action 是否彻底失效，以及 identity evidence gain 对 shared fallback 的干扰上限；必须继续跑三 seed、lesion、no-change、fresh-process checkpoint 和 CI 定向 Gate。只有该 review 证明 organ 的增长是可解释、可回滚且不会把 episodic identity table 固化成通用语言能力假设，才讨论是否生成 default-candidate checkpoint。
