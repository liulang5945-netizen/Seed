...
当前唯一下一步为 **M1-49：explicit association target-credit mix**：把当前 `association` 的 cue→event target 从 `memory.py` 固定写死的完整 `event_pattern` 抽为可序列化配置 `memory_association_event_target_mix`，默认 `1.0` 保持行为不变；只比较 default `1.0` 与 candidate `0.50`，candidate 仅改变 cue→event association target，event-pattern readout、自关联 target、learning rate、event component gains、decoder、topology、memory units 和 identity organ 均不变。记录 target mix/vector digest、association completion/error、old holdout、old retention、new holdout、BWT、causal gain、new delta、checkpoint continuation、exact roundtrip、holdout side effects 和参数预算。只有 candidate 三 seed B5 全通过，才进入 M1-50 target-credit stability review；否则保留参数化改进但回滚 candidate，继续拆分 association target 的 action/outcome credit。
