...
当前唯一下一步为 **M1-23：cue→有效突触支持覆盖审计与隔离原型**：冻结默认 checkpoint、shared decoder、训练数据与 replay，先对 phase-A/phase-B cue 在现有 `cue_encoder → association/readout_receptors → action_readout` 链上的实际 active support、跨 phase 相似度、共享 edge 比例和输出有效 fan-in 做只读审计；随后只在隔离 evaluator 中比较现有 shared support 与“同一 decoder 的 cue-conditioned support mask”两种写入/读取路径，不新增答案表、不扩大参数、不接入默认 checkpoint。Gate 必须同时检查 old/new holdout、support coverage、cross-phase collision、no-change、checkpoint digest 和 fresh-process round-trip；若 support mask 仍不能形成可迁移 old/new 双保留，则停止继续堆叠 replay/decoder，转入 cue 表征结构重设计。
