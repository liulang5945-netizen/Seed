...
**当前唯一动作：E3-4 固定容量对照与结构增长触发前置 Gate**。E3-0 已完成 parent/trial/admitted checkpoint 预检，E3-1 已完成 route/interaction credit，E3-2 已完成 procedural memory intake，E3-3 已完成真实 `WorldTransition` 的 world prediction/local update：holdout error 从 3.1298 降至 0.3080，replay-only 等于 frozen，retention 在门限内，checkpoint 可恢复。现在先建立 fixed-capacity 多 learner 对照、持续误差/容量压力证据和结构候选触发条件；只有固定容量在独立未见任务上失败，才允许生成结构候选，仍不直接扩容、不接 provider。
