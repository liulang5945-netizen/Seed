...
当前唯一下一步为 **M1-47：association learning-rate candidate**：在默认六维 component gains、shared/all、最大分离 cue、重叠 action、`experienced` provenance、同一事件顺序和 identity organ 关闭下，只比较现有 `episodic_learning_rate=0.60` 与 association-only candidate `1.20`；不改变 readout learning rate、event gain、association topology、decoder、memory units 或参数预算。记录 component vector digest、phase-A association completion/error、old holdout、old retention、new holdout、BWT、causal gain、new delta、checkpoint continuation、exact roundtrip、holdout side effects 和参数预算。只有 candidate 三 seed B5 同时通过，才进入 M1-48 的稳定性复验；否则冻结“单纯提高 association 学习强度”解释，保留当前默认配置并转向更明确的 association target/credit 设计。
