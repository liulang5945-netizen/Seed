...
| 7 | M4 连续成长 | **M4.R0 已完成且不晋级；M4.R1 formal 已完成但 retention Gate 否决；M4.R2 capacity diagnosis canary 已完成且不支持扩容；M4.R3 seed11 smoke 已完成，需三 seed smoke 验证** | 三 seed 必须在同一 record-disjoint 三周期课程上复现 owner 归因、旧能力 retention、checkpoint/fresh-restore 和成本；在归因完成前不扩容、不把新分支当作成长成功 |
...
**唯一下一步：完成 M4.R3 的 seed11/29/47 三 seed smoke 与 aggregate。** M4.R2 已验证 A/B 的旧 C cycle2/cycle3 均未退化，但零初始化增量槽没有带来 C″ 新留出收益（A/B gain 均为负），因此当前没有容量压力证据，不能继续扩容。M4.R3 seed11 已在同一 fixed-capacity、record-disjoint C→C′→C″ 课程上完成 readout-only/context-only/joint 三臂 owner 归因；现在只把同一 canary 复制到 seed29/47 并聚合，检查 joint 的正向信号是否可复现且旧 retention 不退化。三 seed smoke 必须验证每臂实际写 owner、protected readout/fabric/memory 不变量、C/C′/C″ 新收益、旧能力 retention、checkpoint/fresh-process 和成本；不以单 seed 结果晋级，不启用 gated temporal candidate、接入 provider/MCP 或接线真实客户端写入。
...
### M4.R3：固定容量更新干扰与表征 credit 归因（seed11 smoke 已完成，三 seed smoke 下一步）
...
- **当前停止线**：先跑 seed11/29/47 同预算 smoke aggregate；只有 joint 的新留出收益和旧能力保持在 cohort 上同时成立，才进入 formal 或更新规则实验。若 cohort 不复现，回到数据密度、课程难度和学习率审计。所有结果先保持 `can_promote=false`。
