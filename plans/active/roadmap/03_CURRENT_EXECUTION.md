...
当前唯一下一步为 **M1-41：staged association/readout write ablation**：在 shared 默认 decoder、M1-38 posthoc 顺序、最大分离 cue、重叠 action、`experienced` provenance、`memory_units=128` 和 identity organ 关闭下，比较当前单次 `all` 写入与同一 episode 先做 `association`、再做 `readout` 的 staged 写入。诊断必须固定事件集合、训练事件数量、seed、replay scale 和 replay target，并明确报告 staged 写入因额外 phase 造成的 write-count/step 差异；同时记录 association/readout 各自 checkpoint digest、old holdout、old retention、new holdout、BWT、causal gain、new delta、continuation、exact roundtrip、holdout side effects 与参数预算。该实验只决定“写入时序”是否值得进入结构设计，不直接改变默认训练规则；若 staged 通过，才进入 M1-42 的单一原子写入 API 设计，否则冻结当前 write target 解释并回到事件编码与 association 几何诊断。
