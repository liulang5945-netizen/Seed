...
当前唯一下一步为 **M1-36：cue-distribution curriculum diagnosis**：保持 `memory_units=128`、shared decoder、identity organ 关闭、replay target `all@1.0` 和 action alphabet 不变，只把 phase-A/phase-B 的 cue 数值分布改为字节空间两端的最大分离区间，比较 cue collision/上下文相似度、old holdout、old retention、new holdout、causal gain、checkpoint continuation 与资源。若最大 cue 分离仍无法通过 B5，则停止继续从输入编号解释遗忘，下一片转向事件表示的训练信号；若通过，再扩大到未见 cue 分布验证泛化。
