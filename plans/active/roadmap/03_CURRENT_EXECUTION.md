...
10. **candidate utility/selectivity 诊断（当前唯一下一步）**：逐 tick 保存 candidate residual、同一 causal feedback、candidate utility（贡献方向与 feedback 的内积）、candidate/parent residual ratio 和 phase 标签；先不改变 forward/learn 公式，只确认 G 退化来自 residual 方向错误还是候选在无效上下文持续输出。utility 必须来自 native predictive error，不读取 task ID、不使用 holdout 反向调门。
11. **candidate output selectivity gate**：诊断确认后只允许增加一个候选局部输出选择门，门状态与 eligibility/utility 一起 checkpoint；parent、pressure、birth mixture、课程、projection learning、fixed/random 对照不变。先跑 canonical，再复跑相同 9-cell 矩阵。
...
R4 的最小交付仍是 `pressure/proposal → shadow materialize → shadow train → validate → lesion/admit/rollback` 的一个可复现 CPU canary 和 versioned report；边界校正和多 seed/order 已证明 pressure growth 相对 fixed-capacity 有稳定收益，birth mixture 进一步暴露了 candidate 输出缺少上下文选择性。下一轮先补 utility 证据，再决定唯一的 output selectivity gate；不能把 pressure proposal、候选存在、materialization、单次训练步或单次 BPB 下降写成成功；R4 通过后才解冻 R5 自主路由，Skill/MCP/provider/客户端继续按第 6 节冻结。
