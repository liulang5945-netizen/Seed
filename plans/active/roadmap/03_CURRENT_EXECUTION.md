...
**M1-66b（当前唯一下一步）：解耦「cue 原型颗粒」与「读取的 value 裁决」，让同 cue 不同 context 不再折进同一原型**。已有等价性诊断（见上）将问题约束在 bank 的匹配合并：写入时 key 相似度 ≥ `match_threshold` 即被判定同一 cue 而并入现有原型，context 维度被吞。候选方向（按上限排序）：① 给 bank 写入侧加「原型内 action 冲突检测」——若新 key 与已占槽相似（≥阈值）但绑定 action 与槽内现有 action 不同，强制开新槽（槽位充足时 `repl=0` 容量 1024 下完全可行），把「同 cue 异构 action」从合并改为分槽；② 若冲突检测不彻底，把 cue 原型从「单一矩心」升级为「每槽多矩心（per-context）」，即 CueBindingBank 的 prototype 改为小 cluster，route 时对 cluster 内各矩心分别算相似度——这是上限最高的形态，直击「context 被吞」的根源；③ 若二者皆非（冲突确实由组织 slot 竞争引起），则放弃复合 key 折叠修复，转而在课程层让 `CUE_SPAN`/`CONTEXT_BASE` 的 key 空间更稀疏（改课程，需 manifest 变更，最次选）。验证门禁与 M1-66 相同：三 seed beats 两消融 + `worst_seed_margin_positive=true` + B1/B3/B4/B5 逐位非退化 + 探针双绿。
