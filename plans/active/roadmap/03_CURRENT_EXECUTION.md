...
H3.6-B按预注册停止规则结项：不读取H3.5-A或H3.6-B final，不追加同质epoch，不进入S2/L2/Mini，也不把surprise、collision或bridge影响写成模型能力。H3.7已把复审结论冻结为4-slot/48维/16-byte phase的分解式 workspace 与 renderer→bridge→slot credit 实现门；三seed正式dev、三组只读消融和aggregate均已完成，aggregate在final前停止，明确不追加epoch、不读final、不切默认入口；当前回到冻结归因顺序，H3.6-B与H3.7 child均作为只读失败证据。
