...
当前唯一下一步为 **M1-65：给记忆通路补上「延迟后恢复 cue 状态」的机制，即把寻址 key 从 fabric 瞬时活动改为一个跨干扰保持的持久痕迹**。这是 M1-64 分支条款「转向更高容量的可塑性结构」的具体落点，且必须按上限更高的方案做：不是把 `identity_organ_match_threshold` 从 `0.9` 调低（那只会让近乎正交的 `0.0983` 也算命中，把寻址失败换成随机误命中，是伪装成修复的指标操纵），也不是扩容（反证一已实测扩容到 `4096` 槽位、`3108801` 参数带来逐位零收益），而是引入一条**在干扰期间持续维持 cue 表征**的结构：cue 被观测时在 fabric 上留下一个带衰减的持久痕迹，写入与读出都以该痕迹为 key，使 `cos(write_basis, read_basis_with_interference)` 从实测的 `mean 0.0983` 抬到 `match_threshold` 以上。开工第一件事不是写结构而是**先固化验收探针**：把本段三个一次性探针里的余弦测量沉淀成 `tests/taiji_native/` 下的常驻测试，断言「经过至少 1 个干扰符号后，写入基与读出基的余弦 ≥ `identity_organ_match_threshold`」——按规则先让它在当前实现上跑出 red（实测应为 `0.0983 < 0.9`），再动实现，这样机制是否真的解决了寻址问题就有了独立于准确率的判据，不会再出现「小课程满分但 foundation 零因果」的错位。该测试变绿之后，才允许用同一份 `eval_taiji_m1_64_foundation_memory.py`（不改课程、不改判据、不降 `interference_length`、不动 manifest 下限 `1000/200/200` 与三 seed）复跑 B2，并要求 `beats_memory_lesion` 与 `beats_identity_lesion` 在三 seed 上同时成立、`worst_seed_margin_positive=true`，同时带 `--b1-corpus data/simple_zh/dialogue_extended_clean.jsonl` 让 B1/B3/B4/B5 的非退化在同一份报告里闭合。若新机制引入了新的持久状态，必须同步扩 `_persistent_digest` 的覆盖面（M1-64 已证明该函数漏掉新键会让 `holdout_updates=0` 变成空断言），并保持剔除 `lineage` 的既有约定。判定条件同样预先写死：若余弦测试转绿但 B2 三 seed 仍不能同时超过两条消融，则说明「可寻址」不等于「可决策」，问题在 value 读出而非 key，下一步转向读出头的容量与训练资格，而不是再回来调 key。
