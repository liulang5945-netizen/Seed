...
当前唯一下一步为 **M1-43：governed association topology-capture candidate**：复用现有 `SparseSynapses.structural_update`，仅对 `memory.association` 在每次受控 write 后按当前 cue trace、association error、`structural_turnover_ratio=0.25`、`structural_capture_target=0.90` 和 `structural_error_threshold=0.35` 做候选 rewiring；不增加 memory units、fan-in、decoder 或 identity organ，默认 shared/all 路径保持关闭。候选与 no-rewire control 必须使用同一 corpus、事件顺序、provenance、seed、训练步数和 replay target；记录每个阶段 topology digest、moved edges、capture/error、old holdout、old retention、new holdout、BWT、causal gain、new delta、checkpoint continuation、exact roundtrip、holdout side effects 和参数预算。只有候选三 seed 同时通过 B5 且结构变化可回滚、参数数量不变，才进入 M1-44 的结构候选评审；否则不保留 association rewiring，回到 event encoding 与 association geometry 的更小变量。
