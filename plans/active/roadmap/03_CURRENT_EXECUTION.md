...
当前唯一下一步为 **M1-45：event-component geometry audit**：不改训练规则或 checkpoint，只把 event pattern 的六类 component（action、outcome、reward、time、episode、provenance）逐一审计。对每个 seed 记录每个 component 的 active support、与 cue 的 cosine、component 间 cosine/能量占比、phase-A/phase-B component cross-cosine、完整 event pattern 的 association completion/error，以及 checkpoint 和参数预算。该审计用于识别具体 nuisance component 或 component collision；只有找到跨 seed 稳定且与 B5 失败一致的单一 component，下一步才允许做该 component 的最小增益候选，否则冻结 event encoder 解释并转向 association learning signal。
