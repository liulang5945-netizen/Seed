...
- **客户端插件：E5-0 核心合同已实现，实际产品接线尚未完成。** `seed_platform/client_extension_host.py` 已提供 Seed-owned extension host；当前 Vue SPA/API 尚未接入 client snapshot 两阶段发布，插件/MCP 市场仍指向未实现的 Legacy manager；在 E5-1 完成前不得声称客户端支持热插拔。
