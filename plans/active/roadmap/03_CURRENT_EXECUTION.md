...
当前唯一下一步为 **M1-58：dynamic protected replay candidate**：保持 factorial 课程、默认 `all@1.0`、shared decoder、现有拓扑和 checkpoint lineage 不变；从 phase-B checkpoint 开始，固定执行 64 次 replay attempt，每次写入前保存可恢复 checkpoint，只用 phase-B train 的 action/outcome margin 判断该写入是否提交：若写入后两类 margin 相对提交前同时下降，则恢复写入前 checkpoint，否则提交。保护决策阶段禁止读取 phase-B holdout/retention，必须记录 attempt 数、accepted/rejected 数、每步 before/after margin、rollback checkpoint digest、write-order digest、memory digest 与物理 write count；候选结束后才允许做一次只读的 phase-A old holdout/retention 和 phase-B new holdout Gate。比较 `no-replay`、当前 `all replay` 与 protected candidate；只有 protected candidate 在三 seed 同时保住 old/new、两类输出且 checkpoint/参数/holdout 隔离全部通过，才允许讨论把保护机制收敛为主路径，否则回滚候选、保留 trace 和 target contract，冻结 replay objective，转向更高质量 memory 训练数据设计。
