...
当前唯一下一步为 **M1-38：ordered cue/action event-binding curriculum**：在 M1-37 的 strongest baseline 上保持最大分离 cue、重叠 action、`memory_units=128`、shared decoder、identity organ 关闭、`all@1.0` 和 `experienced` provenance，只改变 phase-B 与 phase-A 的 replay 顺序：比较当前 phase-B 完成后批量回放 phase-A 与按 phase-B episode 交错插入 phase-A event 的顺序。两者必须使用同一事件集合、同一 digest、同一训练步数和同一 seed；记录 event-binding order、old holdout、old retention、new holdout、BWT、causal gain、new delta、checkpoint continuation、holdout side effects。若 interleaving 通过三 seed B5，才进入 M1-39 的最小结构性事件绑定候选；否则冻结数据顺序解释，转向显式 cue/action binding 读写路径的单变量消融。
