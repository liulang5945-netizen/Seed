...
当前唯一下一步为 **M1-62：训练数据与任务边界重构契约**。先停止修改 `EpisodicField`、replay filter 和 objective scalar，新增 evaluator-owned 的 `MemoryLearningExample` 数据契约与 digest：明确稳定 `cue_key`、可独立组合的 action/outcome value、reward、时间/episode/provenance 旁路字段、重复观测与一次性经验的 role，并强制 train/holdout/retention 的 key/value/查询关系可审计。生成两个对照课程：稳定 key→value 的重复观测课程，以及故意多值冲突的 negative control；两者都使用 factorial action×outcome，不能让 evaluator 预置答案。先只验证数据可学习性、分区不泄漏、目标 credit 可分离和训练前 checkpoint preflight，再比较当前 native association 与 identity organ 仅作上限参照；不改默认 checkpoint。只有数据契约本身通过且 native path 在稳定 key 课程上出现可复现的正绑定，才允许重新设计真正的 key/value learner；否则承认当前 native association 不是合适的训练承载层，转向更高层的可塑性/记忆器官设计。
