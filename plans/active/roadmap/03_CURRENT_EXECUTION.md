...
**当前唯一动作：E1 统一进化语料/经验合同与 checkpoint 前置 Gate**。建立版本化、内容寻址、append-only 的 `EvolutionCorpusArtifact`、`EvolutionExperience` 和统一 ledger：前者接收经治理的 Skill/MCP artifact 知识，后者接收真实 `WorkbenchOutcome` 与后续调用结果。使用确定性 Skill、MCP 和客户端插件 lifecycle fixture 验证来源绑定、用途/许可、partition 锁定、taint/redaction、去重、hash chain、checkpoint 写入/关闭进程恢复/继续追加和损坏 fail-closed。E1 不训练权重、不安装插件、不接 Legacy MCP manager，也不修改客户端 UI；只有它通过后，E2 才接真实 Skill/MCP/客户端插件来源，E3 才第一次直接更新 Taiji-owned semantic/procedural knowledge、route、world prediction 和局部突触。
