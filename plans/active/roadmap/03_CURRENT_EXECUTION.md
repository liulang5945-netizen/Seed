...
M4.V2.R0、M4.V2.R1、R2 与 R3 已完成当前阶段的量尺、迁移、快/慢学习和第一条主路径结构桥 Gate；R2 候选被接受，R3 canary 通过；R4 pressure 已由 R3 bridge 的真实 predictive observation tick 驱动，candidate artifact 已被固化，且独立 shadow materialization Gate 已通过。整体仍保持 `can_promote=false`。当前唯一下一步是 **M4.V2.R4：在 shadow 上完成 bare checkpoint/fresh restore 后的一次最小 residual 训练步与回滚验证**，不向真实主路径开放，不宣称能力增长。
...
4. **shadow 训练（当前唯一下一步）**：先保存 shadow bare checkpoint，在 fresh restore 后只打开 shadow gate，冻结成熟 F1 trunk 与 parent readout，只给 shadow residual/projection/候选 region credit 做一次最小更新；再次保存、fresh restore、续步并验证候选权重确实变化、旧 region 前缀不被写入、失败可回滚到 bare shadow。训练前必须完成 checkpoint preflight，不能直接进入长课程。
...
R4 的最小交付仍是 `pressure/proposal → shadow materialize → shadow train → validate → lesion/admit/rollback` 的一个可复现 CPU canary 和 versioned report；当前已完成 pressure 合同、真实主路径 decision、candidate artifact 与 shadow materialization，尚未完成 shadow 训练、matched-capacity 对照或准入，不能把单次 proposal、候选存在、materialization 或单次 BPB 下降写成成功。R4 通过后才解冻 R5 自主路由，Skill/MCP/provider/客户端继续按第 6 节冻结。
