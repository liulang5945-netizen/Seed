...
17. **R2-H3.6计划目标几何与信用接口复审**：当前唯一下一包。H3.5-A否定的是“确定性signed-hash span作为可迁移plan监督”，不是持久计划层本身。下一步必须比较原生response-state teacher、可组合内容单位和预测式/对比式计划目标，明确哪种几何能让相近意图共享结构、不同答案可分，同时避免外部语义oracle；先形成版本化合同和无训练的target-geometry审计，再决定是否保留planner/bridge。不得直接加epoch、读取final或扩大容量。
...
当前唯一下一步是H3.6计划目标几何与信用接口复审。重点不是再调planner宽度，而是替换无法泛化的signed-hash监督：审查原生response-state teacher、可组合内容单位、预测式/对比式目标及其运行时oracle边界，先做无训练geometry审计再选择一个最小候选。不启动新训练、L2/Mini，不修改默认入口或shared fabric。
