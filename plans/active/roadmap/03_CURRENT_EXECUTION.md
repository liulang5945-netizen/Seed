...
当前唯一下一步为 **M1-20：phase-B 期间交错 phase-A rehearsal 诊断**：冻结当前结构、shared decoder 和所有 slot/cue plasticity 方案，只改变 memory 数据课程的时间顺序；对每个 phase-B train episode 按固定可复现 schedule 插入 phase-A replay，比较无交错、每步交错、周期性交错三种课程，并记录 old holdout/retention、new holdout、replay causal gain、no-replay counterfactual、holdout_updates、checkpoint digest 和 fresh-process round-trip。Gate 要求 old 与 new 同时不低于各自 parent/no-replay 基线；任何只恢复 old、但让 new 归零的课程直接拒绝，不进入 foundation 或 M2。
