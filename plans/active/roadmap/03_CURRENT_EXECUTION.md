...
**M1-66c（当前唯一下一步）：把 B2 的完整通过固化为正式 Foundation B2 结果，并推进首个真实记忆能力收敛验证**。具体：① 将 `eval_taiji_m1_64_foundation_memory.py` 的 `PRIOR_BASELINE` 现状确认（B1/B3/B4/B5 逐位未动，无需重建）；② 运行 `taiji/foundation_evaluation` 的 B2 入口（manifest 登记的 `b2_delayed_memory`）确认与脚本报告一致；③ 用 `holdout 0.715-0.860` 的能力量级与 M1-65 探针（寻址跨干扰保持）共同论证「延迟记忆能力成立」的完整因果链，写入 M1 段收尾；④ 之后按 roadmap M2 顺序进入 F1→F5 首轮真实训练。验证门禁=M1-66b 闭合的 22 项。
