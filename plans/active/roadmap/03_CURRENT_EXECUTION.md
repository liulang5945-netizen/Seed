...
| 6 | M3 最小真实任务验证 | **M3.R0/R1/R2/R3 已完成；下一步 M3.R4**：R0 闭合只读证据边界，R1 训练静态 observation，R2 训练跨 tick task-state transition，R3 形成 capability snapshot 绑定的只读 `ActionIntent`；下一步只做 preview/approval dry-run，不执行副作用 | R3 的 native intent 在未见项目/任务上覆盖 3/3 个应读步骤、route/policy/evidence 均 1.0、歧义澄清 1.0 且工作区不变；R4 需证明写入候选只能进入预览/审批，不会越权执行 |
...
**唯一下一步：进入 M3.R4 preview/approval dry-run。** M3.R3 已在未见项目 sequence 上证明 native transition 输出可被策略边界约束为只读 `ActionIntent`：3/3 应读步骤被 native 接受，route、Workbench policy、live evidence 一致性和歧义澄清均为 `1.0`；static-only 接受 `0`，provider/gold upper bound 为 `3`，stale snapshot 被拒绝，checkpoint/restart、transition owner lesion、CognitiveState 只读和工作区 digest 不变均通过。下一步只做一件事：把一个经过 snapshot/boundary lineage 校验的候选送入 Workbench 的 preview/approval contract，覆盖 `workspace.apply_patch` 等副作用能力的参数预览、审批令牌失效、stale file digest 和拒绝路径；只生成可审计 preview，不调用 executor、不写文件、不切编辑器语言、不执行 terminal/MCP。R4 仍不把写入能力计入 native autonomy，只有 preview/approval fail-closed 与无副作用通过后，才讨论最小人工批准执行。M3.R3 report、M3.R2/R1 report、M2.R4 与 M3.R0 均保留为基线。
...
Gate 结果为 `passed` 且 `can_promote=false`：native 在 5 步 test sequence 中接受 3/3 个应读步骤，intent coverage、route accuracy、Workbench policy allow、live evidence 一致性、澄清准确率均为 `1.0`；static-only 接受 `0`，provider/gold upper bound 接受 `3`，empty 接受 `0`。stale snapshot authorization 被拒绝，intent/planner checkpoint round-trip、transition/adapter checkpoint、transition-owner lesion、CognitiveState 只读、fixture digest 不变全部通过。R3 没有调用任何 executor；`workspace.read` 与 language resolve 只通过 side-effect-free evidence probe，未写文件、未切 editor language、未执行 terminal/MCP。下一步转入 M3.R4 preview/approval dry-run；不把 R3 计为可执行自治或开放域语言能力。

### M3.R4：preview/approval dry-run（下一步）
...
R4 的 project-disjoint fixture 覆盖正确 patch、stale file、stale capability、参数漂移、无效 patch、重复消费 approval 和空/拒绝候选。Gate 指标为 preview 合法率、审批绑定完整率、所有失效/过期路径拒绝率、preview 前后工作区 digest 相等、approval checkpoint 不持久化、旧 R3 read-only 能力保持。R4 不训练新的写入 head、不接真实用户批准、不接 terminal/MCP/editor.set_language；若 dry-run 通过，下一阶段才单独讨论最小人工批准执行。
