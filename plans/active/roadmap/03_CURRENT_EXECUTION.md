...
- **客户端插件：E5-0/E5-1/E5-2 已完成，E5-3 待推进。** `seed_platform/client_extension_host.py`、`api/routes_client_extensions.py` 和 `frontend/src/composables/useClientExtensions.js` 已接入声明式 client snapshot 两阶段发布；旧 `/api/plugins`、marketplace/upload 已统一 410 tombstone，`routes_agent_mcp.py` 与 `routes_agent_workspace.py` 的重复入口已清除。当前仍不能声称真实第三方热插拔，下一步只做 Seed-owned 本地只读 slot runtime canary。
