...
当前唯一下一步为 **M1-50：association-target component vector**：新增可序列化的六维 `memory_association_component_gains`，默认全为 `1.0`；event-pattern readout 继续使用 `memory_event_component_gains` 默认向量，只有 cue→event association target 使用独立向量。只比较默认 episode target gain `1.0` 与 candidate `0.05`，action/outcome/reward/time/provenance 保持 `1.0`，`memory_association_event_target_mix=1.0`、learning rate、decoder、topology、memory units 和 identity organ 均不变。记录两个 vector digest、完整 event 与 association target geometry、old holdout、old retention、new holdout、BWT、causal gain、new delta、checkpoint continuation、exact roundtrip、holdout side effects 和参数预算；若 candidate 三 seed 全通过才进入 M1-51 稳定性复验，否则保留双向量接口但保持两者默认一致。
