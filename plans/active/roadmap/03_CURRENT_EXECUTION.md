...
当前唯一下一步为 **M1-21：冲突感知的 replay admission/write-gate 诊断**：继续冻结 Taiji 结构、decoder、cue encoder、slot bank 和 foundation 配置，只改变 replay episode 是否允许写入及其局部写入强度；以当前 phase-A/phase-B corpus 对比无 gate、按 cue familiarity 放行、按当前 phase-B 冲突拒写三种可审计规则，并记录 old holdout/retention、new holdout、replay admitted/rejected、replay causal gain、holdout_updates、checkpoint digest 与 fresh-process round-trip。Gate 仍要求 old/new 同时不低于各自基线；任何依赖扩大参数、答案表、holdout 反馈或只恢复旧能力的结果直接拒绝。M1-21 未通过前不重跑 full foundation、不进入 M2。
