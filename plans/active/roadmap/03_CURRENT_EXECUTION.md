...
row。`model23/course0` 与 `model23/course1` 均已执行并通过，candidate/fixed-large K 均
为 `1/1`、lesion 均为 `0`，五臂 resource Gate 全部通过；当前累计 5 个 row 为
`executed_passed`，其余 4 个仍为
`not_started`，五臂 resource/side-effect/checkpoint/rollback ledger 完整。**当前唯一下一步**：
只执行 `model23/course2`，完成 model23 slice 后再推进 model31；不运行 9-cell aggregate、
不接 default runtime/provider/MCP/client/CUDA。
