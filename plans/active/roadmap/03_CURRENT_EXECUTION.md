...
当前唯一下一步为 **M1-24：写入支持与读取支持错位诊断**：继续冻结 Taiji 结构、训练数据、replay 和默认 checkpoint，只在隔离 evaluator 中比较 `shared`、`write_mask_only`、`read_mask_only`、`aligned_mask` 四种同一 decoder 路径；分别记录 phase-A/phase-B 的 action evidence、写入/读取有效 fan-in、逐 cue edge Jaccard、old/new holdout、no-write control、holdout_updates、checkpoint digest 与 fresh-process round-trip。该片只回答 cue mask 没有转化成能力收益究竟是“写入路径未隔离”还是“读取路径未对齐”，不新增参数、answer table 或新的 decoder；若四种路径仍无 old/new 双保留，则停止 mask 微调，转入显式 cue representation/capacity redesign 评审。
