...
- **客户端插件：E5-0/E5-1/E5-2/E5-3 已完成，E6-0 待推进。** `seed_platform/client_extension_host.py`、`api/routes_client_extensions.py`、`frontend/src/composables/useClientExtensions.js` 和 `frontend/src/components/ClientExtensionSlot.vue` 已接入声明式 client snapshot 两阶段发布与真实 slot projection；旧 `/api/plugins`、marketplace/upload 已统一 410 tombstone，重复入口和前端旧引用已清除。当前仍不能声称真实第三方热插拔，下一步只建立 Seed-owned MCP capability inheritance candidate 的 schema/policy/shadow 边界。
