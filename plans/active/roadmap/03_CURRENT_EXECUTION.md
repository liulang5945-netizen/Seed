...
当前唯一下一步为 **M1-54：action/outcome readout credit isolation**：在保持 factorial 可辨识课程、默认 shared decoder、association target、event gains、learning rate 和 checkpoint lineage 不变的前提下，只把 replay 的 plasticity target 拆成可审计的 `action_readout`、`outcome_readout` 与兼容的 `readout`/`all` 对照；新增 target contract、checkpoint roundtrip 和参数预算测试，但默认 `all` 行为必须字节/数值兼容。对每个 target 记录 old/new holdout、retention、action/outcome margin、association error、读出权重 delta、replay write count 和 fresh-process checkpoint，并与 no-replay counterfactual 比较。该片只允许验证信用边界，不允许启用新 target 为默认、不增加神经元、不修改 association 拓扑；只有一条 readout target 在三 seed 同时改善对应输出且不损伤另一输出/新能力，才进入后续最小隔离 candidate，否则冻结 target 拆分并回到课程/回放调度重构。
