...
当前唯一下一步为 **M1-51：action/outcome association-credit audit**：不改训练规则、配置默认值或 checkpoint，仅在 phase-A 已训练 field 上按 action、outcome、cue 分组审计 association residual 与 replay readout 的 credit 是否一致。记录每组 event target residual、association completion/error、action/outcome decoder evidence、cue collision、跨 phase 匹配、checkpoint roundtrip 和参数预算；报告必须区分“action/outcome 编码本身不可分”与“association 写入没有把 credit 传到正确 cue”两种原因。只有出现跨 seed 稳定且可复现的单一 credit 缺口，下一步才允许增加一个最小 credit 路径；否则冻结 association target 方向，转入 M1-52 的 replay interference 归因。
