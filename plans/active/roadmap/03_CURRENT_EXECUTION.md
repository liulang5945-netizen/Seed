...
当前唯一下一步为 **M1-57：protected replay objective audit**：冻结现有结构、target contract、`all@1.0`、factorial action×outcome 课程和 checkpoint lineage，不再按样本做静态 admission 过滤；先建立只使用 phase-B **train** 的“新能力保护”诊断，逐 replay write 记录 phase-B train 的 action/outcome evidence、association residual、写入前后 persistent readout delta，并比较 `no-replay`、当前 `all replay` 与 `protected-objective trace-only`。trace-only 只计算保护信号、不回滚、不改变写入，禁止读取 phase-B holdout/retention，目的是确认冲突发生在单次写入、累积写入还是目标竞争。报告必须保留 write-order digest、每步 memory digest、四组合计数、fresh-process checkpoint、参数预算、topology digest 和 `holdout_updates=0` Gate。只有证据显示冲突具有可重复的累积目标形态，才允许下一片实现一次带 rollback 的等预算 protected replay candidate；否则冻结 replay objective，转向重新设计 memory 训练数据和任务边界，不扩大模型、不重跑 full foundation、不进入 M2。
