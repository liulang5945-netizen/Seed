...
当前唯一下一步为 **M1-30：default-candidate shadow admission**：在不启用默认 gate 的前提下，将 review gain `32` 的 identity organ 放入 mixed B1/B2/B5 shadow 课程，与 shared-only 同 seed 对比语言预测、延迟记忆、持续记忆、未知 cue fallback、motor evidence provenance、参数预算和 checkpoint rollback；shadow 只产生候选报告，不替换 default Taiji。只有 mixed 课程确认 identity evidence 不污染 byte prediction、语言输出与 shared fallback，才进入 default-candidate checkpoint 评审。
