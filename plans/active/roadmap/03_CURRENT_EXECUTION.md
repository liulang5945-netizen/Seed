...
- **客户端插件：E5-0/E5-1 已完成，Legacy 表面仍未收敛。** `seed_platform/client_extension_host.py`、`api/routes_client_extensions.py` 和 `frontend/src/composables/useClientExtensions.js` 已接入声明式 client snapshot 两阶段发布；旧 `/api/plugins`、marketplace/upload 仍可能由 Legacy 路由提供，在 E5-2 完成前不得声称旧插件入口已迁移或客户端支持第三方热插拔。
