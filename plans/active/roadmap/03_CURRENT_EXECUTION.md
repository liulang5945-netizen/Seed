...
2026-09-17用户确认新增[统一认知闭环验收原则](02_GATES_AND_CI.md#21-统一认知闭环验收原则2026-09-17用户确认)：优先证明同一模型在真实任务中的记忆、世界预测与规划共同改善行为；局部机制、模块齐全和界面可用不能代替联合能力证据。语言验收保留，但不是整个认知架构的唯一出口。该决定仅更新验收标准，不启动整合支线、不解除暂停、不改变下列唯一收尾动作；后续路线若调整须另行明确范围与预算，不能自动增加并行缺口。
