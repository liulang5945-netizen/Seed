...
当前唯一下一步为 **M1-31：default-candidate checkpoint review**：基于 M1-28～M1-30 的证据生成一个可手动审阅的 identity-organ candidate checkpoint，对比 disabled default、enabled candidate、identity lesion 三者的 checkpoint schema、lineage、planned parameter budget、B1/B2/B5 结果和 rollback 行为；只允许产出 candidate artifact 与 admission report，不自动改默认 gate。评审必须明确“episodic identity candidate”与“通用语言模型 candidate”的边界，未满足真实语言质量 Gate 前不得把 identity organ 宣称为语言器官或默认能力。
