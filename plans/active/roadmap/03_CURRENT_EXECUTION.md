...
16. **R2-H3.5-A分层回答计划隔离候选**：candidate、plumbing smoke和matched训练冻结均已完成。实现与证据见[候选预注册](../../reference/M5_R2_H3_5A_RESPONSE_PLAN_PREREGISTRATION_20260916.md)、[candidate合同](../../reference/contracts/r2_h3_5a_response_plan_v1.json)、[matched预注册](../../reference/M5_R2_H3_5A_MATCHED_RUN_PREREGISTRATION_20260916.md)、[matched合同](../../reference/contracts/r2_h3_5a_matched_run_v1.json)和`reports/taiji_r2_h3_5a_response_plan_smoke_20260916.json`。v3数据为12/8/4、四policy覆盖、family/response跨split隔离，digest=`0bc5b554...abd691`；训练入口已支持response-plan预算和`--defer-final`。当前到达正式训练授权节点：下一步唯一建议是按三seed、legacy control/treatment、10 epoch、300k、final延迟合同执行dev阶段；未获授权前不运行。
...
当前到达需要明确授权的计算节点。唯一下一步建议是执行H3.5-A dev阶段六次matched运行（三seed×legacy control/treatment），每次自动先跑磁盘preflight；随后只做plan消融和dev汇总。未获得正式训练授权前停在这里，不读取final，不启动L2/Mini，不修改默认入口或shared fabric。
