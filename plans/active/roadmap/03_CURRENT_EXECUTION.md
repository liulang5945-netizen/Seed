...
当前唯一下一步为 **M1-32：foundation-first native reentry**：冻结 identity candidate 为 evaluator-only 证据，保持默认 `identity_organ_enabled=false`，回到真正的 Taiji foundation 模型性能主线。先对 disabled default 执行训练前 checkpoint save→fresh-process restore→continue preflight，再使用现有 native foundation/joint trainer 运行不带 identity organ 的 B1/B2/B5 CPU pilot；必须同时记录真实训练 cursor、checkpoint lineage、独立 holdout/retention、B1 byte prediction、B2 causal recall、B5 old/new transfer 和 CPU 资源。若 pilot 失败，只针对训练信号、数据课程或 shared memory 的可证伪问题修正，不再扩展新的 identity decoder；未得到基础能力净提升前，不进入 M2、provider、Skill/MCP 或客户端外围。
