...
**当前唯一动作：E3-3 world prediction/local synapse intake**。E3-0 已完成 parent/trial/admitted checkpoint 预检，E3-1 已完成 route/interaction credit，E3-2 已完成 procedural memory intake：成功 train experience 才能形成动态 action kind，holdout/retention 保持，失败 experience 被排除，checkpoint 可恢复。现在只接收真实 `WorldTransition` 状态—动作—结果证据，使用现有 `WorldDynamicsLearner` 的局部误差更新；digest-only 结果不得伪造为世界状态，holdout/retention 只测量，先不改结构、不接 provider。
