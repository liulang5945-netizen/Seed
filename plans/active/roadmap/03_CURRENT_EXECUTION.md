...
当前唯一下一步为 **M1-55：replay temporal scheduling audit**：冻结 M1-54 的 target contract、默认 `all@1.0`、factorial action×outcome 课程、shared decoder、association 拓扑和 checkpoint lineage，只改变 replay 时间顺序，比较 `no_replay`、`posthoc_replay`（先完整 phase-B 再回放 phase-A）与 `interleaved_replay`（按同一固定索引交替写入 phase-B/phase-A），保证总写入数、provenance、replay scale 和组合计数一致。逐组合记录 action/outcome old holdout、retention、new holdout、margin gap、association error、readout delta、写入顺序 digest，并保留 fresh-process checkpoint、参数预算、topology digest 和 `holdout_updates=0` Gate。只有某一调度在三 seed 同时改善 old/new 且不制造 action/outcome 不对称，才允许进入一个受控 schedule candidate；否则冻结 replay target 和调度方向，回到数据课程/评估设计，不扩大模型、不重跑 full foundation、不进入 M2。
