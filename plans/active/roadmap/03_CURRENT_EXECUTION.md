...
> 修订：2026-09-10；实际代码/报告基线 62f83052。本文覆盖所有旧文档中的执行许可和“下一步”。
...
本轮从 601413cd 的收束基线继续完成了 P0 等 replay validation-only 诊断、P1 失败审计、P1.1 数据契约修复、P2 小预算 validation pilot、P2.1 只读输出/行动链诊断、P2.2 安全 bridge canary 和 P2.3 recovery continuation 数据合同审计；没有读取新的 sealed payload，也没有 promotion 成绩。代码/报告证据以 62f83052、P0 报告、P1 v1/v2 报告、P2 pilot/P2.1/P2.2/P2.3 报告及本结果复审为准。
...
**P2.2 已完成，P2.3 数据合同已通过，下一步只做 P2.3 targeted learning pilot，不进入 P3。** P0 已完成，P1.1 已通过数据 Gate，P2 pilot 已真实训练但未通过能力晋级；P2.1 证明连续 MSE 下降没有转成更高离散命中；P2.2 证明 typed abstention、根目录 recovery route 和世界对齐控制的工程合同可执行，但 recovery/alignment 成绩是 oracle control；P2.3 已把“缺失目标→列举→新证据读取”拆成可重建的 train/validation continuation，初始 recovery 仍归 host policy，候选 K1/K2 才允许进入 fit。P3–P5 是后续路线，不是当前并行待办。不以添加新器官、新 Gate、更大模型或训练次数代替输出、保持和可学习闭环验收。
...
本轮已完成 P0/P1 validation-only 诊断、P1.1 修复、P2 小预算 pilot、P2.1 输出/行动链诊断、P2.2 安全 bridge canary 和 P2.3 continuation 数据合同；下一步只做 targeted learning pilot，不读取 sealed、不扩展 formal 矩阵、不进入结构成长。
...
## 唯一下一步：P2.3 targeted learning pilot

目的：使用已通过的 recovery continuation 数据合同，在不降低全局 confidence floor、且不把 host/oracle 规则计作模型能力的前提下，对同一 P2 父 checkpoint 做一次受控 K1/K2 targeted learning，验证新证据是否能转成真实模型读出和 Workbench 动作。

1. 固化 P2.2 安全出口：confidence `<0.55` 的 6 条低证据样本只能产生 typed abstention；其成功标准是 roundtrip、`action_intent is None`、快照一致和下一步类型正确，不把它们当作 goal/content miss。保留 `workspace.list(path=".")` 的根目录约束，不直接读取缺失路径。此项已通过。
2. 使用已通过的 P2.3 manifest：第一步缺失目标只保留为 host-policy abstention，第二步固定根目录列举，第三步候选观察才进入 K1/K2 fit；候选 observation digest、Percept confidence、K1/K2 input digest、project/template/path 隔离已通过。此项已通过。
3. 训练前做 checkpoint save/独立进程 restore preflight：从同一 P2 parent artifact 启动，先保存父 checkpoint 和 continuation candidate checkpoint，子进程恢复并比较 content digest；失败则停在机械 Gate，不调用 fit。
4. Gate 通过后运行一次 CPU targeted learning pilot：不扩参数、不新增 replay；K1/K2 只用 manifest 的 train candidate examples 做有限 continuation fit；validation candidate、低证据 abstention 和原 P1 v2 五类 validation 只读评分，保持全局 `0.55` floor。
5. 固定验收出口：报告模型读出、K1→K2 级联、planner admission、Workbench 成功、abstention 安全率、recovery continuation 命中、原五类逐类保持、checkpoint digest 和资源。host recovery route 只作工程前置，不计入模型命中；只有真实候选读出/执行和旧类保持同时成立，才允许设计 P2.4/P3；否则按失败层修数据或学习目标。
...
| P2 | 五类学习及保持的独立验证 | **P2.3 数据合同已通过、模型仍未晋级**：6 train + 2 validation continuation 通过，下一步运行 targeted learning 并同时回归原五类保持 |
...
- P2 pilot 已执行上述最小预算和独立 checkpoint preflight；P2.1 完成了 K1→K2 级联与隔离 Workbench 诊断；P2.2 完成了 typed abstention、根目录 recovery 和世界对齐工程 canary；P2.3 continuation 数据合同已通过。当前模型结论仍仅为连续 MSE 拟合改善；高证据可执行 cohort 为 4/10，6/10 行应安全 abstain，R 的真实“列举后重新观察并读取”尚未学习，因此下一步只运行 P2.3 targeted learning，不进入 P3。
...
本轮 P2 pilot、P2.1、P2.2、P2.3 contract 报告与计划同步已提交本地 main。后续唯一入口是 P2.3 targeted learning pilot；不跳到 P3/lineage 实现，也不按历史“下一步”自动开跑。
