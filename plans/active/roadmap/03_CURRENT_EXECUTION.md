...
>
> M4.R9 foundation course audit 已完成：foundation C→C2→C3 的 record-disjoint、digest、预算和 byte 统计 Gate 全通过；C3 相对 C2 的最大 train/holdout JS 比约 1.19×，不是明显课程边界 outlier。R7 的 C3 退化不能归因于数据断裂，下一步只审计单一 update/consolidation rule。
>
...
| M4.R9 foundation course audit（已完成，课程非明显 outlier） | 只读重建 foundation profile 的 C→C2→C3；记录 train/holdout bytes、entropy、unique-byte、JS、digest、selected record 与 overlap，并对齐 R7 frozen difficulty/退化 seed；报告 `reports/taiji_m4r9_foundation_course_audit_20260908.json` | 三 phase digest distinct、record overlap=0、预算满足；C3/C2 最大 boundary JS 比约 `1.19×`，C3 不是明显分布 outlier；seed11/47 仍 C cycle3 退化，转单一 update/consolidation rule audit，不训练 |
...
### M4.R9：foundation 课程边界审计（已完成，2026-09-08）
...
- **数据证据**：报告 `reports/taiji_m4r9_foundation_course_audit_20260908.json` 记录每一 phase 的 train/holdout bytes、unique-byte ratio、entropy、top-byte probability、train↔holdout JS、相邻 phase JS、selected record count、dataset digest 和 lineage overlap；全部 technical Gate 通过。
