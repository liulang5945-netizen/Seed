...
当前唯一下一步为 **M1-42：event-encoding and association-geometry audit**：不改训练规则、不改 checkpoint、不引入新 decoder，只在同一最大分离 cue、重叠 action、shared、`memory_units=128` 配置下审计 phase-A 写入后的 cue pattern、action/outcome event drive、完整 event pattern 与 association completion 的几何关系。每个 seed 必须记录 cue/event active support、cue↔event cosine、action/outcome 对 event 的能量占比、association completion/error ratio、跨 phase pattern cosine、readout topology capture 和参数预算；报告只回答绑定失败发生在输入编码、association 形成还是 action readout 覆盖，不把相关性当作能力 Gate。若出现稳定且可复现的几何瓶颈，下一步只修该瓶颈；否则冻结编码/association 几何解释，进入 M1-43 的最小读写路径证据。
