...
**唯一下一步：M2.R0——修复 active/protected 评分口径，并版本化能力达标/保持/增量判定。**

从“同一模型的两个明显不同 readout 必须得到不同评分”这一失败用例开始，完成数据重复反例、checkpoint/owner 清单和新报告语义。成功后按 R1 执行真实续训。

本次用户要求“先详细修订，后续自行推进”，因此此处是本轮交付终点。计划中的命令和实现均由后续开发按本顺序执行。
