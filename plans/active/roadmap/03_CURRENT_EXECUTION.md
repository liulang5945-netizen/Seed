...
当前唯一下一步为 **M1-48：association-rate independent-seed stability review**：保持 M1-47 的全部变量和 Gate，改用未参与首轮的独立 seed cohort `73/101/131`，重复 default `0.60` 与 candidate `1.20` 的 phase-A → phase-B → replay 评估。报告必须绑定同一 corpus/component vector digest，记录三 seed 的 old holdout、old retention、new holdout、BWT、causal gain、new delta、association completion/error、checkpoint continuation、exact roundtrip、holdout side effects 和参数预算。只有独立 cohort 也全通过，才进入 M1-49 的 candidate checkpoint review，并考虑把 `1.20` 纳入训练 profile；任一失败都保持默认 `0.60`，不把首轮结果推广成架构结论。
