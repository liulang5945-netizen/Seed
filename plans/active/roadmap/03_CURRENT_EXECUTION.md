...
当前唯一下一步为 **M1-52：replay interference per-event attribution**：保持默认 `all@1.0`、shared decoder、`episodic_learning_rate=0.60`、固定 cue/action/event 课程和现有 checkpoint lineage，不改核心结构；在同一 phase-A/phase-B/replay corpus 上逐 episode 追踪 replay 前后每条 phase-A 记忆的 association completion、action/outcome readout、old/new holdout 变化及其与 replay episode 的 cue/event 相似度，建立 `no-replay`、`replay`、`repeated-replay`、`no-change` 四组可对照的 interference attribution。报告必须区分“被同一 cue 覆盖”“共享 readout 被其他 cue 改写”“association residual 未闭合”“评估/课程不可辨识”四类原因，并保留 checkpoint save→fresh-process load→eval-only、holdout updates、参数预算和 topology digest Gate。只有归因锁定为单一且跨 seed 稳定的物理干扰源，才允许设计一个最小隔离改动；否则冻结 memory 写入/target/decoder 方向，转向训练数据课程重构，不重跑 full foundation、不进入 M2。
