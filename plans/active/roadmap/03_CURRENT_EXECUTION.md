...
当前唯一下一步为 **M1-37：replay provenance event-representation diagnosis**：固定最大分离 cue、重叠 action、`memory_units=128`、shared decoder 和 replay target `all@1.0`，只比较 replay 写回时保留原始 `experienced` provenance 与当前 `replayed` provenance 两种事件表示。记录 old holdout、old retention、new holdout、causal gain、provenance readout、checkpoint continuation 和 holdout side effects；若保留原 provenance 能通过 B5，才将其作为训练规则候选，否则冻结 provenance 解释并转向 cue/action 事件绑定的可学习数据课程。
