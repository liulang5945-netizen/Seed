...
- **客户端插件：尚未实现。** 当前 Vue SPA 没有 extension host，API 插件/MCP 市场仍指向未实现的 Legacy manager；在 E5 前不得声称客户端支持热插拔。
