...
> 更新：2026-09-16；R0证据与门禁审计已完成。本次决定将R2整模型语言能力设为当前主线；结构化R2入口、checkpoint前置、P1 developmental 对照、P2序列级只读评价、G1条件接口、H2/H3内部表示/读出审计、H3.1序列路径对照、H3.2 response-start候选读出、H3.3泛化控制与response-phase候选、H3.4逐位置条件信用审计均已完成，本地未提交观察与正式结果继续分账。
...
16. **R2-H3.5-A分层回答计划隔离候选**：candidate与plumbing smoke已完成，见[预注册](../../reference/M5_R2_H3_5A_RESPONSE_PLAN_PREREGISTRATION_20260916.md)、[可机读合同](../../reference/contracts/r2_h3_5a_response_plan_v1.json)和`reports/taiji_r2_h3_5a_response_plan_smoke_20260916.json`。实现包含32维持久plan、训练期确定性response span监督、plan-conditioned byte renderer、owner互斥、checkpoint/identity lineage恢复、candidate清除消融和预分配预算。磁盘preflight的zero/fresh restore/一次更新/atomic child save/child restore/caller保护全部通过；effective=276,610、candidate=13,771≤300k。当前唯一下一包是冻结v3控制数据和matched control/treatment命令，不执行正式训练。只有后续同预算treatment在dev出现plan margin与未见continuation方向一致改善、final无collision/UTF-8/boundary退化且plan消融撤销收益，才允许继续；否则按目标编码、接口信用或结构机制层结项，不以更多epoch掩盖。
...
H3.5-A最小candidate与plumbing smoke已经完成。32维plan、训练期span监督、plan-conditioned renderer、保存恢复、identity lineage、candidate清除消融和预算预报均已落地；磁盘preflight全部通过，effective=276,610、candidate=13,771≤300k。当前唯一下一步是冻结v3控制数据和matched control/treatment命令：必须覆盖共享首codepoint/不同内容、不同表面/同意图、四类policy、history改写与context置换，并固定seed、更新数、final读取权限和plan消融指标；完成前不执行正式训练。H3.5-A仍只验证“持久回答计划”机制，不启动正式pilot、L2或Mini，不修改共享fabric、默认`SeedRuntime.chat`入口、P3b历史文件、Mini验收合同或P5.2d未提交实验文件。
