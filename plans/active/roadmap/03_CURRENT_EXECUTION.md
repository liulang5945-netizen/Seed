...
| 2 | M2.R1～R3 证据重建 | **当前进行**；seed11 与 seed29 三个正式臂均完成，seed47 active-only 完成且为正，下一步执行 seed47 replay/cascade | 技术合格报告 + 独立 C/C′ 能力结果，不要求实验一定成功 |
...
**唯一下一步：按相同 1 MiB / 128 KiB / record-disjoint v2 协议执行 seed47 replay。** 三个 seed 的 active-only 已均为正（seed11 `+0.02444`、seed29 `+0.05344`、seed47 `+0.07520 BPB`），但两 seed 的 replay 已为负；seed47 replay 用于完成第三个成对对照，随后运行 seed47 cascade，再汇总三 seed 证据。正式晋级或架构扩展仍保持冻结。64 KiB pilot 的负增益保留为预算/泛化诊断，不自动推翻或确认正式结果。
