...
**M2 启动（F1→F5 首轮真实训练）**：按 roadmap M2 顺序正式进入「世界—行动—语言后训练」的第一轮真实训练，在 M1 joint checkpoint 上继续，而不是重新初始化。开工前按训练前硬性规则完成两部分 checkpoint 验证（均 `passed` 并固化正式报告）：① 训练 checkpoint 可写/可恢复/防篡改门禁 `reports/taiji_m2_e3_0_training_checkpoint_20260903.json`（parent→trial→admitted 三代 lineage、native 模型往返、tamper/drift 拒绝）；② manifest 要求的正式 checkpoint gate（fresh-process 恢复续走一步）`reports/taiji_m2_checkpoint_preflight_20260903.json`，`gate=taiji-m0-checkpoint-preflight` 与 `plans/manifests/taiji_foundation_baseline_v1.json` 的 `checkpoint_gate` 契约一致，供 M2 结束时随正式 foundation 报告闭合 `checkpoint_gate_status=passed`。随后先做 pilot 规模管线穿透（`scripts/training/train_taiji_joint.py --profile pilot --seed 11`，seed 11）：F1 sequence（holdout BPB `8.006 → 5.653`）、F2 memory（recall `0.5 → 1.0`）、F3 world（error `2.058 → 1.4e-05`）、F4 goal（success `0.5 → 1.0`），`joint_holdout_gain=5.24`，报告 `reports/taiji_m2_pipe_check_seed11_20260903.json`；并用 `--resume .../last.pt --eval-only` 验证 checkpoint 恢复后指标与训练结束时逐位一致（`reports/taiji_m2_pipe_check_seed11_resume_20260903.json`），证明「能保存、能恢复、能继续」。pilot 穿透证明当前代码（含 M1-65 cue 快照寻址、M1-66b per-slot value router）下 joint 训练管线可端到端跑通。配置纪律：foundation 规模下单次全量表测量约 5-8 分钟（sequence holdout/retention 各 131072 字节 score_bytes + 400 条 memory query + world/goal），`--metric-interval` 必须与 `checkpoint_interval` 对齐（256）而非过度评估，否则评估成本压垮训练。**首轮真实训练已启动（后台，三 seed `[11,29,47]` 并行，`--profile foundation`、`--partition-seed` 同 seed、`--checkpoint-interval 256`、默认 `metric_interval=256`，输出 `outputs/taiji-m2-f1f5-foundation-seed{11,29,47}/`，报告 `reports/taiji_m2_f1f5_foundation_seed{11,29,47}_20260903.json`）**：F1 语料 `data/simple_zh/dialogue_extended_clean.jsonl`（foundation 预算 1,048,576/131,072/131,072 字节），F2 1000 条记忆课程，F3 1000 条 world transition（`world_learning_rate=0.02`、`repeats=8`、`register_parameters=True`），F4 1000 条 goal action，child checkpoint 落 `last.pt/best-holdout.pt` 并在最终 report 输出 `child_checkpoint_digest` 与 parent/final 指标。完成判据（本轮固定，不后移）：child checkpoint 在 B1（holdout BPB < 父 checkpoint/frozen_parent）/B2（三 seed beats 两消融）/B3/B4（不退化）上出现可归因净提升，且 `holdout_updates=0`；三 seed 完成后用 foundation evaluator（含 `--b1-corpus` 与新增 wiring 的 B2/B3/B4 真实测量）闭合正式 foundation 报告，`checkpoint_gate_status=passed`。
