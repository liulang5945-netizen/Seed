...
> 修订：2026-09-10；实际代码/报告基线 4798e4c3。本文覆盖所有旧文档中的执行许可和“下一步”。
...
| S/G/K 连续整合 | P3.1 已通过事件所有权、mask、独立恢复和 rollback；S/G 仍 control-only，现有 K1 仍捆绑语义/目标/内容读出 | 进入 P3.2 owner 边界迁移预检；禁止把 control-only 或 lineage 元数据当作 learned 能力 |
...
**P2.2 已完成，P2.3 数据合同/targeted learning、P2.4 retention、P2.5 novelty probe、P2.6 novel learning、P2.7 holdout generalization、P3.0 checkpoint/interrupt-resume contract 和 P3.1 single-cell preflight 已完成但模型仍未 promotion，下一步只做 P3.2 S/K/G owner 边界迁移预检。** P0 已完成，P1.1 已通过数据 Gate，P2 pilot 已真实训练但未通过能力晋级；P2.1 证明连续 MSE 下降没有转成更高离散命中；P2.2 证明 typed abstention、根目录 recovery route 和世界对齐控制的工程合同可执行，但 recovery/alignment 成绩是 oracle control；P2.3 candidate-only fit 暴露了保持问题，P2.4 已用固定 rehearsal 保住旧类；P2.5 定位了新组合 K2 content 缺口，P2.6 学到该目标，P2.7 在 2 个全新 project、4 条全新 path 上达到 K2 content/Workbench `4/4`，局部泛化 Gate 通过；P3.0 证明 K1/K2 continuation 的 checkpoint 状态边界可恢复；P3.1 证明 S/G/K 事件合同可恢复，但 S/G 仍 control-only，且 K1 的目标/内容读出尚未被真实 G owner 接管。P3–P5 是后续路线，不是当前并行待办。不以添加新器官、新 Gate、更大模型或训练次数代替输出、保持和可学习闭环验收。
...
- P2/P3.1 仍只允许使用冻结的 manifest、candidate/scorer/threshold/resource contract；每次训练前必须重新通过 checkpoint 保存/独立恢复 preflight。P3.2 不得绕过 P3.0 parent/P3.1 manifest，不得把外部 goal target 或 K1 已有输出冒充 learned G owner。
...
本轮已完成 P0/P1 validation-only 诊断、P1.1 修复、P2 小预算 pilot、P2.1 输出/行动链诊断、P2.2 安全 bridge canary、P2.3 continuation 数据合同/targeted learning、P2.4 retention canary、P2.5 novel-composition probe、P2.6 novel K2 learning、P2.7 independent holdout generalization、P3.0 checkpoint/interrupt-resume contract 和 P3.1 single-cell preflight；下一步只做 P3.2 S/K/G owner 边界迁移预检，不读取 sealed、不扩参、不进入结构成长或 promotion。
...
## 唯一下一步：P3.2 S/K/G owner 边界迁移预检

目的：站在已有 K1/K2 能力之上，把当前 K1 中捆绑的“语义证据、目标选择、内容选择”拆成可审计的 S/K/G owner，而不是从零训练一个重复的 G。P3.2 先做 owner-transfer 和 K-only 对照，不立即扩参、不读取 sealed、不把外部标签当模型输出。

1. 以 P3.0 parent、P3.1 manifest 和 P3.1 事件格式为唯一输入；定义 `S evidence`、`K semantic/transition`、`G goal/content selection` 的 provenance、读写 mask、checkpoint digest 和 rollback parent。K1 的共享表征可以继承，但 goal/content head 的归属必须显式标为“待迁移”，不能同时记在 K 和 G。
2. 复用现有 K1 输出能力实现最小 G owner：先建立 `GSelectionState`（候选 goal/content、选择置信度、来源、被拒绝候选、目标 digest），由 G 负责选择/拒绝，K 只负责提供语义/world evidence；外部 goal/content target 只能作为 validation label，不能作为 learned arm 的运行输入。
3. 设计 K-only 与 S/K/G owner-transfer 两个完全相同输入的对照：同一 P2.7 holdout、同一 K1/K2 parent、同一 confidence floor 和 Workbench policy；记录 K1/K2 goal/content、safe abstention、Workbench、输入延迟和 checkpoint 字节。没有 owner-transfer 的新增行为时，结论必须是等价而不是“联合能力提升”。
4. 实现 owner-transfer checkpoint：保存 S、K、G 三类 state 与原 K1/K2 worker 的 attached lineage；在 observation、G selection、K readout、action 四个边界各做独立进程恢复，并验证事件、owner digest、worker digest、budget、RNG、最终 logical digest 与 uninterrupted 一致；篡改 G state、错 parent、错 owner mask 必须 fail-closed。
5. 只要 owner-transfer 的 K 输出、safe abstention 或 Workbench 低于 K-only，停止在边界修复；不得用额外 epoch 或放宽阈值掩盖退化。只有对照不劣且 G 的运行输入不再依赖外部 target，才拟定 P3.3 的 G 小步学习，且训练前重新通过 checkpoint 保存/独立恢复 Gate。
...
| P3 | 中断续训与 S/G/K 联合状态整合 | **P3.1 已通过合同 Gate**：S/G/K 事件、mask、独立恢复和 rollback 可复现；当前执行 P3.2 owner 边界迁移预检，S/G 仍非 learned |
...
- P2 pilot 已执行上述最小预算和独立 checkpoint preflight；P2.1 完成了 K1→K2 级联与隔离 Workbench 诊断；P2.2 完成了 typed abstention、根目录 recovery 和世界对齐工程 canary；P2.3 continuation 数据合同/targeted learning、P2.4 retention canary、P2.5 novel-composition probe、P2.6 novel K2 learning、P2.7 holdout generalization、P3.0 checkpoint/interrupt-resume contract 和 P3.1 single-cell preflight 均已完成。当前模型结论从“只有连续 MSE 改善”推进为“一个具体 K2 content 目标在 rehearsal 保持约束下学习，并跨 2 个新 project/4 条新 path 泛化”；P3.0 已把 K1/K2 continuation 纳入可恢复状态边界，P3.1 已把 S/G/K 接线合同闭合但仍标记 S/G control-only，下一步只做 P3.2 owner 边界迁移预检。

### P3：状态整合与同一父代连续课程（P3.1 合同 Gate 已通过，P3.2 当前）

P3.0 已用 [checkpoint/interrupt-resume contract](../../../reports/taiji_m5_k_p3_0_checkpoint_contract_20260910.json) 替代“先做 lineage 再补能力”的顺序：K1/K2 parent、worker、phase、replay stream、RNG、预算和 lineage 元数据已经可以内容寻址、独立恢复和回滚。P3.1 又把 S/G/K 的事件、owner mask、状态 digest 和恢复边界闭合，但没有把 control-only 的 S/G 说成 learned。当前 P3.2 必须沿用已有 K1/K2 能力做 owner-transfer，不能回到原始从零训练，也不能重新授权暂停的 [SGK v1](../../reference/M4V2_SGK_PROMOTION_COURSE_PREREGISTRATION_20260910.md)。
...
4. 在 observation、G selection、K readout、action 四个边界做独立进程恢复；验证事件/owner/worker/budget/RNG/logical digest 一致，篡改 G state、错误 parent、错误 mask 必须拒绝。若对照不劣且 G 不再读取外部 target，才解冻 P3.3 的小步 G 学习。
...
本轮 P2 pilot、P2.1、P2.2、P2.3 contract/targeted、P2.4 retention、P2.5 novel-composition、P2.6 novel-learning、P2.7 generalization、P3.0 checkpoint/interrupt-resume contract 和 P3.1 single-cell preflight 报告与计划同步已提交本地 main。后续唯一入口是 P3.2 S/K/G owner 边界迁移预检；不跳到结构成长、promotion 或外围路线，也不按历史“下一步”自动开跑。
