...
当前唯一下一步为 **M1-53：独立 action/outcome credit curriculum audit**：不改 Taiji 核心、默认配置或 decoder，新增诊断课程把 action `{48,49}` 与 outcome `{43,45}` 做均衡 factorial 组合（四种组合各自出现），同时保持 phase-A/phase-B cue 完全不重叠、样本量和 replay 预算可比；为 action 与 outcome 分别建立 holdout/retention accuracy、margin、association residual 与 replay 前后变化的独立报告，补充当前 paired course 的对照。必须确认四种 action×outcome 组合都被真实写入、没有 evaluator 预置答案、训练/holdout digest 分离、checkpoint save→fresh-process load→eval-only、参数预算和 `holdout_updates=0` 全部通过。只有独立课程能把 action 与 outcome 的 credit 变化分离，才允许进入一个最小 readout/credit 隔离实验；否则冻结 memory 结构，先修正数据课程和 evaluator，不扩大模型、不重跑 full foundation、不进入 M2。
