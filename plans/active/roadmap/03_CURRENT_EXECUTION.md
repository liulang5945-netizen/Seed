...
当前唯一下一步为 **M1-33：shared-memory capacity scaling diagnosis**：在不改变学习规则、不接入 identity organ、不引入新 decoder 的前提下，只比较 shared memory 的 `memory_units` 容量档位，统一跑 phase-A→phase-B→replay 的 B5 old holdout、old retention、new holdout、replay causal gain、checkpoint round-trip、planned parameter budget 和 CPU 时间。若扩大容量能同时恢复 old/new，说明当前瓶颈主要是可增长容量；若容量扩大仍失败，则停止盲目加宽，下一片只针对 cue/action training signal 或数据课程做单变量诊断。
