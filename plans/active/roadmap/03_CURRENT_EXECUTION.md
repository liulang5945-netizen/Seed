...
当前唯一下一步为 **M1-22：phase-B 写入干扰隔离诊断**：冻结结构、shared decoder、cue encoder、slot bank、replay 与 foundation 配置，暂时关闭 replay，只对 phase-B 真实写入比较 `all`、`association`、`readout` 三种原生 learning target 及受控的局部写入强度；在 phase-A parent、phase-B new holdout、old holdout/retention、no-write control、checkpoint digest 和 fresh-process round-trip 上做三 seed 对照。该片只回答“哪一类突触写入造成 old/new 冲突”，不把任何单一 target 直接接入主路径；必须同时保住 old 与 new 且不依赖 holdout 反馈、答案表或扩大参数，才允许进入 foundation replay 评审。M1-22 未通过前不重跑 full foundation、不进入 M2。
