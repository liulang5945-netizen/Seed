...
**当前唯一动作：E4 Skill/MCP 知识与程序内化 Gate**。E3-0 已完成 parent/trial/admitted checkpoint 预检，E3-1 已完成 route/interaction credit，E3-2 已完成 procedural memory intake，E3-3 已完成真实 `WorldTransition` 的 world prediction/local update，E3-4 已完成多 seed fixed-capacity 对照、容量压力测量和持续失败触发前置：32 hidden units / 775 parameters 下，native holdout error 均值为 0.2785，跨 seed 标准差 0.0216，retention regression 为 0，容量压力虽为 1.0，但固定容量未持续失败，因此结构候选触发保持 false。下一步进入 E4，先让受治理 Skill/MCP artifact 知识进入 Taiji-owned semantic/procedural/affordance 内化 Gate；不训练 Qwen、不接 Legacy manager、不把客户端执行器写入模型。
