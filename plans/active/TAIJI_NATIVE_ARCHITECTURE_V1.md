...
状态已滚动更新：上段记录的 producer/feature-source/delayed-credit lesion 已完成并通过 3-seed Gate；当前四步延伸、3/4/5 步变量 episode、不同失败位置和 after-state relation 变化，以及 executive-to-world prediction train/holdout 与 no-online-update calibration control 也已通过；下一步是把 calibration 接入真实 runtime 的持续环境回写和可恢复 trace，避免评估脚本与运行时脱节。
