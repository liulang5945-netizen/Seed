...
> `reports/taiji_runtime_structure_20260826.json`。下一入口是为 candidate cycle 增加显式依赖/冲突
> 判定，对竞争变更、父子拓扑顺序和失效候选逐项 fail-closed。
