...
状态已滚动更新：上段记录的 producer/feature-source/delayed-credit lesion 已完成并通过 3-seed Gate；当前唯一下一步是把 grounded multi-step 从两步扩展到至少四步，允许连续失败重规划和跨步 delayed credit，验证 lineage 与信用链在更长 horizon 仍不丢失。
