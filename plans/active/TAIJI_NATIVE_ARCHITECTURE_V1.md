...
状态已滚动更新：上段记录的 producer/feature-source/delayed-credit lesion 已完成并通过 3-seed Gate；当前四步延伸也已通过，前三步连续失败重规划、跨步 delayed credit 和全程 lineage 均保持；下一步是继续扩大 horizon 与 episode 变体，避免能力只依赖固定四步序列。
