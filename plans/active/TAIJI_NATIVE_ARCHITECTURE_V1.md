...
状态已滚动更新：上段记录的 producer/feature-source/delayed-credit lesion 已完成并通过 3-seed Gate；当前四步延伸及 3/4/5 步变量 episode、不同失败位置和 after-state relation 变化也已通过；下一步是把变量 horizon 接入真实 world-dynamics prediction/error calibration，避免能力只依赖固定 episode 序列。
