...
**2026-09-06 设计复审补充（尚未改变代码合同）：** 上段是 2026-08-26 的实现收口事实，不是永久禁止成熟训练工具的能力定理。手写梯度/LocalAdam 与 autograd/optimizer 可以数学等价；原生身份取决于 Taiji 对参数、状态与决策的所有权。当前 AST 禁令仍按现有代码执行，后续 M2.R2 允许在隔离对照中比较成熟发展训练与当前规则；若证据支持正式迁移，先评审跨时间信用、checkpoint 兼容、在线可塑状态和 native-only 运行边界，再同步修改合同与测试。不能静默删除禁令，也不能为保持“零 autograd”而拒绝有证据支持的能力改进。
