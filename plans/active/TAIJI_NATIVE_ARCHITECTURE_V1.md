...
状态：**核心需求之下的目标架构与能力/工程合同；不是全部已实现的声明**

决策日期：2026-08-25

规则与统一能力设计修订：2026-09-17。本文新增 §0.2、§4.6–4.7、§6.3–6.4、§8.5，解决规则分层、消费接口、学习责任和失败处理缺口；设计合同已明确，相关统一运行路径仍待实施验证。旧实验记录不发布当前动作，执行状态只看 [03](roadmap/03_CURRENT_EXECUTION.md)。
...
下列 P1/P2 为早期局部实现证据，保留其范围，不是当前待执行阶段；最新结果边界见[结果总览](../reference/PROJECT_RESULTS_AND_CONCLUSIONS_20260917.md)。
...
规则的目标是保护有效研究和高上限设计，不是把一次实现永久固定。四层分别管理：

| 层级 | 约束什么 | 例子 | 修改与实施边界 |
|---|---|---|---|
| P：不可妥协边界 | 主体身份、权限、证据诚信 | Taiji 拥有认知；不暗用答案；不越权执行；不伪造结果 | 不可由实验例外绕过；目标改变须项目所有者明确决策，且不解除安全限制 |
| C：能力合同 | 声称某种能力必须证明什么 | 联合行为贡献、保持、连续性、自主触发、结构成长 | 按主张分级；仅约束该能力声明及采用，不要求每个原型实现全部 CR |
| E：工程合同 | 当前接口、保存恢复、隔离、失败处理 | schema、lineage、幂等、checkpoint、兼容迁移 | 可版本化替换；先列影响与回退，代码/测试/文档同步后才能采用 |
| X：实验选择 | 某次实验用什么方法和预算 | residual 出生、槽宽、梯度算法、冻结前缀、种子/阈值 | 探索可替换并留痕；正式协议冻结后不得按结果改门；新方法另立协议 |

按“层级＋声明范围＋协议版本”解决冲突，不按文件日期或更苛刻措辞机械裁决。根需求说明目的，本文定义对象与合同，[02](roadmap/02_GATES_AND_CI.md)定义证据与批准，[03](roadmap/03_CURRENT_EXECUTION.md)只发布执行顺序；VISION 和历史报告均不能自动覆盖它们。具体冻结协议裁决其原实验，不因新一般原则而补发成功。

规则状态分为 `design-only`（仅设计）、`implemented`（已有实现入口）、`verified-scoped`（指定版本/范围已有证据）、`adopted`（指定产品行为已批准）；不能从文档存在直接跳到后两项。负责实现的人提交证据，正式能力/默认采用由项目所有者批准；原范围内修复和相称回归不必为每个字段另开决策。

例外记录必须说明原规则、原因、范围、风险、替代控制、有效版本/预算、退出条件与批准者。P 层没有普通实验豁免；E/X 层例外不自动授予 C 层能力声明。到期或范围改变须复审，不能以“试验中”无限延期。
...
因此，后续不再用“已经有一个同名模块”作为完成标准。隔离原型可先用十字段的最小说明和相关工程检查验证假设；进入主运行时或提出能力主张前，才要求相应版本化接口、迁移和因果证据齐备。不能要求先取得集成后的能力证据，才允许做隔离集成本身。

### 4.2 Taiji 核心对象术语表 v1

以下定义是当前实现和后续目标之间的翻译层。当前代码中尚未出现同名字段的对象，先作为架构语义存在。跨模块/持久化边界必须有版本化语义；内部 list/dict/dataclass 均可使用，禁止的是把无来源、无恢复语义的隐藏状态当正式合同，不是禁止某种容器。
...
### 4.6 统一能力链：从相邻模块变成共同任务状态

**设计合同，尚无整链通过声明。** 核心问题分成三个可验证缺口：模块是否交换同一语义、下游是否真正使用它、任务误差是否归到能够改进行为的参数/状态。增加调用或损失项只解决接线，不自动解决后两项。

采用 Taiji-owned 的任务信念与工作空间作为共同消费上下文，保留多时间尺度模块，不要求所有状态塞进一个无类型向量。目标链为：

```text
观察＋目标/约束 → 相关记忆 → 更新任务信念与工作空间
 → 候选行动/轨迹 → 世界后果预测 → 规划比较 → ActionIntent
 → 权限检查与执行 → 核验实际终态 → 信念/记忆更新 → 下一次选择
                                         └→ 经准入的学习信用
```

此图规定信息责任，不强制每个简单请求调用全部机制。无需规划的任务允许早停；但声称联合能力的验收任务必须真的需要三类机制，不得由脚本强制调用制造表面闭环。

下面是跨模块**逻辑字段**，不是声称当前 dataclass 已包含这些字段。实现时先映射现有 `CognitiveState`、`ActionIntent`、`WorldTransition` 等，只有缺失语义才新增版本：

| 信息/责任方 | 生产与消费合同 | 失效或禁止用途 |
|---|---|---|
| 观察与世界信念：感知/世界模块 | 标记 observed/inferred、来源、对象身份假设、时间及 belief revision；记忆与规划消费，外部事实与模型信念分开 | 冲突观察不能静默覆盖；过期对象/关系使依赖计划重新检查 |
| 检索证据：记忆模块 | 返回内容或稳定引用、经历/来源、发生时间、有效条件、不确定性；世界更新和候选生成记录实际消费项 | 未命中不是伪造空事实；失效记录不作当前事实；检索相似度不冒充正确概率 |
| 共同上下文：工作空间 | 绑定 goal revision、belief revision、相关记忆、对象关系和资源；下游可查看具体消费来源 | 维度相同不代表语义相同；表示 encoder/schema 改变须重建或迁移旧缓存 |
| 后果预测：世界模型 | 对 state/action/horizon 输出 predicted 或 imagined 后状态、风险/成本、不确定性及模型版本；供规划比较 | 不读取真实未来/参考计划；预测不能写成 experienced；分布外信心不能默认置零风险 |
| 候选与决策：执行认知 | 从当时可见信息提出多个合法候选，记录评价、未决假设、选中原因和信息预算；发出 ActionIntent | binder 只能合法绑定参数，不能利用 evaluator 隐藏标签挑选正确路线；目标/能力变化使旧决策失效 |
| 执行与结果：器官/Seed 执行设施 | 接受已授权意图，返回执行事实、receipt、错误与可核验后状态；Taiji 决定是否重规划 | 工具的 success 字段不单独证明目标达成；宿主不得偷偷修正计划后记为模型成功 |
| 反馈/信用：Taiji 学习所有者 | 用 episode、intent、执行尝试、前后状态与参数版本绑定证据；记录哪些更新获准、应用或拒绝 | imagined、未知结果、重复事件及未授权反馈不得混为真实奖励；没有学习也应能完成状态闭环 |

共同信封至少可追溯 schema、episode/goal/intent、生产者、来源种类、时间/状态修订号、表示版本和资源消耗；未适用字段明确标记，不伪造数值。记录引用必须随 bundle 可解析或有完整内容快照，不能只保存失效的内存地址。

**设计选择与上限**：只给聊天接几个调用能快速出轨迹，但不足以形成共同表示和信用；全面推倒重建会丢掉已有边界资产且扩大同时变化量。本轮采用第三种：复用合同/恢复/执行资产，围绕上述共同任务状态建立一条可训练、可损伤、可恢复的隔离纵切片，再以证据决定内部表征和信用算法迁移。共享不等于强制共享所有权重，局部辅助目标不替代任务终态目标。

当前可复用点与断点：

- `taiji/adapter.py` 的 `predict_world_candidates`、`plan_world_actions`、`imagine_world_rollout` 已有预测→选择接口，不能描述成完全没有集成。
- 同文件默认 workspace 路径仍可能直接广播感知特征；需证明相关记忆/信念实际进入候选及选择，不能以字段存在代替。
- `api/seed_runtime.py` 的 `chat` 主要沿 `generate_input` 与语言表层生成；其存在不证明使用上述任务链。`plan_task_sequence` 提供另一条规划接口，需核对同模型身份与 binder 责任，不能拼接成绩。
- 语言应消费 Taiji 的意图、可引用事实、结果与不确定性；执行回执模板可作 UI，但不能算原生语言能力。自然语言解析/生成的学习责任仍属于 Taiji，结构化任务入口只能先隔离验证认知链。

### 4.7 外部行动生命周期与故障责任

内部原子提交不等于外部世界原子事务。目标生命周期如下，具体映射仍须逐入口实现并测试：

| 状态 | 允许的后继 | 必须留下什么 |
|---|---|---|
| proposed / rejected | 经权限、前置和预算检查进入 prepared；拒绝则返回原因 | intent、goal/belief revision、能力/权限版本；拒绝不是执行失败样本 |
| prepared | 持久化执行意图后进入 dispatched；尚未发送可取消 | 幂等键、参数、预期条件、执行器与恢复策略 |
| dispatched | 收到可验证回执进入 observed；超时/断连进入 unknown | attempt/receipt 关联；unknown 不自动视为失败或重发 |
| unknown | 查询外部状态或人工对账后进入 observed / unresolved | 能力支持查询/幂等时才安全重试；不支持则停止该动作而非重复产生副作用 |
| observed | 独立终态核验后 settled | 实际结果与预测差异；每事件仅结算一次，重复为已结算或明确拒绝 |
| settled | 按准入策略写记忆/更新；必要时另发 compensation | 环境补偿是新行动；模型 rollback 不抹去已发生的世界事实 |
| compensation failed / unresolved | 停止依赖该前提的后续动作，报告所需处理 | 真实残留影响，不能伪装恢复成功 |

风险相称地失败处理：缺权限、模型身份不明、错误 schema、血缘不符时拒绝相关操作；普通工具错误转成可观测反馈；缺信息先感知/澄清；预算耗尽停止扩展并保留已知状态。只读且安全的回答可降级并披露未使用的机制，不能静默回到另一模型后仍记为完整链成功。

恢复要覆盖“发送前、发送后回执前、结果收到结算前、结算后落盘边界”。端到端 exactly-once 只有外部幂等/查询合同支持时才能声称；否则提供去重结算和 unresolved 处理，不作不存在的保证。
...
用于形成感知层级、世界模型、语义记忆、语言器官和通用技能。允许批处理、并行模拟、CUDA、autograd/optimizer 等成熟训练方法；算法选择不单独决定原生身份，条件是：

- 不以 Transformer hidden state、teacher logits 或外部模型决策作为 Taiji 的运行时依赖；
- 报告明确区分 `native-local`、`native-assisted` 和 `evaluation-only`；
- 同时披露训练算法与监督来源：autograd/local delta 是算法，真实经历/自监督/外部 teacher 是信号来源，两者不能混为一个“原生/非原生”标签；
- assisted 初始化不能被冒充为 Taiji 已具备自主终身学习能力；
- 辅助数据/teacher 的来源、训练用途与移除后果须披露；运行时不保留隐藏认知依赖。普通 optimizer 不因成熟而必须淘汰；持续适应能力仍需独立验收。

因此，“是否使用 optimizer”不再是身份判据；真正判据是认知能力是否由 Taiji 参数与状态承载，以及离线训练后能否继续原生适应。

**2026-08-26 历史收口（不是当前全仓状态）：** 当时完成了局部学习规则迁移。迁移前 8 个模块（`perception`、`world_learning`、`affordance`、`executive`、`content_selection`、`semantic_memory`、`workspace`、`procedural_memory`）共 13 处 `SGD/Adam + loss.backward()`，与内核的 detached delta 规则构成双学习栈；当时 `verify_taiji_native_v7.py` 的 AST 契约把 `backward` 列为禁用属性。历史收口内容：
...
上述只描述当时模块的实现统一，不规定发展训练和终身学习必须永远使用同一数学更新。

**2026-09-17 现码核对：** `scripts/training/verify_taiji_native_v7.py::_native_import_contract` 和 `tests/taiji_native/test_architecture_contract.py` 已不把 `backward` 列为禁项；`taiji/sequence_workspace.py` 的隔离训练器确实使用它。因此 9月6日“当前 AST 禁令仍执行”的文字已过时，本次纠正而不再声称需要先解除一个不存在的禁令。现有 forbidden imports/Transformer 类及运行时所有权检查仍保留；使用何种发展训练、跨时间信用和在线更新范围，按具体合同记录，不等于 H3.8 负结果候选已获默认采用。
...
M4 v1 的实验说明需要进一步约束能力主张：在同一 `BytePredictiveReadout` 上继续写权重，只证明 fixed-capacity continuation；克隆 protected/active readout 只证明权限隔离或受控适应；旁路 `AdaptiveNeuronNetwork` 的存在不证明成长。固定容量也可研究持续适应，但不能宣称结构发展完成。当前 M4.V2 **特定实现合同**要求如下，不将它误作所有未来终身学习算法的定义：
...
这是后续统一链的设计约束，不是已经全链实现的声明。成熟发展训练负责形成可迁移表示/预测/选择/表达；运行期先区分状态更新与参数学习，再按预算准入快适应和慢巩固。不是把所有参数永远冻结，也不是每次聊天无条件自训练。

| 更新面 | 允许的来源与用途 | 保护与评价 |
|---|---|---|
| 工作/信念状态 | 当前观察、检索证据、目标和实际结果；影响下一次选择 | 即使 frozen 参数评估也可更新任务状态；会话/episode 重置和持久范围须冻结 |
| 情景记忆 | 有来源的经历；观察、推测、想象分区记录 | 回答草稿不是真实 outcome；去重、失效、遗忘与跨会话权限；记住事实不冒称慢参数学会规律 |
| 快参数/资格迹 | 归属清楚且准入的反馈；有限更新适应当前环境 | 指定参数清单、预算、重复/过期处理、快状态重置与恢复；对照 frozen 与反馈消融 |
| 慢参数/语义技能 | 发展训练或审查后的 replay/巩固 | 新旧任务矩阵、污染/保持界、parent 保护、child 准入；不把模型自身未核验输出当正确标签 |
| 结构 | 持续能力缺口和资源证据 | 独立 M4 合同；普通任务闭环不强制先实现结构成长 |

每项训练目标要登记“信号来源→接收状态/参数→跨时间路径→截断位置→下游行为”。优先以真实任务结果约束选择与信用，以真实转移训练世界预测；感知重建、语言内容/渲染等辅助目标单列。非可微执行边界可用记录的资格迹、价值/策略目标等方法，不能仅因不可微而宣称无信用，也不能只画箭头就宣称端到端学会。

工程检查验证有限差分/梯度或局部更新正确、目标确实抵达声明参数；能力检查验证这种变化改善未见任务并保持旧能力。二者分别记录。因果归因须对照实际被消费的路径；冻结 prefix 不等于 downstream 无条件依赖，给出梯度不等于模型必然使用该机制。

**多模态与信息治理设计衔接（2026-09-18，待实施/验证）**：见[VISION §20–21](../reference/VISION_FUTURE_TECHNOLOGY.md)。多模态事件保留专用编码、原始区域/时段、共同对象/关系与来源派生链；OCR/ASR/摘要不能提升原内容的权威。来源完整性、事实可信度、用途/学习许可和指令权威分别维护；工作信念可分析未核验假设，持久记忆、快参数、慢巩固按影响面准入。普通学习不得修改可信授权/安全政策；发现污染分别撤销派生记录、恢复快状态或回退慢核心，权重恢复不撤销外部副作用。该段补设计责任，不声称现有所有路径已实现，也不改旧冻结实验或自动授权媒体采集/训练。

### 6.4 恢复、迁移与新数据启动

| 操作 | 合同 | 不允许混称 |
|---|---|---|
| exact resume | 同协议/数据摘要/模型与目标 schema，恢复参数、优化器、RNG、计数、pending 和数据位置；新进程验证后续一步 | 缺关键状态时不得静默重置后称精确续训 |
| warm-start / 新实验 | 保护旧 parent，明确可导入参数/语义映射；新 run/数据/协议血缘；重置或显式迁移优化器、计数、缓存、快状态；重新 preflight | 换 corpus 后沿用旧目标 map 或绕过摘要校验，不是合法迁移 |
| schema migration | 独立版本转换、字段映射、损失说明、兼容/拒绝测试、迁移前后行为核验 | 只改版本号或用随机初始化补缺件不算无损恢复 |

现有 `LanguageAlignmentTrainer` 的 corpus digest 不匹配拒绝继续保留。上述 warm-start 是所需的显式协议，不声称该加载器已经支持；缺少迁移实现时停在该入口，不因此宣布新数据训练在架构上永远不允许。checkpoint 前置要求覆盖这次实际使用的参数和状态，不能借用别的模型的保存成功记录。

### 6.5 长期运行的认知与版本合同（设计入口）

具体学习机制与测试见[VISION §22](../reference/VISION_FUTURE_TECHNOLOGY.md#22-长期运行的补全设计不确定性信念修订记忆预算技能与持续评价)，不声明当前运行路径已经全部实现：

| 核心对象/所有者 | 补齐的责任 | 消费与失败处理 |
|---|---|---|
| `BeliefState` / 工作空间 | 区分事实有效时间与获知时间、作用域、未知/否定/争议、来源和revision；对象/关系理解仍由模型形成 | 回答、世界预测、规划消费当前适用视图；纠错/撤销使相关派生结论与计划待复核，不全局最后写入胜出 |
| 风险/选择头 | 从自由输出及真实结果学习错误/成功估计，并在独立数据校准；权限独立 | 有预算地选择回答、求证或停止；高置信不能提权，全部拒绝不能算能力 |
| 记忆/技能/自我状态 | 分层资源预算、可追溯压缩、真实/想象分离、技能前置/终止、目标取消与能力版本 | 保留策略/技能复用须有下游收益；私有作用域由可信身份选择，不从输入文字获取权限 |
| 学习/发布所有者 | 固定请求版本、shadow候选、获准反馈游标、幂等结算、安全边界切换与显式迁移 | 拒绝半更新和不兼容状态接续；读取/动作/持久写入仍校验最新撤销，旧快照不能复活已撤权任务 |

并发起点采用单一提交所有者与版本快照，未来扩并行由profile和一致性证据决定，不设永久单线程上限。冻结评估不学习测试标签；在线协议独立登记反馈何时可见、更新对象、总资源和保持探针。恢复不重置已用预算、不撤销外部副作用；完整协议沿用§4.7、§6.4及VISION §21的信息治理。

### 6.6 大方向覆盖与架构演进入口

用户要求查漏的是与多模态/恶意信息同级的能力与风险方向，详见[VISION §23](../reference/VISION_FUTURE_TECHNOLOGY.md#23-能力与风险版图补全被局部主线遮蔽的八个大方向)。本架构的对象/事件、工作空间、世界预测、学习与输出层应允许后续接入：真实知识发展经历；抽象量/规则与可验证求解；空间与主动感知；参与者视角和共同背景；目标/偏好/代理奖励分离；正常数据的权益/偏差/输出保护；创作结构与专用生成器官；跨语言共享语义及专用适配。

上述是候选架构责任，不是八个已实现模块或八个并行新核心；按任务迁移收益决定共享与专门化，避免每个方向复制一套认知所有者。§6.5的版本、记忆和恢复保护不能替代这些能力的形成。实施包必须说明发展经历如何塑造相关表示/选择、运行时谁消费以及何种真实行为获益；已有A–E设计按需复用，当前核心合同和执行顺序不因本节自动改变。
...
本节约束成熟能力的内容形成和适应主张；固定初始化、显式控制器及资源上限可作为隔离原型/基线，必须标记其角色。用户目标和安全边界不由学习自行改写。
...
- 数据集问题到答案的固定映射；
- 用 evaluator cue/隐藏答案 prompt/样例分支决定答案或动作；合法对象索引、公开输入协议不因此被禁；
- 通过改阈值掩盖失败机制；
- 仅凭 Python list/dict 或检索命中宣称 Taiji 已学习，而无所有权、消费贡献和持续性证据；
- 为了“非 Transformer”而禁止必要的可学习抽象或选择性路由。

### 8.5 规则实施清单与变更纪律

不另建第二套大门编号。以下 ID 用于追踪本次规则修订，正式能力出口仍用 A/轴/M/L 原编号：

| 规则 | 层级与当前状态 | 实施/核验责任与触发条件 |
|---|---|---|
| GOV-OWN 认知所有权 | P；已有局部实现/测试，不推定全链通过 | 新入口、provider、binder 或决策来源变化时查实际调用与去外围对照 |
| GOV-JOINT 联合贡献 | C；验收原则已定，统一能力未证 | 同 bundle 真实终态、简单策略及语义有效的三类消融；按02 §2.1评审 |
| GOV-SEM 跨模块语义 | E；§4.6设计已定，完整映射待实施 | 接口 owner 负责来源/表示版本/消费/失效测试；不兼容不得静默广播 |
| GOV-ACT 行动生命周期 | E；局部恢复资产可复用，整链待验 | 执行器与 Taiji 结算各负其责；按§4.7逐边界故障注入 |
| GOV-LEARN 学习权限 | C/E；局部学习有证据，统一信用待验 | learner 参数清单、反馈准入、更新 trace、保持/恢复；只声称实测更新层 |
| GOV-RESUME 血缘与迁移 | E；现有 exact-resume 守卫保留，新 warm-start 待实现 | loader/trainer 负责参数映射、父保护与fresh restore；禁止绕过摘要检查 |
| GOV-METHOD 方法可替换 | X；本次明确层级，不自动改具体实现 | CR-4/M4.V2、路由/调度、局部梯度等按各自协议；换方法先比较、后迁移 |
| GOV-CLAIM 结论强度 | P/C；文档执行，非自动能力判定 | 每次报告区分事实、假设、因果证据和决策；未测/不可判/失败分列 |

新规则使用最小条目：ID/层级、要防的风险、作用范围、可验证断言、实现与证据位置、允许的降级/例外、复审触发点。未实施规则写 design-only，不凭空填已通过。低风险文档或局部修复只做相称检查；涉及核心状态/数据/动作权限/正式门或预算才升格评审。

探索、确认、采用三阶段不得混合：探索允许可撤销原型和公开方法变化，但不读取封存 final、不发布晋级；确认在看正式结果前冻结主指标/不确定性规则/预算/停止线；采用才验同入口、迁移、产品安全与当前 CI。样本量、种子数、精确保持或容差依声明风险与测量方差预先确定，不给所有实验套同一个任意数字，也不事后挑有利规则。

故障归类优先于补门：代码违反已定合同→修复并做相关回归；仪器不测所声明量→旧能力判定待核，修仪器后重验；证据不足→不可判；实现正确且确认门失败→限定机制负结果；改变目标/核心表示/权限/预算→新决策。每次新增硬门说明旧协议漏掉的具体风险及退出条件，避免规则越写越多却永远不能结项。
...
> 绕过其他项；maintenance result 记录随 native checkpoint 保存。直接 neuron birth (`add`) 现已
> 纳入同一 candidate contract，standalone region tick 可生成候选并通过统一 materialize、holdout、
> budget、trial、commit/rollback 与 checkpoint continuation；direct-add 子门禁见
> `reports/taiji_runtime_structure_20260826.json`。该 cycle 现已具备显式依赖/冲突判定：反向输入仍
> 按依赖拓扑顺序执行，依赖失败会阻断下游，同一 substrate 的竞争变更全部 `failed_closed`；不同
> neuron identity 的 `add` 可并存并按依赖连续出生，队列只对同一目标 unit 去重。下一入口是建立
> 三层以上自适应区域的规模化结构维护 Gate，覆盖跨区域 route、混合 add/split/prune、资源竞争、
> checkpoint continuation 和拓扑不变量。
> 三层规模化 Gate 已通过：`source→relay→target` route 在 connected split 后保留并按受影响边
> 展开，standalone neuron `add` 与 network split 可在同一 maintenance cycle 中提交，checkpoint、
