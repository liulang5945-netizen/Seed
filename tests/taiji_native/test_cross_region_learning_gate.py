...
def test_cross_region_learning_gate() -> None:
    ...
    assert report["metrics"]["selected_holdout_transfer"] > report["metrics"]["fixed_full_holdout_transfer"]
    assert report["metrics"]["selected_holdout_transfer"] > report["metrics"]["random_holdout_transfer"]
