...
REPORT = (
    Path(__file__).resolve().parents[2] / "reports" / "taiji_m4v2_b3_k_three_batch_20260910.json"
)


def test_b3_k_three_batch_has_three_three_example_course_cells() -> None:
    report = json.loads(REPORT.read_text(encoding="utf-8"))

    assert report["status"] == "passed"
    assert report["run_kind"] == "bounded-three-example-diagnostic"
    assert report["bounded_batch_count"] == 3
...
def test_b3_k_three_batch_preserves_learning_boundaries() -> None:
    report = json.loads(REPORT.read_text(encoding="utf-8"))

    assert report["no_update_control_passed"] is True
    assert all(
        cell["report"]["train_episode_count"] == 3
        and len(cell["report"]["train_experience_digests"]) == 3
