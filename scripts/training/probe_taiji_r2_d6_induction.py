"""R2-D6 H-B induction-copy learnability probes (contract section 4).

Two arms, seed 20260917, micro-batch 8, 30 epochs, lr 0.01, lambda=1.0 copy
supervision throughout (the D4 objective stays frozen as the shared base); the
single new variable is graph v8 copy induction:

- ``B1``: original train + v8 induction.  The D4 regression gates must hold;
  the final ``copy_induce_bias`` sign is reported (v1 has no multi-byte
  pressure, so the sign forecasts interference or benefit).
- ``B2``: fixture v2 train (three 6-byte colors) + v8 induction.  The D5-A01
  gate set applies (multibyte 72/72, single-byte regression, copy probability,
  entry-misbind collapse, stability) plus the bias readout.

Train only; dev and final unread.  Usage:
``python scripts/training/probe_taiji_r2_d6_induction.py --arm B2``
"""
...
NEW_COLORS = ("琥珀", "珊瑚", "翡翠")
CHECKPOINT_ROOT = Path("reports/r2_d6_checkpoints")
...
ARMS = {
    # arm -> (fixture, label)
    "B1": ("v1", "hb_induction_original_train"),
    "B2": ("v2", "hb_induction_multibyte_train"),
}
...
def _value_length_groups(
    ...
    with torch.no_grad():
        for row in rows:
            ...
            generated = prototype.generate(row["prefix"].encode("utf-8")).bytes_out.decode(
                "utf-8", errors="replace"
            )
            groups[key].append(generated == row["response"])
    return {
        name: {
...
def main() -> int:
    ...
    fixture_version, label = ARMS[args.arm]
    ...
    trainer = SequenceWorkspaceTrainer(
        prototype, learning_rate=LEARNING_RATE, code_revision=f"r2-d6-{args.arm}-probe"
    )
    ...
    bias_end = float(prototype._parameters["copy_induce_bias"].detach().item())
    ...
    if fixture_version == "v2":
        groups = _value_length_groups(prototype, rows)
    ...
    report = {
        "format": "taiji-r2-d6-induction-probe-v1",
        "version": 1,
        "contract": CONTRACT,
        ...
        "reading": (
            "R2-D6 H-B induction-copy learnability probe (train-only); the bias "
            "sign is the mechanism-use readout; not a capability claim"
        ),
        "growth_admitted": False,
        "can_promote": False,
    }
    out_path = PROJECT_ROOT / OUT_DIR / f"r2_d6_probe_{args.arm}_{label}_20260918.json"
