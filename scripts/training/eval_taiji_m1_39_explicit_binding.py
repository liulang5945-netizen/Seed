"""Diagnose an explicit cue-local action binding against the shared readout."""
...
FORMAT = "taiji-native-m1-39-explicit-binding-v1"
DEFAULT_REPORT = PROJECT_ROOT / "reports" / "taiji_m1_39_explicit_binding_20260902.json"
SEEDS = (11, 29, 47)
DECODERS = ("shared", "cue_selective")
...
def _probe(model: Taiji, queries: tuple[Any, ...], actions: tuple[int, ...]) -> float:
    ...
    for query in queries:
        model.reset_dynamics(episode_id=f"m1-39-probe-{query.query_id}")
        model.observe(
...
def _config_for_decoder(seed: int, decoder: str) -> TaijiConfig:
    if decoder not in DECODERS:
        raise ValueError(f"unsupported episodic binding decoder: {decoder}")
    values = _config(seed).to_dict()
    values["memory_action_decoder"] = decoder
    return TaijiConfig.from_dict(values)


def _binding_topology_digest(model: Taiji) -> str:
    memory = model.memory
    if model.config.memory_action_decoder == "shared":
        ...
        route = {
            ...
            "cue_local_action_readout_pre_index": memory.local_action_readout.pre_index.detach().cpu(),
        }
...
def _condition_record(
    ...
    for seed in SEEDS:
        ...
        phase_a = Taiji(config, episode_id=f"m1-39-phase-a-{decoder}-{seed}")
        ...
        binding_digest = _binding_topology_digest(phase_a)
        ...
        records.append(
            {
                "seed": seed,
                "decoder": decoder,
                "binding_topology_digest": binding_digest,
...
def run_diagnosis() -> dict[str, Any]:
    ...
    return {
        "format": FORMAT,
        "version": 1,
        "status": "passed" if explicit["condition_gate_passed"] else "failed",
        "variable_changed": "episodic cue/action binding decoder only",
        "baseline_decoder": "shared",
        "candidate_decoder": "cue_selective",
        ...
        "event_set_digest": event_set_digest,
        "parameter_budgets_equal": shared_budget == explicit_budget,
        ...
        "conclusion": {
            ...
            "next_boundary": (
                ...
                if explicit["condition_gate_passed"]
                else "cue-selective binding did not pass B5; preserve shared baseline and split cue encoding from action write"
            ),
        },
    }
