"""C-entry parity formal v3 runner (readout-revised, fresh sealed split).

Preregistration: ``plans/reference/M4V2_B3_K_C_PARITY_V3_PREREGISTRATION
_20260910.md``.  The only design change from the v1 formal is the candidate
readout: weight AVERAGING of the two channels instead of weight addition,
which doubles the shared parent and was refuted by the v1 formal plus the
validation-only attribution probe.  Both arms reuse the v2 artifacts
unchanged (no retraining); the sealed split is the freshly materialized,
never-read v2 artifact.  Gates G1-G3 are frozen and identical in structure
to the v1 formal.  ``can_promote=false`` regardless of outcome.
"""
...
REPORT_FORMAT = "taiji-m4v2-b3-k-c-parity-formal-v3"
VERSION = 1
MODEL_SEEDS = (17, 23, 31)
COURSE_SEEDS = (0, 1, 2)
V2_BUILD_REPORT = PROJECT_ROOT / "reports" / "taiji_m4v2_b3_k_c_parity_v2_build_20260910.json"
SEALED_TEST = PROJECT_ROOT / "plans" / "manifests" / "taiji_m4v2_b3_k_c_sealed_test_v2.json"
...
DEFAULT_REPORT = PROJECT_ROOT / "reports" / "taiji_m4v2_b3_k_c_parity_formal_v3_20260910.json"
PRESEALED_REPORT = (
    PROJECT_ROOT / "reports" / "taiji_m4v2_b3_k_c_parity_formal_v3_presealed_20260910.json"
)
...
def _widened_average_learners(
    ...
    """Weight-averaging readout: the calibration-preserving composition."""
...
def _verify_inputs() -> dict[str, Any]:
    ...
    if internal_digest != recomputed:
        raise ValueError("sealed v2 internal artifact_digest mismatch")
    if sealed.get("status") != "materialized-unread":
        raise ValueError("sealed v2 artifact is not in materialized-unread state")
    ...
    return {
        "sealed_v2_sha256": hashlib.sha256(sealed_bytes).hexdigest(),
        "sealed_v2_internal_digest": internal_digest,
        "sealed_v2_task_seed": sealed.get("task_seed"),
...
def main() -> int:
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="parity_v3_"))
    try:
        for model_seed in MODEL_SEEDS:
    ...
    pre_sealed = {
        "format": "taiji-m4v2-b3-k-c-parity-formal-v3-presealed-v1",
        "version": 1,
    ...
    # Sealed v2 bytes are read only after the pre-sealed artifact (frozen
    # epsilon) is on disk.
    sealed = json.loads(SEALED_TEST.read_text(encoding="utf-8"))
    sealed_rows: list[dict[str, Any]] = []
    temp_root = Path(tempfile.mkdtemp(prefix="parity_v3_sealed_"))
    try:
        for model_seed in MODEL_SEEDS:
            artifacts, parent_digest, bundle, projector = _context(
                worker_root=WORKER_ROOT, model_seed=model_seed
    ...
    payload = {
        ...
        "can_promote": False,
        "preregistration": ("plans/reference/M4V2_B3_K_C_PARITY_V3_PREREGISTRATION_20260910.md"),
        "design_change": {
            "readout": "weight averaging of the two channels (calibration-preserving)",
            "previous_failure": "v1 formal: weight addition doubled the shared parent (+0.107/course)",
            "retraining": "none; v2 artifacts reused with digest verification",
        },
        "input_verification": input_verification,
        ...
        "verdict": {
            ...
            "interpretation": (
                "readout-revised equal-budget learning-rule comparison on a "
                "freshly materialized never-read sealed split"
            ),
        },
        "sealed_read_count": 1,
        "elapsed_seconds": time.perf_counter() - started,
        "boundary": (
            "C-entry parity formal v3 only; no promotion, no default runtime "
