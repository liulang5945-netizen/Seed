...
REPORT_FORMAT = "taiji-m4v2-c-stage-formal-v2"
VERSION = 1
...
SEALED_TEST = PROJECT_ROOT / "plans" / "manifests" / "taiji_m4v2_c_stage_sealed_test_v5.json"
...
DEFAULT_REPORT = PROJECT_ROOT / "reports" / "taiji_m4v2_c_stage_formal_v2_20260910.json"
PRESEALED_REPORT = PROJECT_ROOT / "reports" / "taiji_m4v2_c_stage_formal_v2_presealed_20260910.json"
...
def _verify_inputs() -> dict[str, Any]:
    ...
    if internal_digest != recomputed:
        raise ValueError("sealed v5 internal artifact_digest mismatch")
    if sealed.get("status") != "materialized-unread":
        raise ValueError("sealed v5 artifact is not in materialized-unread state")
    ...
    return {
        "sealed_v5_sha256": hashlib.sha256(sealed_bytes).hexdigest(),
        "sealed_v5_internal_digest": internal_digest,
        "sealed_v5_task_seed": sealed.get("task_seed"),
...
def main() -> int:
    ...
    # G1 (v2 amendment): learning evidence is measured on the sealed split
    # (the primary measurement, covering the weak classes) instead of the
    # 3-episode validation proxy whose noise swamped the replay mechanism
    # in the v1 formal.  Both learning arms must improve vs frozen on
    # sealed in >= 2/3 courses.
    g1_courses = [
        course
        for course in COURSE_SEEDS
        if sealed_c_course[course] < 0.0 and sealed_fs_course[course] < 0.0
    ]
    ...
    gates = {
        "G1_learning_floor": {
            "rule": "C and FS sealed combined deltas < 0 in >= 2/3 courses (v2: sealed-based learning evidence)",
    ...
    payload = {
        ...
        "can_promote": False,
        "preregistration": ("plans/reference/M4V2_C_STAGE_FORMAL_V2_PREREGISTRATION_20260910.md"),
        "input_verification": input_verification,
