"""H-FBW matched dev: does up-weighting the FIRST response byte move dev ``exact``?

Candidate spec: plans/reference/M5_R2_READOUT_DIAGNOSIS_20260917.md (countermeasure form b) + M5_R2_DIAG_A_COINCIDENCE_20260917.md

Two arms, same corpus, same budget, same seeds -- the **only** difference is the frozen
first-byte loss weight (readout diagnosis: 16/16 dev episodes err on byte 0):

* ``control``     : first_byte_weight = 1.0  (plain mean; must reproduce the legacy arm)
* ``treatment``   : first_byte_weight = 5.0  (up-weights the first response byte)
...
DEFAULT_REPORT = Path("reports/r2_hfbw_matched_dev_20260917.json")
REPORT_FORMAT = "taiji-r2-hfbw-matched-dev-v1"
...
def _run_arm(arm: str, *, seed: int, fbw: float, report_dir: Path) -> dict[str, Any]:
    ...
    trainer.enable_first_byte_weight(weight=fbw)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="H-FBW matched dev (two arms)")
    parser.add_argument("--report", type=Path, default=DEFAULT_REPORT)
    parser.add_argument("--output-dir", type=Path, default=Path("reports/r2_hfbw_matched_dev"))
    ...
    arms = [
        _run_arm(arm, seed=seed, fbw=fbw, report_dir=report_dir)
        for seed in SEEDS
        for arm, fbw in (("control", FBW_CONTROL), ("treatment", FBW_TREATMENT))
    ]
    ...
    if both_zero:
        verdict = "h_fbw_rejected: both arms have dev exact = 0"
    elif treatment_exact <= control_exact:
        verdict = "h_fbw_not_supported: treatment dev exact is not above control"
    else:
        verdict = "h_fbw_supported: treatment dev exact is above control"
    ...
    payload: dict[str, Any] = {
        ...
        "frozen": {
            ...
            "epochs": EPOCHS,
            "max_episodes": MAX_EPISODES,
            "first_byte_weight_control": FBW_CONTROL,
            "first_byte_weight_treatment": FBW_TREATMENT,
        ...
        "scope": (
            ...
            "to the chat() path (H-FBW spec section 3, residual risk 1)"
        ),
    }
