"""Aggregate the M4.R1 fixed-capacity consolidation formal reports.

This is a read-only, fail-closed evidence aggregator.  It validates the
shared record-disjoint C/C'/C'' chain, the fixed consolidation strength, the
source-checkpoint lineage, and the full technical Gate before computing the
seed-panel capability and resource statistics.  A successful aggregation is
not a promotion: the candidate remains non-promotable when old-course
retention still degrades.
"""
...
AGG_FORMAT = "taiji-m4r1-consolidation-aggregate-v1"
AGG_VERSION = 1
EXPECTED_SEEDS = (11, 29, 47)
EXPECTED_CONSOLIDATION_STRENGTH = 0.5
EXPECTED_CONFIG = {
    "epochs": 1,
...
def _validate_report(report: dict[str, Any], *, expected_seed: int) -> dict[str, Any]:
    ...
    if float(report.get("consolidation_strength")) != EXPECTED_CONSOLIDATION_STRENGTH:
        raise ValueError(f"seed {expected_seed} consolidation strength mismatch")
    ...
    if float(arm.get("consolidation_strength")) != EXPECTED_CONSOLIDATION_STRENGTH:
        raise ValueError(f"seed {expected_seed} cascade strength mismatch")
    ...
    training = _require_mapping(arm.get("training"), "cascade training")
    resource = _require_mapping(training.get("resource"), "cascade training.resource")
    resources = _require_mapping(arm.get("resources"), "cascade resources")
    return {
        "seed": expected_seed,
        ...
        "resources": {
            "training_elapsed_seconds": float(resource["elapsed_seconds"]),
            "learn_seconds": float(resource["learn_seconds"]),
            "scoring_seconds": float(resources["scoring_seconds"]),
            "peak_working_set_bytes": int(resource["peak_working_set_bytes"]),
            "body_bytes_per_second": float(resource["body_bytes_per_second"]),
        },
    }
...
def aggregate_reports(reports: Sequence[dict[str, Any]]) -> dict[str, Any]:
    ...
    retention_gate_passed = all(
        value <= 0.0 for value in (*c2_cycle3_deltas, *c_cycle2_deltas, *c_cycle3_deltas)
    )
    aggregate = {
        ...
        "resources": {
            "training_elapsed_seconds_mean": mean(
                [entry["resources"]["training_elapsed_seconds"] for entry in entries]
            ),
            "scoring_seconds_mean": mean(
                [entry["resources"]["scoring_seconds"] for entry in entries]
            ),
            "peak_working_set_bytes_max": max(
                entry["resources"]["peak_working_set_bytes"] for entry in entries
            ),
        },
        "interpretation": (
            "technical consolidation and checkpoint gates passed and all three "
            "C'' holdouts improved, but the candidate is not promoted: C retention "
            "degraded after cycle 2 and again after cycle 3 for all three seeds"
        ),
    }
    return {
        "format": AGG_FORMAT,
        ...
        "status": "passed",
        "can_promote": False,
        "consolidation_strength": EXPECTED_CONSOLIDATION_STRENGTH,
        "seeds": entries,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--report",
        type=Path,
        default=PROJECT_ROOT / "reports" / "taiji_m4r1_consolidation_aggregate_20260908.json",
    )
