"""Materialize the C-stage formal v2 sealed-test split (fresh, never-read).

v2 preregistration: ``plans/reference/M4V2_C_STAGE_FORMAL_V2
_PREREGISTRATION_20260910.md``.  Task seed 151; three episodes' first
files cover D (``.h``), R (missing), and A (python); disjointness
enforced against the train/validation fixture and sealed v1/v2/v3/v4.
"""
...
REPORT_FORMAT = "taiji-m4v2-c-stage-sealed-test-v5"
VERSION = 1
SEALED_TASK_SEED = 151
...
SEALED_EPISODE_PATHS = (
    ...
    ("sealed5_missing_00.txt", "sealed5_typescript_00.ts", "sealed5_python_01.py"),
    ("sealed5_python_02.py", "sealed5_rust_01.rs", "sealed5_typescript_01.ts"),
)
...
def _write_sealed_files(root: Path) -> None:
    for index, paths in enumerate(SEALED_EPISODE_PATHS):
        for path in paths:
            if path.startswith("sealed5_missing"):
                continue
            stem, extension = path.rsplit(".", 1)
            language = stem.removeprefix("sealed5_").rsplit("_", 1)[0]
            value = SEALED_TASK_SEED + index * 23 + len(stem)
            pad = "s" * (2 * index + len(stem))
            if language == "python":
                body = (
                    f"def sealed5_answer_{index}(value: int) -> int:\n"
                    f"    return value + {value}\n"
                    f"# taiji c-stage sealed v5 {pad}\n"
                )
            elif language == "typescript":
                body = (
                    f"interface Sealed5Answer{index} {{ value: number; }}\n"
                    f"export const sealed5Answer{index} = "
                    f"(input: Sealed5Answer{index}): number => input.value + {value};\n"
                    f"// taiji c-stage sealed v5 {pad}\n"
                )
            elif language == "rust":
                body = (
                    "fn main() {\n"
                    f'    println!("taiji-c-stage-sealed5-{index}-{value}");\n'
                    "}\n"
                    f"// taiji c-stage sealed v5 {pad}\n"
                )
            elif language == "header":
                body = (
                    f"#pragma once\nint sealed5_shared_{index};\n"
                    f"// taiji c-stage sealed v5 {pad}\n"
                )
            else:
...
def _materialize(output: Path) -> dict[str, object]:
    ...
    temp_parent = PROJECT_ROOT / ".tmp-m4v2-c-stage-sealed-test-v5"
    ...
    try:
        ...
        if sealed_file_digests & baseline_file_digests:
            raise ValueError("sealed v5 file digest overlaps train/validation fixture")
        if sealed_file_digests & prior_digests:
            raise ValueError("sealed v5 file digest overlaps a prior sealed artifact")
        if len(sealed_observations) != 9:
            raise ValueError("sealed v5 must contain nine unique step observations")

        anchor = _observe_all(
            temp_root,
            registry=registry,
            split="c-stage-sealed-v5-anchor",
        ...
        for episode_index, paths in enumerate(SEALED_EPISODE_PATHS):
            ...
            transition = _transition_examples(
                sequence,
                split=f"c-stage-sealed-v5-{episode_index}",
            )
            episodes.append(
                {
                    "episode_id": f"sealed-v5-{episode_index}",
