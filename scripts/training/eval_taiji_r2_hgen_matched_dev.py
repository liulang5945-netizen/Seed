"""H-GEN matched dev: does generation-state training move dev ``exact``?

Candidate spec: plans/reference/M5_R2_NEXT_CANDIDATE_GENERATION_STATE_TRAINING_20260917.md
...
frozen ``phi_max`` of the scheduled-sampling schedule (spec section 5):

* ``control``     : phi_max = 0.0  (teacher forcing; must reproduce the TF path)
* ``treatment``   : phi_max = 0.5  (linear schedule over the frozen horizon)
...
DEFAULT_REPORT = Path("reports/r2_hgen_matched_dev_20260917.json")
REPORT_FORMAT = "taiji-r2-hgen-matched-dev-v1"

# ---- frozen budget (spec section 4) -----------------------------------------
# Re-verified at the H3.8 budget (30 x 25) after the budget-discrimination finding:
# the original 2 x 12 verdict was per that finding an *invalid* budget (control could
# not produce any non-zero dev exact there), so it could not support the claim that
# exposure bias is excluded.  Three seeds for the same reason as H-OBJ.
SEEDS = (20260917, 20260918, 20260919)
EPOCHS = 30
MAX_EPISODES = 25
PHI_MAX_TREATMENT = 0.5
PHI_MAX_CONTROL = 0.0
...
def _run_arm(arm: str, *, seed: int, phi_max: float, report_dir: Path) -> dict[str, Any]:
    ...
    trainer.enable_scheduled_sampling(
        phi_max=phi_max,
        total_steps=max(1, EPOCHS * min(MAX_EPISODES, len(train_split))),
        generator=generator,
    )
        ...
        epoch_records.append(
            {
                ...
                "phi_at_end": trainer.current_generation_state_ratio(),
            }
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="H-GEN matched dev (two arms)")
    parser.add_argument("--report", type=Path, default=DEFAULT_REPORT)
    parser.add_argument("--output-dir", type=Path, default=Path("reports/r2_hgen_matched_dev"))
    ...
    arms = [
        _run_arm(arm, seed=seed, phi_max=phi_max, report_dir=report_dir)
        for seed in SEEDS
        for arm, phi_max in (("control", PHI_MAX_CONTROL), ("treatment", PHI_MAX_TREATMENT))
    ]
    ...
    if both_zero:
        verdict = "h_gen_rejected: both arms have dev exact = 0"
    elif treatment_exact <= control_exact:
        verdict = "h_gen_not_supported: treatment dev exact is not above control"
    else:
        verdict = "h_gen_supported: treatment dev exact is above control"
    ...
    payload: dict[str, Any] = {
        ...
        "frozen": {
            ...
            "epochs": EPOCHS,
            "max_episodes": MAX_EPISODES,
            "phi_max_control": PHI_MAX_CONTROL,
            "phi_max_treatment": PHI_MAX_TREATMENT,
        ...
        "scope": (
            ...
            "to the chat() path (H-GEN spec section 3, residual risk 1)"
        ),
    }
