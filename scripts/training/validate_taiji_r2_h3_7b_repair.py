"""Bounded train-only repair validation, never an H3.7 capability rerun."""
...
def main():
    ...
    def fresh_check(path):
        output = subprocess.check_output(
            [
                sys.executable,
                "-m",
                "scripts.training.validate_taiji_r2_h3_7b_repair",
                "--output",
                str(args.output),
                "--arm",
                args.arm,
    ...
    try:
        ...
        persist()
        report["training"] = trainer.train(epochs=1, max_episodes=12)
        if trainer.global_step != 537 or trainer.episode_count != 12:
        ...
        report["status"] = "passed_engineering_only"
        persist()
