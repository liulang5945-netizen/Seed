"""Compare retention-preserving update rules without increasing G capacity.

P4.4 reproduced the P4.2/P4.3 retention failure on new, structure-matched
identities.  P4.5 therefore keeps the 13-parameter G and compares three
update rules on new data only:

* ``new-only``: the baseline update;
* ``rehearsal-interleaved``: the same update with a separately generated,
  fit-eligible rehearsal cohort; and
* ``constrained-update``: the new-task update projected into a fixed trust
  region around the parent checkpoint after every local update.

The P4.4 sibling artifact is used only to derive the retention shape.  A new
retention artifact is generated for evaluation and is never sent to ``fit``.
No rule in this script changes the topology, input contract, selection
threshold, K checkpoints, or promotion state.
"""
...
REPORT_FORMAT = "taiji-m5-k-p4-5-update-rule-gate-v1"
MANIFEST_FORMAT = "taiji-m5-k-p4-5-update-rule-gate-manifest-v1"
VERSION = 1
DEFAULT_MANIFEST = (
    PROJECT_ROOT / "plans" / "manifests" / "taiji_m5_k_p4_5_update_rule_gate_manifest_v1.json"
)
DEFAULT_REPORT = PROJECT_ROOT / "reports" / "taiji_m5_k_p4_5_update_rule_gate_20260911.json"
P4_1_MANIFEST = (
...
SEEDS = (0, 1)
MARGIN_EPSILON = 1e-9
# A fixed 15% parent-state trust region is registered before looking at the
# P4.5 result.  It is large enough to permit new-task movement while making
# the retention-preserving constraint explicit and measurable.
TRUST_REGION_RATIO = 0.15
    ...
    return tuple(
        {
            "index": index,
            "class_key": class_key,
            "project_id": f"p4-5-{split}-project-{class_key.lower()}",
            "variant_paths": paths,
...
def _rebind_pressure_record(
    ...
    *,
    ...
    candidate_set = GSelectionCandidateSet.create(
        example_id=(
            f"p4-5:{split}:{source_index}:width-{width}:{old_candidates.candidate_set_digest}"
        ),
        family_id=f"p4-5:{split}:family:{source_index}",
        split=split,
    ...
    *,
    ...
    for spec in _new_specs(split, offset):
        ...
        candidate_set, behavior_set, diagnostic = _behavior_record(
            ...
            label=f"p4-5-{split}-{int(spec['index'])}",
        )
        base_records.append(
            {
                "experience": experience,
...
def _structured_specs(contract: Mapping[str, Any], *, namespace: str) -> tuple[dict[str, Any], ...]:
    ...
    for index, row in enumerate(contract["rows"]):
        if int(row["role_counts"].get("proposal", 0)) > 0:
            project_id = f"p4-5-{namespace}-project-a"
            prefix = f"p40_p45_{namespace}_a"
            paths = (
            ...
            project_id = (
                f"p4-5-{namespace}-project-a" if safe_index == 0 else f"p4-5-{namespace}-project-b"
            )
            prefix = f"p40_p45_{namespace}_{'a' if safe_index == 0 else 'b'}"
            paths = (
        ...
        specs.append(
            {
                ...
                "variant_paths": paths,
                "state_profile": state_profile,
                "task_seed": 45000 + index + (100 if namespace == "rehearsal" else 0),
    ...
    *,
    ...
    for spec in specs:
        ...
        candidate_set, behavior_set, diagnostic = _behavior_record(
            ...
            goals=goals,
            content_plans=content_plans,
            label=f"p4-5-{namespace}-{split}-{int(spec['index'])}",
        )
        base_records.append(
            {
                "experience": experience,
...
def _manifest_identity(payload: Mapping[str, Any]) -> tuple[set[str], set[str]]:
    ...
    if isinstance(payload.get("records"), list):
        collections = [payload["records"]]
    else:
        collections = [
            value for value in payload.get("records", {}).values() if isinstance(value, list)
        ]
...
def _parent_state(learner: GSelectionLearner) -> tuple[torch.Tensor, ...]:
    return tuple(parameter.detach().cpu().clone() for parameter in learner.model.parameters())


def _state_distance(learner: GSelectionLearner, state: Sequence[torch.Tensor]) -> float:
    differences = [
        parameter.detach().cpu() - reference
        for parameter, reference in zip(learner.model.parameters(), state, strict=True)
    ]
    return float(torch.linalg.vector_norm(torch.cat([value.reshape(-1) for value in differences])))


def _project_to_parent(
    learner: GSelectionLearner, state: Sequence[torch.Tensor], radius: float
) -> bool:
    distance = _state_distance(learner, state)
    if distance <= radius + MARGIN_EPSILON:
        return False
    scale = float(radius) / distance
    with torch.no_grad():
        for parameter, reference in zip(learner.model.parameters(), state, strict=True):
            projected = reference + (parameter.detach().cpu() - reference) * scale
            parameter.copy_(projected.to(device=learner.device, dtype=parameter.dtype))
    return True


def _constrained_fit_sequence(
    ...
    *,
    ...
    learning_rate: float,
    order_seed: int,
    parent_state: Sequence[torch.Tensor],
    radius: float,
) -> dict[str, Any]:
    items = tuple(record["candidate_set"] for record in records)
    if not items or any(item.split != "train" for item in items):
        raise ValueError("P4.5 constrained fit requires non-empty train records")
    if len(items) != len({item.candidate_set_digest for item in items}):
        raise ValueError("P4.5 constrained fit candidate sets must be unique")
    dataset_digest = content_digest(
        {
            ...
            "constraint": "parent-trust-region",
            "radius": float(radius),
        }
    )
    projection_count = 0
    ...
    for epoch in range(int(epochs)):
        ...
        for index in order:
            ...
            error = mean_squared_error_delta(learner.model(inputs), targets)
            apply_linear_delta(learner.model, inputs, error, float(learning_rate))
            projection_count += int(_project_to_parent(learner, parent_state, radius))
            maximum_distance = max(maximum_distance, _state_distance(learner, parent_state))
    ...
    return {
        ...
        "maximum_parent_distance": maximum_distance,
    }
...
def _fit_sources(records: Sequence[Mapping[str, Any]]) -> list[Mapping[str, Any]]:
    return [record for record in records if bool(record.get("fit_eligible"))]
...
def _run(
    *,
    ...
    run_dir = DEFAULT_OUTPUT_ROOT / f"taiji_m5_k_p4_5_update_rule_gate_{uuid4().hex}"
    ...
    try:
        ...
        p4_4_report = _load_json(P4_4_REPORT)
        p3_5_report = _load_json(P3_5_REPORT)
        if (
            p4_4_report.get("status") != "completed"
            or p4_4_report.get("outcome") != "retention_failure_reproduced"
        ):
            raise ValueError("P4.5 requires completed P4.4 retention_failure_reproduced")
        if p4_4_report.get("manifest_digest") != p4_4_manifest.get("manifest_digest"):
            raise ValueError("P4.4 manifest/report digest mismatch")
        if _digest_without(p4_4_manifest, "manifest_digest") != p4_4_manifest.get(
            "manifest_digest"
        ):
            raise ValueError("P4.4 manifest content digest mismatch")
        if p4_4_report.get("training_performed") or p4_4_report.get("fit_called"):
            raise ValueError("P4.4 source must remain validation-only")
        if p4_4_report.get("growth_admitted") or p4_4_report.get("can_promote"):
            raise ValueError("P4.5 cannot consume an admitted P4.4 artifact")
        if p4_4_manifest.get("source_p4_2_manifest_digest") != p4_2_manifest.get(
            "manifest_digest"
        ) or p4_4_manifest.get("source_p4_3_manifest_digest") != p4_3_manifest.get(
            "manifest_digest"
        ):
            raise ValueError("P4.4 source chain drifted")
        ...
        if content_digest(parent_payload) != str(parent_metadata["digest"]):
            raise ValueError("P4.5 parent external checkpoint digest drifted")
        ...
        if str(p4_2_manifest["parent_g_checkpoint_digest"]) != str(parent_metadata["digest"]):
            raise ValueError("P4.5 parent G lineage drifted")
        parent_restore = _independent_g_restore(parent_path)
        if not parent_restore.get("independent_process_restore"):
            raise RuntimeError("P4.5 parent independent restore failed")
        ...
        train_records = _build_pressure_split(
            ...
            source_manifest_digest=str(p4_4_manifest["manifest_digest"]),
            projector=projector,
            semantic=semantic,
            transition=transition,
        ...
        rehearsal_records = _build_structured_records(
            contract=contract,
            namespace="rehearsal",
            split="train",
            fit_eligible=True,
            scratch=run_dir / "data" / "rehearsal",
            parent_digest=parent_digest,
            worker_bundle_digest=bundle.bundle_digest,
            source_manifest_digest=str(p4_4_manifest["manifest_digest"]),
            projector=projector,
        ...
        train_fit = _fit_sources(train_records)
        rehearsal_fit = _fit_sources(rehearsal_records)
        all_records = [
            *train_records,
            *validation_records,
            *holdout_records,
        ...
        for manifest in (p4_1_manifest, p4_2_manifest, p4_3_manifest, p4_4_manifest):
        ...
        identity_gate = {
            ...
            "rehearsal_records": len(rehearsal_records) == int(contract["row_count"]),
            "retention_records": len(retention_records) == int(contract["row_count"]),
            "train_fit_positive": len(train_fit) >= 8,
            "rehearsal_fit_positive": len(rehearsal_fit) == len(rehearsal_records),
        ...
        if not all(identity_gate.values()):
            raise ValueError(
                f"P4.5 identity gate failed: {[key for key, value in identity_gate.items() if not value]}"
            )
        parent_metrics = {
        ...
        parent_norm = float(
            torch.linalg.vector_norm(torch.cat([value.reshape(-1) for value in parent_state]))
        )
        trust_region_radius = parent_norm * TRUST_REGION_RATIO
        ...
        for seed in SEEDS:
            ...
            learners = {
                ...
                "rehearsal-interleaved": GSelectionLearner.from_checkpoint(
                    copy.deepcopy(parent_payload), device="cpu"
                ),
                "constrained-update": GSelectionLearner.from_checkpoint(
                    copy.deepcopy(parent_payload), device="cpu"
                ),
            }
            zero_checkpoints = {
            ...
            if not all(item["passed"] for item in zero_checkpoints.values()):
                raise RuntimeError(f"P4.5 zero-step checkpoint preflight failed for seed {seed}")
            new_fit = _fit_sources(train_records)
            fit_new = _fit_sequence(
                learners["new-only"],
                new_fit,
                epochs=TRAINING_EPOCHS,
                learning_rate=LEARNING_RATE,
                order_seed=seed,
                source_labels=["new-task"] * len(new_fit),
            )
            rehearsal_sequence = [*new_fit, *rehearsal_fit]
            fit_rehearsal = _fit_sequence(
                learners["rehearsal-interleaved"],
                rehearsal_sequence,
                epochs=TRAINING_EPOCHS,
                learning_rate=LEARNING_RATE,
                order_seed=seed,
                source_labels=["new-task"] * len(new_fit) + ["rehearsal"] * len(rehearsal_fit),
            )
            fit_constrained = _constrained_fit_sequence(
                learners["constrained-update"],
                new_fit,
                epochs=TRAINING_EPOCHS,
                learning_rate=LEARNING_RATE,
                order_seed=seed,
            ...
            if not all(item["passed"] for item in trained_checkpoints.values()):
                raise RuntimeError(f"P4.5 trained checkpoint preflight failed for seed {seed}")
            tamper_gate = {
            ...
            if not all(tamper_gate.values()):
                raise RuntimeError(f"P4.5 tamper gate failed for seed {seed}")
            metrics = {
                arm: {
            ...
            new_task_gate = {
                arm: _gate_noninferior(metrics[arm]["holdout"], metrics["new-only"]["holdout"])
                for arm in ("rehearsal-interleaved", "constrained-update")
            }
            retention_gate = {
            ...
            seed_results.append(
                {
                    ...
                    "fit": {
                        "new-only": fit_new,
                        "rehearsal-interleaved": fit_rehearsal,
                        "constrained-update": fit_constrained,
                    },
        ...
        constrained_retention_passed = all(
            all(retention_gate["constrained-update"].values())
            for result in seed_results
            for retention_gate in [result["retention_gate"]]
        )
        constrained_new_task_noninferior = all(
            all(result["new_task_gate"]["constrained-update"].values()) for result in seed_results
        )
        constrained_new_task_gain = all(
            float(result["metrics"]["constrained-update"]["holdout"]["selected_utility_mean"])
            > float(parent_metrics["holdout"]["selected_utility_mean"]) + MARGIN_EPSILON
            or float(result["metrics"]["constrained-update"]["holdout"]["behavior_target_hit_rate"])
            > float(parent_metrics["holdout"]["behavior_target_hit_rate"]) + MARGIN_EPSILON
            for result in seed_results
        )
        retention_repaired = constrained_retention_passed
        update_rule_repaired = (
            retention_repaired and constrained_new_task_noninferior and constrained_new_task_gain
        )
        if update_rule_repaired:
            outcome = "update_rule_repaired"
        elif retention_repaired:
            outcome = "update_rule_no_gain"
        else:
            outcome = "update_rule_unresolved"
        source_gate = {
            "p4_4_completed_and_reproduced": True,
            "p4_4_validation_only": True,
        ...
        manifest = {
            ...
            "fit_policy": {
                ...
                "growth_admitted": False,
                "p4_4_sibling_fit_count": 0,
                "retention_fit_count": 0,
            },
            "update_rules": {
                "new-only": {"train_records": len(train_fit)},
                "rehearsal-interleaved": {
                    "train_records": len(train_fit),
                    "rehearsal_records": len(rehearsal_fit),
                },
                "constrained-update": {
                    "train_records": len(train_fit),
                    "constraint": "parent-trust-region",
            ...
            "rehearsal_candidate_set_digests": [
                record["candidate_set"].candidate_set_digest for record in rehearsal_records
            ],
        ...
        payload.update(
            {
                ...
                "trust_region": {
                    "ratio": TRUST_REGION_RATIO,
                    "parent_norm": parent_norm,
                    "radius": trust_region_radius,
                },
                "retention_repaired": retention_repaired,
                "constrained_new_task_noninferior": constrained_new_task_noninferior,
                "constrained_new_task_gain": constrained_new_task_gain,
                "update_rule_repaired": update_rule_repaired,
                "outcome": outcome,
                "experiment_passed": bool(all_gates and update_rule_repaired),
                "growth_admitted": False,
                "can_promote": False,
                "interpretation": (
                    f"completed: outcome={outcome}; fixed-capacity update-rule comparison; "
