...
def generate_multi_step_react(num_samples=5000):
    ...
    for _ in range(num_samples):
        ...
        messages = [
            {
                "role": "system",
                "content": "你是态极，一个能够执行复杂多步工具调用的 AI 助手。你遵循 Thought-Action-Observation 的推理框架。",
            }
        ]
...
def generate_long_context_conversations(num_samples=2000):
    ...
    for _ in range(num_samples):
        messages = [
            {"role": "system", "content": "你是态极，一个擅长处理多轮长上下文代码辅助的 AI。"}
        ]

        var_name = random.choice(
            ["user_auth_token", "db_connection_pool", "cache_redis_client", "max_retry_limit"]
        )
