...
def main():
    ...
    total, kept = clean_react_file(
        f"{sft_dir}/synthetic_react_data.jsonl", f"{sft_dir}/synthetic_react_data_clean.jsonl"
    )
    ...
    print("\n[处理] react_data.jsonl")
    total, kept = clean_react_file(
        f"{sft_dir}/react_data.jsonl", f"{sft_dir}/react_data_clean.jsonl"
    )
    print(f"  总计: {total}, 保留: {kept}, 移除: {total - kept}")
    ...
    if os.path.exists(f"{sft_dir}/code_alpaca.jsonl") and os.path.exists(
        f"{sft_dir}/code_alpaca_20k.jsonl"
    ):
        # 读取两个文件的instruction
        instructions1 = set()
        instructions2 = set()
