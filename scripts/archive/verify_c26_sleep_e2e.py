...
def main():
    ...
    try:
        ...
        check(
            "A3. sleep 编排：synaptic_consolidation 再次执行",
        ...
        check(
            "E. 磁盘恢复：consolidated 标记保留",
            ok5
            and all(
                e["consolidated"] == i["high_freq"] for i, e in zip(MEMORY_ITEMS, bank2.entries)
            ),
        )
        ...
        for item in [i for i in MEMORY_ITEMS if i["high_freq"]]:
            ...
            print(
                f"    {item['key'] if 'key' in item else item['label']}: " f"{out[:24]!r}",
                flush=True,
            )
        check(
            "F. 记忆向量条件化生成非空（不破坏生成）",
