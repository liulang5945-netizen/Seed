...
def main():
    ...
    check(
        "A1. 节奏参数注册为 Parameter（omega/coupling/gaba_amp + gaba_vec buffer）",
        all(k in sd_keys for k in ("omega", "coupling", "gaba_amp", "gaba_vec")),
        f"keys={sd_keys}",
    )
    osc.step(dt=1.0)
    check(
        "A2. float 状态兼容（phase property / step 推进）",
        abs(osc.phase - 0.5) < 1e-6,
        f"phase={osc.phase:.4f}",
    )
    u = osc.unit()
    check(
        "A3. unit 兼容（float 状态 cos/sin）",
    ...
    theta.sum().backward()
    check(
        "A5. theta_tensor 对 omega 可微（dθ/dω=t）",
        osc.omega.grad is not None and abs(float(osc.omega.grad.item()) - 2.0) < 1e-6,
    ...
    gate_t.sum().backward()
    check(
        "A6. gaba_gate_tensor 对 omega 可微（门控进梯度流）",
    ...
    try:
        ...
        osc_c = OscillatorNode(
            nid="osc_t", omega=0.5, coupling=0.4, gaba_amp=0.08, dim=16, phase=0.0
        )
        ...
        new_p = pd.evolve(
            ...
            external_phases=[ep],
            external_weights=[osc_c.coupling],
        )
        loss = new_p.sum()
        loss.backward()
        check(
            "B1. 牵引权重（coupling）经 evolve 可微",
            osc_c.coupling.grad is not None and float(osc_c.coupling.grad.abs().item()) > 0,
            f"grad_coupling={None if osc_c.coupling.grad is None else float(osc_c.coupling.grad):.4f}",
        )
        check(
            "B2. 牵引相位（ω）经 evolve 可微",
            osc_c.omega.grad is not None and float(osc_c.omega.grad.abs().item()) > 0,
    ...
    check(
        "C0. 装配振荡器为 nn.Module（含可学习参数）",
        len(oscs) == 2
        and all(
            hasattr(o, "omega") and hasattr(o, "coupling") and hasattr(o, "gaba_amp") for o in oscs
        ),
    )
    try:
        emb_in = torch.tensor(
        ...
        def _spy_evolve(*a, **k):
            ...
            if _ew:
                seen_ew["count"] += 1
                seen_ew["all_tensor"] = seen_ew["all_tensor"] and all(
                    torch.is_tensor(x) for x in _ew
                )
            return _orig_evolve(*a, **k)

        with (
            mock.patch.object(OscillatorNode, "phase_unit_tensor", _spy_ph),
            mock.patch.object(cortex.ensemble.gamma_oscillator, "evolve", _spy_evolve),
        ):
        ...
        check(
            "C1. 振荡器可微相位参与训练（phase_unit_tensor 被调用）",
            seen_ph["count"] > 0,
            f"calls={seen_ph['count']}",
        )
        rl = out.get("osc_rhythm_loss")
        check(
            "C2. osc_rhythm_loss 计算（有限值）",
    ...
    osc_p = OscillatorNode(
        nid="osc_persist", omega=0.5, coupling=0.4, gaba_amp=0.08, dim=8, phase=0.0
    )
    with torch.no_grad():
    ...
    osc_p2 = OscillatorNode(
        nid="osc_persist", omega=0.5, coupling=0.4, gaba_amp=0.08, dim=8, phase=0.0
    )
    osc_p2.load_state_dict(sd_p)
    ok_roundtrip = (
        abs(osc_p2.omega.item() - osc_p.omega.item()) < 1e-6
    ...
    check(
        "D1. OscillatorNode state_dict 往返保留参数",
        ok_roundtrip,
    ...
    check(
        "D2. cortex.save_state/load_state 含 oscillators 持久化集成",
        '"oscillators"' in _src,
        "save_state 写 + load_state 读",
    )
    check(
        "D3. 装配注入仍为 theta+gamma 双层（set_oscillators 兼容）",
    ...
    try:
        ...
        check("E2. get_last_phase 正常", isinstance(lp, float), f"phase={lp}")
    except Exception as e:
