...
def compute_avg_loss_ensemble(ens, embeddings, general_sp, text, fusion_mode="soft", seq_len=64):
...
def main():
    ...
    ap.add_argument(
        "--ckpt",
        default="data/neurons/collab_v1_mixed.ckpt.pt",
        help="协作层 checkpoint 路径（评估 v2 用 --ckpt data/neurons/collab_v2_routing.ckpt.pt）",
    )
    ap.add_argument("--n-eval", type=int, default=8, help="每域评估文本条数（seed 固定，可复现）")
    args = ap.parse_args()
    ...
    for src in DOMAINS + ["dialogue"]:
        ...
        print(
            f"  {src:8s} {math.exp(cs) if cs==cs else float('nan'):10.2f} "
    ...
    for src in DOMAINS:
        ...
        print(
            f"  {src:8s} {math.exp(cg) if cg==cg else float('nan'):10.2f} "
            f"{math.exp(ind) if ind==ind else float('nan'):10.2f} {es:9.1f}%"
        )
