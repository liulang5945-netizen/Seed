...
def main():
    ...
    parser.add_argument("--hub-ckpt", default="data/hub_neuron/neuron_hub.pt", help="hub ckpt 路径")
    parser.add_argument(
        "--collab-name", default=COLLAB_NAME, help="协作层 ckpt 文件名（含 hub 通道的训练产物）"
    )
    ...
    try:
        ...
        check(
            "A1. 10 neuron 综合体装配（hub 规格正确）",
            len(cortex.neurons) == 10
            and hn.config.spec == "expert"
