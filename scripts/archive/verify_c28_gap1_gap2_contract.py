...
def main() -> None:
    print("=" * 64, flush=True)
    print("C28 Gap 1（场记忆自动捕获）+ Gap 2（coaction 连续路径）回归守卫", flush=True)
    print("=" * 64, flush=True)
    ...
    record_calls: list = []
    original_get_sleep = (
        cortex_mod.get_sleep_engine if hasattr(cortex_mod, "get_sleep_engine") else None
    )
    ...
    try:
        ...
        cortex_mod.Cortex._capture_field_memory(mock_cortex, "测试 prompt 内容", "生成的回复文本")
    finally:
        sleep_mod.get_sleep_engine = original_global_get

    check(
        "G1.1a record_field_memory 被调用 1 次",
        len(record_calls) == 1,
        f"calls={len(record_calls)}",
    )
    if record_calls:
        c = record_calls[0]
        check(
            "G1.1b field_state 正确传入（非空张量）",
            c["vector_norm"] is not None and c["vector_norm"] > 0,
            f"norm={c['vector_norm']:.4f}",
        )
        check(
            "G1.1c label 来自 prompt[:40]",
            c["label"] == "测试 prompt 内容",
            f"label={c['label']!r}",
        )
        check("G1.1d text 是生成结果", c["text"] == "生成的回复文本", f"text={c['text']!r}")
        check("G1.1e phase 来自 get_last_phase()", c["phase"] == 1.234, f"phase={c['phase']}")
    else:
    ...
    check(
        "G1.2 generate(auto_capture=True) → _generate_p7 收到 auto_capture=True",
    ...
    check(
        "G1.3a generate(auto_capture=False) → _generate_p7 收到 auto_capture=False",
    ...
    check(
        "G1.4 generate() 默认 → auto_capture=True 透传给 _generate_p7",
        len(generate_p7_calls) >= 1 and generate_p7_calls[-1] is True,
    ...
    check("G1.5a _generate_p7 源码含 auto_capture 参数", "auto_capture" in p7_src, "")
    check(
        "G1.5b _generate_p7 源码含 'if auto_capture and result_text.strip()' 门控",
        "if auto_capture and result_text.strip()" in p7_src,
        "",
    )
    check(
        "G1.5c _generate_p7 源码含 _capture_field_memory 调用",
    ...
    check(
        "G1.6a generate() 源码含 auto_capture: bool = True 默认",
        "auto_capture: bool = True" in gen_src,
        "",
    )
    check(
        "G1.6b generate() 把 auto_capture 透传给 _generate_p7（单候选路径）",
    ...
    check(
        "G1.7 _capture_field_memory 方法定义存在且调 record_field_memory",
    ...
    check(
        "G2.1 continuous_forward 源码包含 coaction.update 调用",
    ...
    check(
        "G2.2a continuous_forward 的 coaction.update 带 round_num=t+1",
        "round_num=t + 1" in cf_src or "round_num=t+1" in cf_src,
        "",
    )
    check(
        "G2.2b continuous_forward 的 coaction 带 try/except 非致命包装",
    ...
    check(
        "G2.3 continuous_forward coaction 带 len(active_this) >= 2 门控",
    ...
    check(
        "G2.4a 离散 forward 也有 coaction.update（对照基准）",
    ...
    check(
        "G2.5 continuous_forward 至少 1 处 coaction.update",
        cf_coaction_count >= 1,
        f"cf_count={cf_coaction_count}",
    )
    check(
        "G2.5b 离散 forward 至少 1 处 coaction.update（对照）",
    ...
    print("\n" + "=" * 64, flush=True)
    if failed == 0:
        print(f"判定: 全部 {passed} 维 PASS — Gap 1 + Gap 2 契约闭合", flush=True)
        print("  Gap 1: generate() 默认自动沉淀场记忆；auto_capture=False 可隔离", flush=True)
        print("  Gap 2: continuous_forward 每步补全 coaction 统计", flush=True)
    else:
