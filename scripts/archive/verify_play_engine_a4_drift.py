...
def main():
    ...
    print("=" * 64, flush=True)
    print(
        f"自举门槛 A4 完整：{N_MICRO} 次 micro-sleep + "
        f"{N_MICRO // CHECKPOINT_EVERY} 个 checkpoint",
        flush=True,
    )
    print("=" * 64, flush=True)
    ...
    print("\n[2/4] 预测量 pre-sleep A1 真实版 + 注入 24 条记忆（同 A4 准备）...", flush=True)
    ...
    for g in ("dialogue", "knowledge", "unfamiliar"):
        ...
        if s_pre is None or s_post is None:
            check(f"A4.{g[0]}: post std >= pre × 0.95", False, f"pre={s_pre} post={s_post}")
            continue
        ratio = s_post / s_pre if s_pre > 0 else None
        ok = ratio is not None and s_post >= s_pre * 0.95
        check(
            f"A4.{g[0]}: post std >= pre × 0.95",
            ok,
            (
    ...
    check("A4e: 100 次 micro-sleep 无崩溃", crash_count == 0, f"crashes={crash_count}/{N_MICRO}")

    a4_pass = failed == 0
    if a4_pass:
        verdict = "A4 完整 PASS：100 次 micro-sleep 后 judge 信号不退化，" "演化曲线无 10% 级跳水"
        next_step = (
            "A4 完整通过——play 引擎常态化下经验不破坏 judge。下一步 "
