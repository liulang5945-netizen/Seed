...
def main():
    ...
    try:
        ...
        q = cortex._rolling_nll_quality(
            res, "这是已经生成的测试文本内容，用于验证滚动后验窗口计算。", "zh", window=12
        )
        ok = bool(q) and all(isinstance(v, float) and v == v and abs(v) < 1e4 for v in q.values())
        check(
            "A1. 滚动 NLL 输出合法（非空、有限值）",
            ok,
    ...
    base_scores_uneven = {k: (0.9 if k == "zh_aug0_dialogue" else 0.5) for k in zh_domain_all}
    nll_only_aug1_bad = {k: (-5.0 if k == "zh_aug1_dialogue" else -1.0) for k in zh_domain_all}
    result_uneven = {"round1_scores": base_scores_uneven}

    def _evolve(
        active, streaks=None, nll=nll_only_aug1_bad, probe_ret=None, result=None, gids=None
    ):
        with (
            mock.patch.object(cortex, "_rolling_nll_quality", return_value=nll),
            mock.patch.object(cortex, "_probe_inactive_fused", return_value=probe_ret or {}),
        ):
            return cortex._instance_route_evolve(
                list(active),
                result or result_uneven,
                "已生成文本",
                "zh",
                dict(streaks or {}),
                window=12,
    ...
    check(
        "B1b. 连续劣化剔除（chunk 级收敛）",
        "zh_aug1_dialogue" not in n2 and set(n2) == set(act) - {"zh_aug1_dialogue"},
        f"n2={n2}",
    )
    # B3（并入）：跨域 neuron 不动
    n3, _ = _evolve(act + ["code"], {})
    check(
        "B3. 跨域 neuron 不被演化（C22 域内约束）",
        "code" in n3 and "zh_aug1_dialogue" in n3,
        f"n3={n3}",
    )
    # B2: 加入（未激活同域后验显著更高）
    nll_aug1_best = {k: (-1.0 if k == "zh_aug1_dialogue" else -5.0) for k in zh_domain_all}
    n4, _ = _evolve(
        ["zh_aug0_dialogue", "zh_std0_dialogue", "zh"],
        nll=nll_aug1_best,
        probe_ret={"zh_aug1_dialogue": 0.7, "zh_aug2_dialogue": 0.1},
    )
    check("B2. 未激活同域后验显著更高 → 加入（双向）", "zh_aug1_dialogue" in n4, f"n4={n4}")
    # B4: min_active 保护（全劣化剔到下限保护）
    nll_all_bad = {k: (-1.0 if k == "zh_aug0_dialogue" else -5.0) for k in zh_domain_all}
    ...
    check("B4. min_active 保护（同域激活 >= 2）", len(dom_keep) >= 2, f"cur={cur}")
    # B5: 稳定（无劣化时激活集不变）
    nll_equal = {k: -1.0 for k in zh_domain_all}
    base_equal = {k: 0.5 for k in zh_domain_all}
    n5, s5 = _evolve(zh_domain_all, {}, nll=nll_equal, result={"round1_scores": base_equal})
    check(
        "B5. 无劣化时激活集不变（迟滞收敛）",
    ...
    try:
        ...
        check(
            "C1. 长生成输出非空", isinstance(out, str) and len(out.strip()) > 0, f"out={out[:40]!r}"
        )
        # 集成触发证据：chunk 评估点至少触发一次（演化逻辑正确性由 B 段单元
        # 保障；触发次数依赖实际生成长度——模型可能因 EOS/退化截断提前停止，
        # 且 BioOSS 牵引会改变生成分布，故不绑定固定次数）。
        check("C2. 实例级路由在 chunk 边界触发", calls_c["n"] >= 1, f"evolve_calls={calls_c['n']}")
        check("C3. 触发后输出不退化", not cortex._is_degenerate_text(out), f"out={out[:40]!r}")
    except Exception as e:
    ...
    try:
        ...
        check(
            "D1. 关闭时生成正常（非空不退化）",
            isinstance(out_off, str)
            and len(out_off.strip()) > 0
            and not cortex._is_degenerate_text(out_off),
            f"out={out_off[:40]!r}",
        )
        check("D2. 关闭时实例级路由不调用", calls_d["n"] == 0, f"evolve_calls={calls_d['n']}")
    except Exception as e:
