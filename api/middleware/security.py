...
# ===== 速率限制 =====
#
# WARNING: RateLimiter uses an in-process Python dict for request tracking.
# This means:
#   - Each uvicorn worker maintains its own independent counter.
#   - With N workers, the effective limit = max_requests × N.
#   - For single-worker deployments (default desktop mode), this is fine.
#   - For multi-worker deployments, consider:
#       1. Running behind a reverse proxy (nginx) with its own rate limit.
#       2. Replacing the dict backend with Redis/shared memory.
#       3. Keeping worker count at 1 (recommended for desktop use).
#
class RateLimiter:
    ...
    def is_allowed(self, client_id: str) -> bool:
        now = time.time()
        # 清理过期请求记录
        remaining = [ts for ts in self.requests[client_id] if now - ts < self.window_seconds]
        # 内存修复：过期后列表为空则删除键，避免键随客户端数量无限增长
        if remaining:
            self.requests[client_id] = remaining
        else:
...
class AuthValidator:
    ...
    VALID_API_KEYS = {k.strip() for k in _env_keys.split(",") if k.strip()} if _env_keys else set()
...
def create_rate_limit_middleware(limiter_instance: RateLimiter):
    ...
    async def rate_limit_middleware(request: Request, call_next: Callable):
        ...
        if not limiter_instance.is_allowed(client_id):
            return JSONResponse(
                status_code=429, content={"error": "Too many requests, please retry later"}
            )
