...
class BrowserCrawler:
        ...
        while to_visit and collected < max_pages:
            ...
            for url, cur_depth in batch:
                ...
                if result:
                    ...
                    collected += 1
                    logger.info(
                        f"  [{collected}/{max_pages}] {result.title[:60]} ({result.url[:80]})"
                    )
                    if cur_depth < depth:
    ...
    def browse_with_pagination(self, seed_url, max_pages=20):
        ...
        try:
            while collected < max_pages:
                ...
                self._pages.append(
                    BrowsedPage(url=current_url, title=title, content=content, ts=time.time())
                )
                collected += 1
                logger.info(f"  [{collected}/{max_pages}] {title[:60]} ({current_url[:80]})")
                next_url = self._eval(
                    page,
                    "var s=document.querySelector('a[rel=next], a:has-text(next)'); return s?s.href:null",
                )
    ...
    def _browse_single(self, url, depth):
        context = self._browser.new_context(
            viewport={"width": 1280, "height": 800}, user_agent="Mozilla/5.0"
        )
        pg = context.new_page()
        try:
            ...
            return BrowsedPage(
                url=url, title=title, content=content[:20000], links=links[:30], ts=time.time()
            )
    ...
    def _extract_main(self, pg):
        try:
            ...
            js = (
                r'const m=document.querySelector("main,article,[role=main],.content,#content");'
                + js
            )
    ...
    def _extract_visible_links(self, pg, base_url):
        try:
            ...
            for l in links[:50]:
                try:
                    if urllib.parse.urlparse(l).netloc == base_domain and not any(
                        x in l for x in [".jpg", "png", "pdf", "zip", "css", "js"]
                    ):
                        res.append(l)
                except BaseException as e:
                    logger.debug(
                        "【BrowserCrawler._extract_visible_links】处理失败（非致命）: %s", e
                    )
            return list(set(res))
        except Exception:
            return []
    ...
    def _save_results(self):
        ...
        try:
            ...
            with open(
                os.path.join(self.data_dir, "browse_results.json"), "w", encoding="utf-8"
            ) as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
        except BaseException as e:
            logger.debug("【BrowserCrawler._save_results】处理失败（非致命）: %s", e)

    def stats(self):
        return {"pages": len(self._pages), "dir": self.data_dir}
...
def browser_crawl(url):
    ...
    try:
        ...
        if not pages:
            return f"Browse {url} returned no pages."
